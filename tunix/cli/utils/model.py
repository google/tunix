...
ModelModule = automodel.ModelModule
...
def _get_base_model(model_config: dict[str, Any], mesh: jax.sharding.Mesh):
  ...
  model_params = automodel.obtain_model_params(model_config['model_name'])
...
def _create_model_from_checkpoint(
  ...
  This function is used to create a model from a checkpoint. It is used for
  models that have support for `create_model_from_checkpoint` in their params
  module, such as Gemma3.
  ...
  """
  model_params = automodel.obtain_model_params(model_name)
  params_lib = automodel.get_model_module(model_name, ModelModule.PARAMS)
...
def _create_model_from_params(
  ...
  model_module_lib = automodel.get_model_module(model_name, ModelModule.MODEL)
...
def create_model(
  ...
  if model_name.startswith('gemma3') and model_source == 'gcs':
    ...
    model, model_params = _create_model_from_checkpoint(
  ...
  # gemma is ready.
    ...
    def nnx_conversion():
      ...
      model, params = _create_model_from_params(params_path, model_name)
