...
class ToyTransformer(nnx.Module):
  ...
  def __call__(
    ...
    if attention_mask is not None:
      N = x.shape[1]  # pytype: disable=attribute-error
      mask = attention_mask[..., :N]
    elif query_lens is not None: # Handle RPA Case
      B = query_lens.shape[0]
      N = x.shape[0]  # pytype: disable=attribute-error

      if N == B: # Decode
        mask = jnp.eye(x.shape[0], dtype=jnp.bool_)[None, :, :]  # pytype: disable=attribute-error
      else: # Prefill
        batch_idxs = jnp.repeat(jnp.arange(B), query_lens, total_repeat_length=N)

        n_valid = jnp.sum(query_lens)
