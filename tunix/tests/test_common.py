...
class ToyTransformer(nnx.Module):
  ...
  def __call__(
    ...
    # Generate masking so that attention is identical
    # for the ragged and batched cases.
    if attention_mask is not None:  # Batched case
      N = x.shape[1]
      mask = attention_mask[..., :N]
    elif seq_lens is not None:  # Ragged case
      B = seq_lens.shape[0]
      N = x.shape[0]
      ...
      raise ValueError(
          'Attention mask and sequence lengths cannot both be None'
      )
