...
Array: TypeAlias = jax.Array | jax.ShapeDtypeStruct | np.ndarray
ModelInputs: TypeAlias = jaxtyping.PyTree[Array]


def _shape_and_dtype(name: str, value: Any) -> tuple[tuple[int, ...], Any]:
  if not isinstance(
      value, (jax.Array, jax.ShapeDtypeStruct, jax.core.Tracer, np.ndarray)
  ):
    raise TypeError(f"{name} must be a JAX/NumPy array or ShapeDtypeStruct")
...
def _concrete_numpy(value: Any) -> np.ndarray | None:
  ...
  if isinstance(value, (jax.ShapeDtypeStruct, jax.core.Tracer)):
    return None
