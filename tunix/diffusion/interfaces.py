...
"""Callable interfaces for model-agnostic JAX diffusion integrations."""
...
class DiffusionBatchAdapter(Protocol[RawBatchT_contra]):
  ...
  def __call__(self, batch: RawBatchT_contra, /) -> types.DiffusionTokenBatch:
    """Prepares a raw batch as target-aligned diffusion arrays."""
    ...
...
class DiffusionLogitsFn(Protocol):
  ...
  def __call__(
    ...
    """Returns unvalidated, target-aligned logits from a model scorer.

    ``compute_diffusion_logits`` invokes this callable and validates its output
    as floating-point ``[batch, length, vocabulary]`` logits against the
