...
class Sampler(base_sampler.BaseSampler):
  ...
  def _sample(
    ...
    if sampler_state.sampling_mode == 'beam_search':
      ...
      if sampler_state.sampling_mode == 'greedy':
        ...
        jax.debug.print("KEY: {i}", i=jax.device_get(decoding_step))
        jax.debug.print("Seed: {i}", i=jax.device_get(sampler_state.seed))
  ...
  def _prefill_fn(
    ...
    positions = sampler_state.positions
    beam_search_sampling_state = None
    jax.debug.print("ORIG LOGITS: {i}", i=logits)
  ...
  def _decode_fn(
    ...
    def cond_fn(sampler_state: _SamplingState):
      ...
      while cond_fn(sampler_state):
        sampler_state = sample_with_params(sampler_state)
  ...
  def __call__(
    ...
    # TODO: Images and audios need to processed in a ragged format like tokens 
    processed_images = None
    if is_gemma4 and images is not None:
      raise ValueError("Images are not yet supported.")

    processed_audios = None
    if is_gemma4 and audios is not None:
      raise ValueError("Audios are not yet supported.")
