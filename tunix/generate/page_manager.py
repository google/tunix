...
class Block:
  ...
  def write_values(self, num_values: jax.Array, values: jax.Array,
                   page_indices: jax.Array, page_offsets: jax.Array, layer_id: str = "tokens") -> 'Block':
    """Write packed 1D array of values into allocated paged memory of block."""
    max_n_pages = self.pages[layer_id].shape[0]
    batch_size = values.shape[0]

    target_page_indices = jnp.where(
        jnp.arange(batch_size) < num_values, page_indices, max_n_pages)

    updated_arr = self.pages[layer_id].at[target_page_indices, page_offsets].set(values, mode='drop')
    new_pages = dict(self.pages)
    new_pages[layer_id] = updated_arr
...
class TpuCpuPageManager:
  ...
  seq_lens: jax.Array  # i32[batch_size] # TODO: rename to seq_num_pages
  ...
  @jax.named_call
  def update_tpu_layer_pages(self, layer_id: str, updated_layer_pages: jax.Array):
    new_pages = {**self.tpu_block.pages, layer_id: updated_layer_pages}
    new_tpu_block = dataclasses.replace(
        self.tpu_block,
        pages=new_pages
    )
  ...
  def offload(self, num_pages: int | jax.Array,
              page_idxs: jax.Array, layer_id: str = "tokens") -> tuple['TpuCpuPageManager', jax.Array]:
    ...
    tpu_vals = self.tpu_block.pages[layer_id][safe_tpu_phys]
    tpu_vals_cpu = _put_on_target_device(tpu_vals, self.cpu_block.pages[layer_id])

    safe_cpu_phys = jnp.where(is_real,
                              padded_allocated_page_data[:page_idxs.shape[0]],
                              self.cpu_block.pages[layer_id].shape[0])
    
    updated_arr = self.cpu_block.pages[layer_id].at[safe_cpu_phys].set(tpu_vals_cpu, mode='drop')
    new_cpu_pages = dict(self.cpu_block.pages)
    new_cpu_pages[layer_id] = updated_arr
  ...
  def load(self, num_pages: int | jax.Array,
    ...
    cpu_vals = self.cpu_block.pages[layer_id][safe_cpu_phys]
    cpu_vals_tpu = _put_on_target_device(cpu_vals, self.tpu_block.pages[layer_id])

    safe_tpu_phys = jnp.where(is_real,
                              padded_allocated_page_data[:page_idxs.shape[0]],
                              self.tpu_block.pages[layer_id].shape[0])
    
    updated_arr = self.tpu_block.pages[layer_id].at[safe_tpu_phys].set(cpu_vals_tpu, mode='drop')
    new_tpu_pages = dict(self.tpu_block.pages)
    new_tpu_pages[layer_id] = updated_arr
  ...
  def load_values(self, values: jax.Array,
                  lens: jax.Array, layer_id: str = "tokens") -> 'TpuCpuPageManager':
    ...
    new_tpu_block = self.tpu_block.write_values(values_ragged.capacity, values,
                                                phys_page_ids, page_offsets, layer_id)
  ...
  def to_array(
    ...
    packed_tokens = self.tpu_block.pages[layer_id][phys_page_ids, page_offsets]
