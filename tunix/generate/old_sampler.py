...
class RaggedArray:
  ...
  def intra_offsets(self) -> jax.Array:
    cum_sums = jnp.cumulative_sum(self.lens, include_initial=True)
...
class LayerCacheConfig:
  ...
  # TODO: These should be gathered using the mesh
  ...
  def init(self, page_indices, available_page_indices, num_available_pages) -> 'LayerCache':
    # TODO: Propery set KV packing
    kv_packing = 2
    pages = jax.lax.empty(
      (
          self.max_num_pages,
    ...
    num_pages_per_shard = self.max_num_pages // self.num_shards
    
    # TODO: Investigate why memory errors occur when
    # page indicies is not initalized. 
    # 
    # Indices should only be accessed for allocated pages and 
    # thus should not need to be initalized. 
    ...
    return LayerCache(
      ...
      max_num_pages_per_seq_per_shard=self.max_num_pages_per_seq_per_shard,
...
class LayerCache:
  ...
  num_shards: int 
  window_size: int | None = None
  max_num_pages_per_seq_per_shard: int
  ...
  def kv_pages(self) -> jax.Array:
    return jnp.stack([self.k_pages, self.v_pages], axis=1)
  """
  ...
  def allocate(self, q_lens: jax.Array) -> Self:
    ...
    total_pages_required = cdiv(self.kv_lens + q_lens, self.page_size)
    ...
    page_indices_to_allocate = RaggedArray(
      data=self.available_page_indices, 
      lens=num_pages_to_allocate
    )
    page_indices_rows = page_indices_to_allocate.row_idxs 
    page_indices_cols = self.num_local_pages[page_indices_rows] + page_indices_to_allocate.intra_offsets 

    # TODO: Verify that pages are available before allocation 
    # pages_to_allocate = self.available_page_indices[:page_indices_to_allocate.total_length]    
    updated_page_indices = self.page_indices.at[page_indices_rows, page_indices_cols].set(
        page_indices_to_allocate.data
    )
    
    updated_num_available_pages = self.num_available_pages - page_indices_to_allocate.total_length
    updated_available_page_indices = jnp.roll(self.available_page_indices, -page_indices_to_allocate.total_length)
  ...
  def release(self, should_release: jax.Array) -> Self:
    # should_release b[batch_size]
    
    # Set released cache lengths to 0
    ...
    # Append released pages indices (unavailable page indices 
    # are treated as buffer) 
...
def _init_cache(
  ...
  page_indices=jnp.zeros(
    (layer_cfg.batch_size, layer_cfg.max_num_pages_per_seq_per_shard),
    dtype=jnp.int32,
  )
  num_pages_per_shard = layer_cfg.max_num_pages // layer_cfg.num_shards
  ...
  return {
      f'layer_{i}': layer_cfg.init(page_indices, available_page_indices, num_available_pages),
      f'page_indices': page_indices,
      f'available_page_indices': available_page_indices,
      f'num_available_pages': num_available_pages,
...
class Sampler(base_sampler.BaseSampler):
  ...
  def init_sample_state(
      self,
      all_input_ids: jax.Array, # 1D Array of tightly packed valid tokens
      lens: jax.Array, # 1D Array of sequence lengths
      total_sampling_steps: int,
      include_logits: bool,
      forbidden_token_ids: tuple[int, ...] | None,
      temperature: float,
    ...
    capacity = batch_size * max_prompt_length

    if False and hasattr(self.transformer, 'init_cache'):
    ...
    # valid_mask = all_input_ids != self.tokenizer.pad_id()
    # flat_mask = valid_mask.flatten()

    data = jnp.full((capacity,), self.tokenizer.pad_id(), dtype=jnp.int32)
    data = data.at[:batch_size].set(all_input_ids)

    token_buffer = RaggedArray(
        data=data,
        lens=lens
    )

    token_buffer = jax.lax.empty(
      (
          max_num_pages,
  ...
  def __call__(
    ...
    # TODO: We should not pad tokens if using ragged pagged attention. 
    # We need to update other types of attention to use non-padded tokens
    """
