...
class VanillaSampler:
  ...
  def _queue_new_requests(
    ...
    prompt_tokens = [self._tokenize(p) for p in prompts)
    lens = [(len(p) for p in prompt_tokens)]
    
    cache = cache.allocate(lens)
    cache = cache.load_prompt_tokens(prompt_tokens, lens=lens)
  ...
  def _prefill_fn(
    ...
    page_cols = intra_offsets // cache.page_size
    page_offsets = intra_offsets % cache.page_size
  ...
  def _sample_step_fn(
    ...
    is_active = ~dones
    active_seq_lens = is_active.astype(int)
    ...
    token_idxs = (cache.seq_lens - 1)

    page_cols =  token_idxs // cache.page_size
    page_offsets = intra_offsets % cache.page_size
    phys_page_ids = cache.page_indices[seq_idxs, page_cols]
