...
from .vllm_async_driver import VLLMAsyncDriver
...
class VllmSampler(base_sampler.BaseSampler):  # pylint: disable=invalid-name
  ...
  def __init__(
    ...
    self._driver: VLLMAsyncDriver | None = None
  ...
  def _create_driver(self) -> VLLMAsyncDriver:
    engine_args = self._build_engine_args()
    return VLLMAsyncDriver.from_engine_args(
        engine_args,
    )
  ...
  def _model_runner(self):
    ...
    if self._driver is not None:
      ...
      raise RuntimeError(
          "Model runner access is not available in async server mode."
      )
