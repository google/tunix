...
class VanillaSampler:
  ...
  def _load_requests(self, sampling_state: _SamplingState) -> _SamplingState:
    ...
    q_lens = jnp.zeros([len(r) for p in prompt_tokens], jnp.int32) 
    cache = cache.allocate(q_lens=q_lens)
    cache = cache.load_prompt_tokens(prompt_tokens, lens=q_lens, key="token_buffer")
  ...
  def _model_step_fn(
    ...
    is_decode = jnp.arange(max_seqs) < n_decode
