...
def main():
  ...
  parser.add_argument("--name", type=str, default="rollout", help="Name of the rollout worker")
  parser.add_argument("--registration_service_address", type=str, default="localhost:12345", help="Address of the registration service")
  parser.add_argument("--rollout_service_port", type=int, default=11111, help="Port of the rollout worker service")
  args = parser.parse_args()

  is_local = "localhost" in args.registration_service_address or "127.0.0.1" in args.registration_service_address
  ...
  with grpc.insecure_channel(args.registration_service_address) as channel:
    ...
    while True:
      try:
        ...
        logging.info(f"[{args.name}] register {"succeeded" if response.success else "failed"}: {response.reply}")
        break
        ...
        if e.code() == grpc.StatusCode.UNAVAILABLE:
          ...
          raise RuntimeError(f"[{args.name}] register failed: {e.code()} - {e.details()}")
