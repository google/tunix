...
class VLProcessor:
  ...
  def __call__(
  ...
  ) -> list[list[np.ndarray]] | tuple[list[list[np.ndarray]], list[str]]:
    """Pre-process images and text.
    ...
    batch). Also, optionally preprocesses the text.
    ...
    Args:
      ...
      text: The text to pre-process. Mostly for adding image placeholder tokens.
    
    Returns:
      If text is None, returns the processed images. Otherwise, returns the
      processed images and text.
    """
    ...
    if text is not None:
      if not isinstance(text, list):
        text = [text]
      preprocessed_text = [self.preprocess_text(t) for t in text]
      return processed_images, preprocessed_text
  ...
  def preprocess_text(self, text: str):
    ...
    This is a placeholder function that will be replaced by the model-specific
    VL processor.
