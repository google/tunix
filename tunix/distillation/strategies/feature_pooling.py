...
def compute_cosine_distance_loss(
    student_features: jax.Array, teacher_features: jax.Array
) -> jax.Array:
  """Computes the cosine distance between two feature maps."""
  return jnp.mean(optax.cosine_distance(student_features, teacher_features))
...
class FeaturePoolingStrategy(base_strategy.BaseStrategy):
  ...
  def __init__(
    ...
    self.alpha = alpha
    self.feature_loss_fn = feature_loss_fn or compute_cosine_distance_loss
  ...
  def compute_loss(
    ...
    teacher_features = feature_extraction.reduce_array_to_target_shape(
