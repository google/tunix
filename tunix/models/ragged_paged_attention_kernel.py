...
def _ragged_paged_attention_kernel(*args, **kwargs):
    ...
    if return_logits:
        logits_hbm_ref = args[12]
        scratch_refs = args[13:]
    else:
        logits_hbm_ref = None
        scratch_refs = args[12:]
    
    args_to_pass = args[:12] + (logits_hbm_ref,) + scratch_refs
    ...
    def flash_attention_step1_qk_softmax(
        ...
        *,
        ...
        if kwargs.get("return_logits", False):
            @pl.when(kwargs.get("return_logits", False))
            def _save_logits():
                from jax.experimental.pallas import tpu as pltpu
                if logits_hbm_ref is not None:
                    pltpu.make_async_copy(
                        src=s,
                        dst=logits_hbm_ref.at[seq_idx, kv_head_idx, pl.ds(bq_start * num_q_heads_per_kv_head, actual_bq_csz * num_q_heads_per_kv_head), pl.ds(bkv_start, bkv_csz)],
                        sem=sems[0,0],
                        wait=False
                    )
...
def ragged_paged_attention(
  ...
  """
    ...
    def run_rpa_kernel(
        ...
        *,
        ...
        kernel = pl.pallas_call(
            ...
            ) + ([pltpu.HBM(shape=(kv_lens.shape[0], q.shape[0], q.shape[1] * num_q_heads_per_kv_head, pages_per_seq * page_size), dtype=q.dtype) if tpu_version >= 7 else jax.ShapeDtypeStruct(shape=(kv_lens.shape[0], q.shape[0], q.shape[1] * num_q_heads_per_kv_head, pages_per_seq * page_size), dtype=q.dtype)] if return_logits else []),
        ...
        res = run(scalar_prefetches, q, kv, kv_cache)
        return (res[0], res[1], res[2]) if return_logits else (res[0], res[1], None)
    ...
    # Decode-only
    q, kv_cache, logits_d = run_rpa_kernel(
        q,
        kv_cache,
    ...
    if chunk_prefill_size is not None:
        # Prefill-only
        q, kv_cache, logits_p = run_rpa_kernel(
            q,
            kv_cache,
    ...
    # Mixed
    q, kv_cache, logits_m = run_rpa_kernel(
        q,
        kv_cache,
    ...
    if return_logits:
        logits = (logits_d if logits_d is not None else 0) + \
                 (logits_p if logits_p is not None else 0) + \
                 (logits_m if logits_m is not None else 0)
    else:
        logits = None
