...
"""Performance benchmark and XProf profiler for Gemma4 Attention kernels.
...
_ATTN_TYPE = flags.DEFINE_enum(
    "attn_type",
    "GLOBAL",
    ["GLOBAL", "LOCAL_SLIDING"],
    "Attention type: GLOBAL or LOCAL_SLIDING.",
)
_BLOCK_SIZE = flags.DEFINE_integer(
    "block_size", 512, "Flash / Splash attention block size."
)
_NUM_KV_HEADS = flags.DEFINE_integer(
    "num_kv_heads", 1, "Number of KV heads (1 for MQA, 2 for GQA)."
)
...
def profile_attention_variant(
  ...
  with jax.set_mesh(mesh):
    for i in range(warmup_iters):
      with jax.profiler.TraceAnnotation(f"{name}_warmup_{i}"):
        _, out, _ = forward_step(attn_module, x, positions, attn_mask)
  ...
  # Capture XProf trace
  tag = f"gemma4_{name}_seq{_SEQ_LEN.value}_{_ATTN_TYPE.value.lower()}"
  xprof_url = ""
  ...
  if xprof_session is not None:
    ...
    try:
      ...
      logging.info("[%s] Programmatic XProf session skipped (likely running under end-to-end profiler): %s", name, e)
  else:
...
def main(argv: Sequence[str]) -> None:
  ...
  attn_type = getattr(model_lib.AttentionType, _ATTN_TYPE.value)

  logging.info(
      "Configuration: batch_size=%d, seq_len=%d, dtype=%s, attn_type=%s,"
