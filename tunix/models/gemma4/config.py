...
class ModelConfig:
  ...
  def __post_init__(self):
    ...
    if self.use_split_attention and not self.use_flash_attention:
      raise ValueError(
          'use_split_attention=True requires use_flash_attention=True: the '
          'eager attention path does not consume the split prefix KV and would '
          'silently drop the prefix (numerically wrong).'
      )
