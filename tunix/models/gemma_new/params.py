...
from tunix.models.gemma_new import gemma as model_lib
...
def _make_preprocess_fn(cfg: model_lib.TransformerConfig):
   ...
   if cfg.head_dim % 2 != 0:
       raise ValueError(f"Gemma2 head_dim must be even for RoPE, got {cfg.head_dim}")
               ...
               if (q is not None) and (k is not None) and (v is not None):
                   ...
                   if not (q.shape == k.shape == v.shape == exp):
                       raise ValueError(
                           ...
                           f"q={getattr(q,'shape',None)}, k={getattr(k,'shape',None)}, v={getattr(v,'shape',None)}, "
                           f"expected={exp}"
                       )
