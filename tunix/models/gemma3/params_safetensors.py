...
def _make_preprocess_fn(cfg: model_lib.Gemma3Config):
  ...
  def to_NDH(q: jnp.ndarray) -> jnp.ndarray:
    # 目标 (N, D, H)
  ...
  def to_KDH(x: jnp.ndarray) -> jnp.ndarray:
      ...
      if fused_qkv:
        if (q is not None) and (k is not None) and (v is not None):
          ...
          if not (q.shape == k.shape == v.shape == exp):
            raise ValueError(
              f"[gemma3 preprocess] layer {layer_id}: fused q/k/v shape mismatch: "
              f"q={getattr(q, 'shape', None)}, k={getattr(k, 'shape', None)}, "
              f"v={getattr(v, 'shape', None)}, expected={exp}"
            )
          out[f"layers.{layer_id}.attn.qkv_einsum.w"] = jnp.stack([q, k, v], axis=0)  # (3,N,D,H)
        ...
        if (k is not None) and (v is not None):
          k = to_KDH(k)
          v = to_KDH(v)
          out[f"layers.{layer_id}.attn.kv_einsum.w"] = jnp.stack([k, v], axis=0)  # (2,K,D,H)
          slots.pop("k", None); slots.pop("v", None)
          wrote = True
