...
class SpanGroup:
  ...
  def find_all_inner_groups(self, name: str) -> list[SpanGroup]:
    return [item for item in self.inner if isinstance(item, SpanGroup) and item.name == name]
...
def span_group_tostring(group: SpanGroup, born: float = 0.0) -> str:
  ...
  def _tostring_recursive(
      out: str, group: SpanGroup, indent: int
  ) -> str:
    out += "  " * indent
...
def span_group_batch_query_nth(batch: list[SpanGroup], name: str, index: int) -> list[SpanGroup]:
  out_batch = []
  for group in batch:
...
def span_group_batch_query_all(batch: list[SpanGroup], name: str) -> list[SpanGroup]:
  out_batch = []
  for group in batch:
