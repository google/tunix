...
def generate_device_timeline_id(id: str | JaxDevice) -> str:
  ...
  if isinstance(id, str):
    ...
    return f"{getattr(id, 'platform')}{getattr(id, 'id')}"
  else:
    raise ValueError(f"Unsupported id type: {type(id)}")
...
class NoopTracer:
  ...
  ) -> Iterator[_AsyncWaitlist]:
    yield _AsyncWaitlist()


class PerfTracer(NoopTracer):
  """An API to collect events to construct thread and devices timelines."""
  ...
  def _get_or_create_host_timeline(self, timeline_id: str) -> Timeline:
    timeline = self._host_timelines.get(timeline_id)
    if timeline is None:
      timeline = Timeline(timeline_id, self._born)
      self._host_timelines[timeline_id] = timeline
    return timeline

  def _get_or_create_device_timeline(self, timeline_id: str) -> AsyncTimeline:
    timeline = self._device_timelines.get(timeline_id)
    if timeline is None:
      timeline = AsyncTimeline(timeline_id, self._born)
      self._device_timelines[timeline_id] = timeline
    return timeline
