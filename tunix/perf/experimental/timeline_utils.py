...
def add_idle_spans(
  ...
  # pylint: disable=protected-access
  with tl._lock:
    tl_last_span_id = tl._last_span_id

  if not new_tl.spans:
    new_tl._last_span_id = tl_last_span_id  # pylint: disable=protected-access
    return new_tl
...
def flatten_overlapping_spans(
  ...
  with tl._lock:
    spans = list(tl.spans.values())
    tl_last_span_id = tl._last_span_id
    tl_spans_keys = list(tl.spans.keys())
  ...
  exec_tl._last_span_id = tl_last_span_id
  queue_tl._last_span_id = tl_last_span_id
  next_queue_id = (max(tl_spans_keys) + 1) if tl_spans_keys else 0
