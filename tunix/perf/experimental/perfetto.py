...
def _create_span_name(name: str, tags: dict[str, Any]) -> str:
  """Creates a descriptive name for the span based on its tags."""
  parts = []
  if "global_step" in tags:
    parts.append(f"step={tags['global_step']}")

  if name == "peft_train_step" and "role" in tags:
    parts.append(f"role={tags['role']}")

  if name == "rollout":
    if "group_id" in tags:
      parts.append(f"group_id={tags['group_id']}")
    if "pair_index" in tags:
      parts.append(f"pair_index={tags['pair_index']}")
  ...
  def write_timelines(self, timelines: dict[str, Timeline]) -> None:
    ...
    for i, t_id in enumerate(sorted_ids):
      ...
      for s in sorted_spans:
        ...
        packet_begin.track_event.name = _create_span_name(s.name, s.tags)
