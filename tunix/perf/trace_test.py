...
from tunix.perf.trace import PerfTracer, NoopTracer, BaseTimeline, HostTimeline, DeviceTimeline
...
def make_host_timeline(born: float,
                       intervals: list[tuple[float, float]] | None = None,
                       labels: list[str] | None = None,
                       epochs: list[int] | None = None) -> HostTimeline:
...
def make_device_timeline(id: str,
                         born: float,
                         intervals: list[tuple[float, float]] | None = None,
                         labels: list[str] | None = None,
                         epochs: list[int] | None = None) -> DeviceTimeline:
...
class TracerTest(parameterized.TestCase):
  ...
  def test_host_ok(self, mock_perf_counter):
    ...
    self.assertListEqual(get_timelines(tracer),
                         [make_host_timeline(0.0,
                                             [(2.0, 3.0)],
                                             ["x"],
                                             [1])])
  ...
  def test_device_ok(self, mock_perf_counter):
    ...
    with tracer.interval("x", devices=["tpu0"]) as interval:
      interval.device_end(mock)
    tracer.end_epoch()

    tracer.synchronize()

    mock.block_until_ready.assert_called_once()
    self.assertListEqual(get_timelines(tracer),
                         [make_host_timeline(0.0,
                                             [(2.0, 3.0)],
                                             ["x"],
                                             [1]),
                          make_device_timeline("tpu0", 0.0,
                                                [(2.0, 5.0)],
                                                ["x"],
                                                [1])])
  ...
  def test_device_interval_begin_algorithm(self, mock_perf_counter):
    ...
    self.assertEqual(mock.block_until_ready.call_count, 2)
    # Note "tpu0:step2:begin" is max("host:step2:begin" 4.0, "tpu0:step1:end" 5.0)
    self.assertListEqual(get_timelines(tracer),
                         [make_host_timeline(0.0,
                                             [(2.0, 3.0),(4.0, 6.0)],
                                             ["step1", "step2"],
  ...
  def test_noop_interface_is_same(self):
    noop_public_attrs = [name for name in dir(NoopTracer()) if not name.startswith("_")]
    perf_public_attrs = [name for name in dir(PerfTracer()) if not name.startswith("_")]
