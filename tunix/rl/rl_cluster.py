...
class MetricsBuffer:
  ...
  # TODO(tsbao): support string metrics.
  metrics: dict[
      str, tuple[list[ArrayLike], Callable[[ArrayLike], ArrayLike] | None]
...
class RLCluster:
  ...
  def __init__(
    ...
    # algorithm. E.g. if there are additional inner loops that update the model,
    # we should properly update the global steps.
    self.global_steps = 0
  ...
  def _log_metrics(self, metrics_buffer: MetricsBuffer) -> None:
    ...
    print("metrics_buffer: ", metrics_buffer)
  ...
  def buffer_metrics(
      self,
      metrics: dict[
          str, tuple[ArrayLike, Callable[[jax.Array], jax.Array] | None]
      ],
