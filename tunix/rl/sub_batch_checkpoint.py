...
class SubBatchCheckpointManager(sft_checkpoint_manager.BaseCheckpointManager):
  ...
  def try_restore(
    ...
    Args:
      ...
      num_generations: The run's `num_generations`. Declines the restore if it
        does not match the value the snapshot was saved with.
      num_iterations: The run's `num_iterations`. Declines the restore if it
        does not match the value the snapshot was saved with.
      mini_batch_size: The run's `mini_batch_size`. Declines the restore if it
        does not match the value the snapshot was saved with.
