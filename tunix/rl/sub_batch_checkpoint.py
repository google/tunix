...
# Written into the ledger root the first time the feature runs on a
# checkpoint root, so an empty ledger under a trainer that already has
# checkpoints is either a first enable (no marker) or a lost ledger (marker).
ENABLED_MARKER = ".sub_batch_enabled"
...
class SubBatchGeometryError(ValueError):
  ...
  change geometry deliberately, move or delete `<root>/sub_batch` (the
  in-progress step is then abandoned; `<root>/actor` stays usable).
  """
...
class SubBatchCheckpointManager(sft_checkpoint_manager.BaseCheckpointManager):
  ...
  def mark_enabled(self) -> None:
    """Writes the enable marker into the ledger root (see ENABLED_MARKER)."""
    if self._root_directory:
      path = epath.Path(self._root_directory) / ENABLED_MARKER
      path.parent.mkdir(parents=True, exist_ok=True)
      path.write_text("")

  def _marked_enabled(self) -> bool:
    return bool(self._root_directory) and (
        epath.Path(self._root_directory) / ENABLED_MARKER
    ).exists()
  ...
  def peek_latest_meta(self, train_steps: int) -> dict[str, Any] | None:
    """Returns the meta-data of the latest snapshot for the given `train_steps`."""
    if self._checkpointer is None:
      return None
    chosen = self._select_step(train_steps)
    if chosen is None:
      return None
    try:
      return dict(
          self._checkpointer.load_checkpointables(
              chosen, abstract_checkpointables={"meta": None}
          )["meta"]
      )
  ...
  def purge_steps_above(self, bound: int) -> None:
    ...
    for step in stale:
      ...
      step_dir = epath.Path(self._checkpointer.directory) / str(step)
      try:
        if step_dir.exists():
          step_dir.rmtree()
      except OSError as e:
  ...
  def _describe_missing(self, train_steps: int, leftover: list[int]) -> str:
    ...
    return (
        ...
        " was purged. Inspect the directory, or move or delete"
        f" {self._root_directory} to give up the in-progress step"
        " deliberately (the actor weights under the checkpoint root stay"
        " usable)."
    )
  ...
  def _invalid_msg(self, key: int, what: str) -> str:
    ...
    return (
        ...
        f" it, or move or delete {self._root_directory} to give up the"
        " in-progress step deliberately (the actor weights under the"
        " checkpoint root stay usable)."
    )
  ...
  def _unreadable_msg(self, key: int, what: str, exc: BaseException) -> str:
    ...
    if local == 0 or fallback is None:
      return msg + (
          ...
          f" changed, otherwise move or delete {self._root_directory} to"
          " give up that step deliberately (the actor weights under the"
          " checkpoint root stay usable)."
      )
    return msg + (
  ...
  def try_restore(
    ...
    Returns:
      ...
      lineage and purged) or an entirely empty ledger root (train_steps > 0
      there means the feature was enabled on an existing run: WARNING, the
      in-progress step is not resumed).
    ...
    Raises:
      ...
      and on a fresh anchor. An entirely empty root under a trainer at
      train_steps > 0 is a first enable (the root gets ENABLED_MARKER) unless
      the marker is already there, which means the ledger was lost:
      SubBatchLedgerMissingError.
    """
    if self._checkpointer is None:
      return None
    ...
    if chosen_step is None:
      ...
      if not leftover:
        if train_steps > 0:
          if self._marked_enabled():
            raise SubBatchLedgerMissingError(
                f"Sub-batch restore: {self._root_directory} carries the"
                f" enable marker but no snapshot, while the trainer is at"
                f" train_steps={train_steps}: the ledger was lost (deleted"
                " or moved) after the feature ran on this checkpoint root."
                " Nothing was purged. Restore the ledger, or delete the"
                f" marker {ENABLED_MARKER} to start the next step fresh"
                " from the restored weights deliberately."
            )
          logging.warning(
              "Sub-batch restore: no ledger under %s for trainer"
              " train_steps=%d, so sub-batch checkpointing was enabled on an"
              " existing run. This restart keeps stock restart semantics:"
              " the in-progress step is NOT resumed, the next step starts"
              " fresh from the restored weights.",
        ...
        return None
      if train_steps == 0:
        # Anchor restored, crash before window 0 got a snapshot (k == 1:
        # the first snapshot of a run is the 1.0 precommit): step 0 reruns
        # from scratch, keys above are dead lineage. A parent >= 2 could
        # only survive here if retention dropped window 0 under the
        # durable anchor.
        if max(leftover) // KEY_BASE >= 2:
          logging.warning(
              "Sub-batch restore: trainer at train_steps=0 but the ledger"
              " under %s holds parents up to %d and none for window 0:"
              " retention outran the trainer's durable checkpoint. Step 0"
              " reruns from the anchor; the leftovers are purged.",
              self._root_directory,
              max(leftover) // KEY_BASE,
          )
        self.purge_steps_above(0)
        return None
      raise SubBatchLedgerMissingError(
    ...
    def _require_same(
      ...
      raise SubBatchGeometryError(
          ...
          " configuration; relaunch with the original value, or move or"
          f" delete {self._root_directory} to change geometry deliberately"
          " (the in-progress step is then abandoned; the actor weights"
          " under the checkpoint root stay usable)."
      )
