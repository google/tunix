...
class SubBatchLedgerMissingError(SubBatchRestoreError):
  """Raised when trainer checkpoint T > 0 has no ledger with parent T.

  Occurs while other ledgers exist (retention/lineage bug or a foreign ledger
  root), or when the trainer store is lost on an already-enrolled run.
  """


class SubBatchUnreadableError(SubBatchRestoreError):
  """Raised when the selected snapshot did not load or did not validate.

  Covers torn directories, I/O failures, incompatible trees, or payloads/meta
  that do not decode (the failure kind is described in the message).
  """


class SubBatchPurgeError(SubBatchRestoreError):
  """Raised when a dead-lineage snapshot could not be deleted.

  Recovery stops because, left on disk, its key would later be selected over
  the rerun's own snapshots.
  """
...
class SubBatchCheckpointManager(sft_checkpoint_manager.BaseCheckpointManager):
  ...
  def mark_enabled(self, train_steps: int = 0) -> None:
    ...
    Reusing a record is only for retrying enrollment before any ledger was
    written. An already-running workload resumes through try_restore().

    Args:
      train_steps: The baseline trainer apply count at enrollment.

    Raises:
      SubBatchLedgerMissingError: If already enrolled at a different baseline,
        or if surviving ledger snapshots exist while the trainer store is
        missing.
    """
    ...
    if path.exists():
      ...
      if self._checkpointer is not None and self._checkpointer.checkpoints:
        raise SubBatchLedgerMissingError(
            f"Sub-batch enrollment at {path} (baseline {train_steps}) has"
            f" {len(self._checkpointer.checkpoints)} surviving ledger"
            " snapshot(s), but the trainer checkpoint store is missing;"
            " refusing to purge recovery evidence or overwrite W(0). Restore"
            " the matching trainer checkpoints before continuing."
        )
      return
  ...
  def set_durable_train_steps(self, train_steps: int) -> None:
    """Records the trainer's last durable apply count for retention.

    Retention keeps the newest key of every parent >= `train_steps`. Call only
    after the trainer checkpoint for `train_steps` is known durable (the
    learner does so after `checkpoint_manager.wait()`) and before the save that
    should observe it. Never lowers the floor. A caller-supplied preservation
    policy owns retention: no-op.

    Args:
      train_steps: Confirmed durable apply count from the trainer.
    """
  ...
  def save(
    ...
    Args:
      train_steps: The trainer's apply count of the weights the accumulator sits
        on.
      local: Within-window micro-step counter (resets at each apply).
      iter_steps: The trainer's global micro-step counter.
      global_step: The global RL step index.
      grad_accum_steps: Configured gradient accumulation steps.
      step_complete: Whether this snapshot is the final apply of the global
        step.
      completed_group_ids: Prompt IDs of groups fully trained across all epochs.
      trained_trajectory_counts: Mapping from (prompt_id, group_index) to
        trained epoch count.
      active_group_trajectories: Rollout payloads retained for this step.
      training_state: Live gradient accumulator state, or None at apply
        boundaries.
      num_generations: Number of generations per prompt.
      full_batch_size: Full batch size per global step.
      num_iterations: Number of training epochs per step.
      mini_batch_size: Mini-batch size per optimizer apply.
      geometry: Additional configuration/geometry dictionary.
      anchor_step: Trainer step corresponding to the start of the global step.
    """
    ...
    if step_complete and training_state is not None:
      raise ValueError(
          "A completed global step must not carry a gradient accumulator."
      )
  ...
  def purge_steps_above(self, bound: int) -> None:
    ...
    Args:
      bound: Maximum encoded key to retain; any key strictly greater is purged.
  ...
  def _describe_missing(self, train_steps: int, leftover: list[int]) -> str:
    """Builds diagnostic text for SubBatchLedgerMissingError.

    Describes why window `train_steps` is empty while `leftover` keys survive.
    Decodes the latest survivor and reads its meta (or reports it unreadable);
    nothing is purged.

    Args:
      train_steps: Restored trainer apply count.
      leftover: Surviving encoded snapshot keys on disk.

    Returns:
      Formatted diagnostic error string.
    """
  ...
  def _invalid_msg(self, key: int, what: str) -> str:
    """Builds message for SubBatchUnreadableError when validation fails.

    Used when the snapshot loaded but its meta/payload does not validate:
    nothing purged, operator decides.

    Args:
      key: Encoded sub-batch snapshot key.
      what: Description of the validation failure.
  ...
  def _unreadable_msg(self, key: int, what: str, exc: BaseException) -> str:
    """Builds message for SubBatchUnreadableError when loading fails.

    Reports which key/directory failed to load `what`, the kind of failure read
    off the exception chain, and the operator's options. Kinds (orbax 0.12.1):
    a missing checkpointable is a bare KeyError (torn directory); I/O failures
    and structural drift both surface as NoEntryError (a KeyError subclass)
    with the real cause chained, so the bare type is tested first, then OSError
    in the chain. The label is advisory; the class and the no-purge contract do
    not depend on it.

    Args:
      key: Encoded sub-batch snapshot key.
      what: Name of the checkpointable that failed to load.
  ...
  def try_restore(
    ...
    for field in ("has_training_state", "step_complete", "anchor_step"):
      if meta.get(field) is None:
        raise SubBatchUnreadableError(
            self._invalid_msg(chosen_step, f"meta lacks {field}")
        )
    has_training_state = bool(meta["has_training_state"])
    if bool(meta["step_complete"]) and has_training_state:
      raise SubBatchUnreadableError(
          self._invalid_msg(
              chosen_step,
              "a completed global step carries a gradient accumulator",
          )
      )
