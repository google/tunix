...
class SubBatchGeometryError(ValueError):
  ...
  window, num_generations, num_iterations). A mismatch raises
  instead of silently adapting. To change geometry deliberately, start a run
  with a fresh checkpoint root.
  """


class SubBatchCheckpointManager(sft_checkpoint_manager.BaseCheckpointManager):
  # BENCHMARK instrumentation (temporary): per-call wall timings of the
  # synchronous (training-thread) portions of save()/wait(), for the
  # overhead decomposition. Read-only for callers; None until the first
  # call. Removed with the rest of the bench scaffolding.
  last_save_timings: dict[str, Any] | None = None
  ...
  def save(
    ...
    # BENCHMARK instrumentation (temporary): counting extant snapshots
    # around the save exposes when the preservation policy deletes a parent
    # step on disk (deletions happen during preservation checks inside save).
    _n_snaps_before = (
        len(self._checkpointer.checkpoints)
        if self._checkpointer is not None
        else 0
    )
    _n_subdirs_before = (
        len(os.listdir(self._root_directory))
        if self._root_directory and os.path.exists(self._root_directory)
        else 0
    )
    checkpointables = {
        "meta": {
    ...
    self.last_save_timings = {
        "wait_s": round(_t_wait - _t0, 4),
        "build_s": round(_t_build - _t_wait, 4),
        "save_call_s": round(_t_save - _t_build, 4),
        "total_sync_s": round(_t_save - _t0, 4),
        "snaps_before": _n_snaps_before,
        "snaps_after": _n_snaps_after,
        "subdirs_before": _n_subdirs_before,
        "subdirs_after": _n_subdirs_after,
    }

  def wait(self) -> None:
    """Blocks until any in-flight async save finishes.

    Records `last_wait_s` for the benchmark timing decomposition.
    """
    if self._checkpointer is not None:
      _t0 = time.perf_counter()
      self._checkpointer.wait()
      self.last_wait_s = round(time.perf_counter() - _t0, 4)
  ...
  def try_restore(
    ...
    Raises:
      SubBatchGeometryError: When the snapshot geometry (e.g.,
        grad_accum_steps, num_generations) does not match the current run.
    """
    if self._checkpointer is None:
      return None
    ...
    # SubBatchGeometryError).
    def _require_same(field: str, saved, current) -> None:
      ...
      raise SubBatchGeometryError(
          ...
          f" {field}={saved} but this run is configured with {current}."
          " Sub-batch resume requires relaunching with the SAME"
          " configuration; relaunch with the original value, or start a"
          " fresh checkpoint root to change geometry deliberately."
      )
    ...
    if isinstance(custom, dict):
      sub_step_val = custom.get("sub_step_elapsed", 0.0)
      if isinstance(sub_step_val, (int, float, str)):
        sub_step_elapsed = float(sub_step_val or 0.0)
      exp_start_val = custom.get("experiment_start_time", 0.0)
      if isinstance(exp_start_val, (int, float, str)):
        experiment_start_time = float(exp_start_val or 0.0)
