...
def resolve_sub_batch_checkpointing_defaults(
  ...
  Args:
    ...
    run_options: The run's (trainer-stream) checkpointing options, read
      protocol-tolerantly (attributes may be absent); only the
      stream-neutral fields (async toggle, async options) above are consulted.
  ...
  return checkpoint_options.TunixCheckpointingOptions(
      save_decision_policy=(
          save_policy
          or ocp.training.save_decision_policies.FixedIntervalPolicy(
              interval=1
          )
      ),
...
class SubBatchState:
  ...
  Attributes:
    ...
    sub_batch_key: The encoded key this snapshot was saved at
      (`train_steps * KEY_BASE + local`).
    ...
    grad_accum_steps: `gradient_accumulation_steps` the run was configured
      with. Restore declines on mismatch.
    step_complete: True when this snapshot was taken at the final apply of its
      global step. On resume the learner then starts fresh at
      `global_step + 1` instead of resuming mid-step.
    completed_group_ids: Prompt groups whose rollouts are fully generated
      (skip regeneration on resume).
    ...
    training_state: The live gradient-accumulation state, opaque to the
      manager. Both training modes persist the trainer's
      `GradientAccumulator` contents (`{"grads": <param tree>, "denom":
      scalar}`); None when the snapshot was taken at an apply boundary (the
      accumulator was just reset -- `denom == 0` -- and a restored trainer
      starts its window with a fresh accumulator anyway, so there is nothing
      to inject).
  """
  sub_batch_key: int
  iter_steps: int
...
class SubBatchCheckpointManager(sft_checkpoint_manager.BaseCheckpointManager):
  ...
  def __init__(
    ...
    Args:
      ...
      options: Checkpointing options; fields left None are filled with
        sub-batch defaults (`resolve_sub_batch_checkpointing_defaults`) --
        NOT the trainer's defaults, whose save policy would silently skip
        snapshots.
