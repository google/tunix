...
from tunix.rl.experimental.agentic.agents.base_agent import LLMBaseAgent, Trajectory
...
class TrajectoryCollectEngine:
        ...
        *,
        ...
        Args:
            ...
            final_reward_fn (Optional[Callable]): Optional function to compute
                additional reward at episode end. Takes (task, response) and returns float.
                Defaults to zero if not provided.
            max_steps (int): Maximum number of interaction steps before forced termination
            gamma (float): Discount factor for return calculation (1.0 = no discounting)
            timeout (float): Maximum episode duration in seconds before timeout termination
        """
        self.agent = agent
        self.env = env
    ...
    async def collect_multiple(
        ...
        *,
        ...
        Args:
            pairs (List[Tuple[LLMBaseAgent, BaseEnv]]): List of (agent, environment) pairs
        ...
        Yields:
            Tuple[int, Trajectory]: (pair_index, completed_trajectory) as episodes finish
        """
    ...
    async def _one_step(self) -> bool:
        ...
        Returns:
            bool: True if episode should terminate (done or timeout), False to continue
        """
        ...
        if action is None:
            logger.warning("Agent returned None action, using empty action list as fallback")
            action = []
