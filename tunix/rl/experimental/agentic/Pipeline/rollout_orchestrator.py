...
concurrent collection of trajectories from multiple agent-environment pairs
and groups them into batches for further processing.
"""
...
from tunix.rl.experimental.agentic.trajectory.trajectory_collect_engine import BaseEnv
from tunix.rl.experimental.agentic.trajectory.trajectory_collect_engine import LLMBaseAgent
from tunix.rl.experimental.agentic.trajectory.trajectory_collect_engine import Trajectory
...
class RolloutOrchestrator:
  ...
  async def _runner(
    ...
    and puts them into the `GroupQueueManager`. It handles potential
    exceptions during trajectory collection and respects the `_stop` event
    and `episodes_per_pair` limit.
  ...
  async def run_and_yield_batches(
    ...
    a `GroupQueueManager` to group collected trajectories and yields batches
    of trajectories as they become available. The orchestrator continues
    running until all `episodes_per_pair` are collected for all pairs or
    the `_stop` event is set.
