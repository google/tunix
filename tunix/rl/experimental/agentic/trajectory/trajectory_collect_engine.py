...
class TrajectoryCollectEngine:
  ...
  async def collect(self, mode: str = "Conversation") -> Any:
    ...
    Args:
        mode (str): Output format. Options: - "Text": return full Trajectory
          object (default). - "Token": return flattened tokenized dict for
          training. - "Step": return stepwise tokenized data only. -
    ...
    await self.compute_mc_returns()
    await self.compute_trajectory_reward()
    await self._close()

    if mode not in ["Text", "Step", "Token", "Conversation"]:
      raise ValueError(
          f"Unsupported mode: {mode}, currently supported modes: "
          f" {['Text', 'Step', 'Token', 'Conversation']}",
      )
  ...
  async def _reset(self):
    ...
    if self.tokenizer is not None and self.chat_parser is not None:
      ...
      prompt_tokens, _ = utils.convert_messages_to_tokens_and_masks(
          init_messages,
  ...
  async def _one_step(self) -> bool:
    ...
    if self.tokenizer is not None and self.chat_parser is not None:
      ...
      if cur_step is not None:
        ...
        # env tokens
        if env_messages:
          env_tokens, env_masks = utils.convert_messages_to_tokens_and_masks(
              env_messages,
              tokenizer=self.tokenizer,
              parser=self.chat_parser,
              contains_first_msg=False,
  ...
  async def compute_trajectory_reward(self):
  ...
  async def compute_mc_returns(self):
