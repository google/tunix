...
class ToolEnvironment(BaseEnv):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            task (Dict | None): Task specification containing problem description,
                ground truth, or other task-specific parameters. If None, defaults to empty dict.
            tool_map (Dict[str, type[BaseTool]]): Mapping of tool names to their
                implementation classes for tool discovery and execution.
            reward_fn: Reward function that takes (task, action) and returns RewardOutput.
                If None, defaults to zero_reward with a warning.
    ...
    def reset(self) -> tuple[Dict, Dict]:
        ...
        Returns:
            tuple[Dict, Dict]: Initial observation (task) and empty info dict
        """
        self.step_count = 0
        return self.task, {}
    ...
    def from_dict(env_args: Dict) -> "ToolEnvironment":
        ...
        Args:
            env_args (Dict): Configuration dictionary containing:
                - tool_map: Tool implementations mapping
                - reward_fn: Reward function (optional)
                - max_steps: Episode length limit (optional, defaults to 10)
                - Other entries become the task specification
