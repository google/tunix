...
class BaseEnv(ABC):
    ...
    def reset(self) -> tuple[dict, dict]:
        ...
        Returns:
            tuple[dict, dict]: A tuple containing:
                - initial_observation (dict): The starting state/observation for the agent
                - info_dict (dict): Additional metadata about the reset (debug info, metrics, etc.)
        """
        pass
    ...
    def step(self, action: Any) -> tuple[Any, float, bool, dict]:
        ...
        Returns:
            tuple[Any, float, bool, dict]: A tuple containing:
                - next_observation (Any): The resulting state/observation after the action
                - reward (float): Numerical reward signal for the action taken
                - done (bool): Whether the episode has terminated (success, failure, or limit reached)
                - info (dict): Additional step information (metrics, debug data, intermediate results)
        """
        pass
    ...
    async def step_async(self, action: Any) -> tuple[Any, float, bool, dict]:
        ...
        that perform I/O operations or when running multiple environments concurrently.
