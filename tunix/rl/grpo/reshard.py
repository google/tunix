...
def reshard_pytree(
  ...
  dst_sharding = jax.tree_util.tree_map(
  ...
  # The experimental reshard API can lead to OOM when the input and output
  # sharding are not one to one mapping. It will trigger an all gather.
  # We actively look into alternative under b/424892771.
  reshardfn = None

  # BEGIN GOOGLE-INTERNAL
  try:
    from tunix.google.rl.grpo import reshard  # pylint: disable=g-import-not-at-top # pytype: disable=import-error
    reshardfn = reshard.experimental_reshard
  except ImportError:
    logging.error(
        'Cannot import Pathways client. Make sure you have'
        ' linked //third_party/py/tunix/google/rl/grpo:reshard to your binary.'
    )
  # END GOOGLE-INTERNAL
  ...
  if reshardfn is None:
    try:
      ...
      reshardfn = experimental_reshard.reshard
  ...
  if reshardfn is None:
    ...
    resharded_array = jax.device_put(source, dst_sharding)
  else:
