...
def pack_sequences(
  ...
  Args:
    ...
    sequences_per_update: Sequences per mini-batch/update
      (= mini_batch_size * num_generations). Required: the update boundary is a
      mini-batch property, and the producer's list granularity does not carry
      it (a colocated producer enqueues a whole mini-batch, a streaming one a
      micro-batch), so it cannot be inferred from the input stream.
