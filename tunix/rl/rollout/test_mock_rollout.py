...
class MockRolloutTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      {"testcase_name": "fixed", "dist": "fixed", "mean": 8.0, "std": None},
  )
  @mock.patch.object(mock_rollout.time, "sleep", autospec=True)
  def test_generate_distributions(self, mock_sleep, dist, mean, std):
    m = self._create_mock_rollout(
    ...
    self.assertTrue(
        all(
            1 <= length <= self.base_rc.max_tokens_to_generate
            for length in lengths
        )
    )

    if dist == "fixed" and mean is not None:
      self.assertTrue(all(length == int(mean) for length in lengths))
