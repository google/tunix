...
class MLDiagScalarBackend:
  ...
  def log_scalar(
    ...
    try:
      mldiag_metrics.record(event, val, step=int_step)
    except Exception as e:  # pylint: disable=broad-exception-caught
      logging.warning(
          "Failed to record metric '%s' to ML Diagnostics (%s). Disabling "
          "MLDiagScalarBackend for the remainder of this run.",
          event,
          e,
      )
      self._disabled = True
