...
## 4. What would separate the arms

1. **Introduce real off-policy lag** with `off_policy_steps > 0` or
   `num_iterations > 1`. That is the regime SC is designed for. With
   `num_iterations > 1` the PPO ratio becomes meaningful again in both arms
   (see `score_centering_validation.md` §3.6).
2. **Run longer and replicate.** Use at least 100 steps with 2 to 3 seeds per
   arm. The paired per-step reward differences here have a standard deviation
   of about 0.05, so 14 paired steps give a standard error of about 0.013.
   Differences in mean reward below about 0.03 cannot be resolved.
3. **Give the Qwen recipe the same env budget as Gemma.** Qwen uses
   `env_kwargs={"max_steps": 8}` while Gemma uses 15. Many 12x12 grids cannot
   be solved in 8 moves, which caps reward near 0.2 and thins the advantage
   signal.
4. **Separate the clip-noise effect from SC.** An SC-off run with a wider
   gspo-token band, or with `loss_algo="grpo"`, would show whether the 4%
   clip rate costs anything by itself.

---

## 5. Issues encountered

### 5.1 vLLM rollout with DP=1 fails at engine init (blocking)

```
AssertionError: `with_sharding_constraint` acts as an assert when all axes of
mesh are of type `Explicit`. The array sharding: P('data', 'model') did not
match the sharding provided: P('data', None).
```

`VllmSampler` always passes `device_indexes` to tpu-inference. With DP=1,
`tpu_runner._create_2d_mesh` then calls `jax.make_mesh` without `axis_types`,
and JAX 0.10 defaults those to `Explicit`. The sampler's
`with_sharding_constraint` on the vocab-sharded logits
(`tpu_inference/layers/jax/sample/sampling.py:122`) becomes a hard assertion.
DP>1 goes through the per-rank `dp_scheduler` path and is unaffected, which is
why the Gemma recipe (rollout mesh `(2, 4)`) never hit this.

**Workaround:** the Qwen recipe now uses a separate `(2, n/2)` rollout mesh.
**Fix:** tpu-inference should pass `axis_types=Auto` in the
`enforce_device_order` branch, or `reshard` before the constraint.

### 5.2 The Qwen recipe did not import in OSS

`train_frozenlake_qwen3.py` imported `tunix.google.stubs.utils_stub`, which
does not exist in this repo. It now downloads the model with
`huggingface_hub.snapshot_download`, the same way the Gemma recipe does.

### 5.3 The per-step log line reports the last micro-batch, not the step

The `[step N] ... adv_abs_mean=... clipfrac=...` line in
`agentic_rl_learner.py` (around line 1088) reads `merged_train_micro_batch`,
which is the last micro-batch of the step and not the full batch.
`adv_abs_mean=0.000` next to a nonzero `grad_norm` looks contradictory but is
not. Use TensorBoard `actor/train/*`, or `grad_norm`, which is accumulated
over the step, to judge whether a step was empty.

### 5.4 Stopping a run leaks vLLM workers and drops the last TensorBoard point

Sending SIGTERM to the trainer PID leaves the vLLM DP workers orphaned
(re-parented to PID 1) and still holding `/dev/vfio/*`. The next run then fails
with `open(/dev/vfio/0): Device or resource busy`. Launch the trainer with
`setsid` and kill the whole process group.

Separately, `flush_every_n_steps=1` did not flush the final step's actor
metrics within 30 s, so both arms are missing their last point in
TensorBoard.

### 5.5 Carried over from the previous report

`exact_token_continuity=False` is still required (§4.1 of
`score_centering_validation.md`). The shutdown hang (§4.2) did not recur on the
Gemma run in this report, which exited cleanly.
