...
class TrajectoryCollectEngine:
  ...
  def __init__(
    ...
    self.max_steps = self.env.max_steps  # respect env max_steps
    self.gamma = gamma
    ...
    if self.max_context_limit and not self.tokenizer:
      logging.warning(
          "max_context_limit is set to %d, but no tokenizer was provided. "
  ...
  async def collect(self, mode: str = "Conversation") -> Any:
    ...
    while True:
      ...
      if current_step and self.tokenizer:
        a_tokens = getattr(current_step, "assistant_tokens", []) or []
        e_tokens = getattr(current_step, "env_tokens", []) or []
    ...
    if mode == "Trajectory":
      ...
      return {
          ...
          "loss_mask": loss_mask,  # TDOO:b/484422277
