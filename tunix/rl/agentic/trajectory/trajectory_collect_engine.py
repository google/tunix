...
class TrajectoryCollectEngine:
  ...
  def _adopt_or_check_prompt(self, rollout_output, *, submitted) -> None:
    """Owns the first-turn prompt ids; later turns must echo the recorded ones.

    Args:
      rollout_output: the single-row result of this turn's model call.
      submitted: the ids sent for this turn, or None on the first turn.

    Raises:
      ValueError: the engine's prompt does not match the recorded history.
    """
    actual = generate_utils.unpad_prompt(
        rollout_output.left_padded_prompt_tokens[0],
        rollout_output.prompt_lengths[0],
    )
    if submitted is None:
      self.agent.trajectory.prompt_tokens = actual
      self.agent.trajectory.prompt_length = len(actual)
    elif not np.array_equal(actual, submitted):
  ...
  async def _one_step(self) -> bool:
    ...
    if self.exact_token_continuity:
      self._adopt_or_check_prompt(
          rollout_output, submitted=call_kwargs.get("prompt_token_ids")
      )
    ...
    if (
        ...
        and not self.agent.trajectory.steps
