  ...
  async def _one_step(self) -> bool:
    ...
    if cur_step is not None and self.tokenizer and self.chat_parser:
      ...
      if assistant_message:
        cur_step.assistant_tokens, n_append = self.chat_parser.update_assistant_end_tokens(rollout_output.tokens[0], assistant_message["content"])
        cur_step.assistant_masks = np.concatenate([np.ones(len(cur_step.logprobs), dtype=np.int32), np.zeros(n_append, dtype=np.int32)], axis=0)
        cur_step.logprobs = np.concatenate([cur_step.logprobs,np.zeros(n_append, dtype=np.float32)], axis=0)

        logging.log_first_n(logging.INFO, f"{assistant_message=}", 4)
