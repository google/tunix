...
class TrajectoryCollectEngine:
    ...
    if mode == "Trajectory":
      ...
      for idx, step in enumerate(self.agent.trajectory.steps):
        ...
        if assistant_tokens is not None:
          ...
          if has_routed_experts:
            ...
            routed_experts.append(np.asarray(step_routed, dtype=np.int32))
        if env_tokens is not None:
          ...
          if has_routed_experts:
            ...
            routed_experts.append(np.asarray(step_env_routed, dtype=np.int32))
      ...
      if has_routed_experts:
        ...
        conv_routed = (
            ...
            if routed_experts
            else np.zeros((0,) + sample_shape, dtype=np.int32)
        )
        ...
        if prompt_len > 0:
          ...
          prompt_routed_arr = np.asarray(prompt_routed, dtype=np.int32)
        else:
          prompt_routed_arr = np.zeros((0,) + sample_shape, dtype=np.int32)
  ...
  async def _one_step(self) -> bool:
    ...
    if (
      ...
      init_routed = np.asarray(rollout_output.routed_experts[0], dtype=np.int32)
      prompt_len = (
      ...
      delta_routed = np.asarray(rollout_output.routed_experts[0], dtype=np.int32)
