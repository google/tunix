...
class Step:
  ...
  Attributes:
    ...
    assistant_routed_experts: Per-token routed expert IDs for assistant tokens
      [length, num_layers, top_k], or None.
    env_routed_experts: Per-token routed expert IDs for environment observation
      tokens [length, num_layers, top_k], or None.
  """
  ...
  assistant_routed_experts: Optional[np.ndarray] = None
  env_routed_experts: Optional[np.ndarray] = None


UNSET_ROUTED_EXPERT: int = -1
...
def is_valid_trajectory_dict(traj: Any) -> bool:
  """Returns True when a trajectory dict has a valid status and trainable tokens."""
  if not isinstance(traj, dict):
    return False
  status = traj.get("status")
  if isinstance(status, str):
    status = TrajectoryStatus.__members__.get(status.upper(), status)
  if status in INVALID_TRAJECTORY_STATUSES:
    return False
  # Overlong trajectories (`MAX_STEPS_REACHED`, `MAX_CONTEXT_LIMIT_REACHED`)
  # keep their status when `overlong_filter=True`, and are masked out by
  # `TrajectoryCollectEngine` zeroing `conversation_masks`.
  masks = traj.get("conversation_masks")
  if masks is None:
    return True
  return bool(np.sum(masks) > 0)
...
class TrajectoryItem:
  ...
  This class acts as a flexible container representing an individual rollout
  trajectory.
  Attribute lookups for fields not defined explicitly on the dataclass (such as
  `status`, `reward`, `prompt_tokens`, `conversation_tokens`,
  `conversation_masks`,
  `routed_experts`, or `policy_version`) fall back dynamically via `__getattr__`
  to
  ...
  Attributes:
    ...
    traj: The trajectory payload. Typically a dictionary in tokenized RL
      training workflows (e.g. from `TrajectoryCollectEngine(mode="Token")`
      containing `"prompt_tokens"`, `"conversation_tokens"`,
      `"conversation_masks"`, etc., or an error dictionary), or a `Trajectory`
      dataclass instance when collecting step-by-step agent interactions
      (`mode="Trajectory"`) for evaluation or inspection.
    metadata: Arbitrary metadata dictionary for tracking, lineage, or extra
      parameters.
    is_valid: Whether the trajectory completed with a valid status and trainable
      tokens.
  """
  ...
  _is_valid: bool | None = dataclasses.field(default=None, repr=False)
  ...
  def __init__(
    ...
    for k, v in kwargs.items():
      if v is not None:
        self.metadata[k] = v
    self._is_valid = None if is_valid is None else bool(is_valid)

  @property
  def is_valid(self) -> bool:
    """Whether the trajectory completed with a valid status and trainable tokens."""
    if self._is_valid is not None:
      return self._is_valid
    return is_valid_trajectory_dict(self.traj)

  @is_valid.setter
  def is_valid(self, value: bool | None) -> None:
    self._is_valid = None if value is None else bool(value)
