...
class AgenticRLConfig(algo_config_lib.AlgorithmConfig):
  ...
  Parameters:
    ...
    sub_batch_checkpointing: Whether to checkpoint the sample rollout components
      ...
      contract across a mid-step resume: the step's behavior policy is the
      trainer checkpoint at the
      ...
      sampler_is='token' are rejected there. Recovery is validated for a
      single controller; multi-process recovery is not validated.
...
class AgenticRLLearner(abc.ABC, Generic[TConfig]):
  ...
  _sb_geo_train = None
  _sb_anchor_step = 0
  _sb_resumed_mid_step = False
  _sb_restored_full_batch_size = None
  _sb_restored_geometry = None
  _sb_step_complete_for_step = None
  _sb_last_snapshot_key = -1
  _sb_window_train_steps = -1
  _sb_local = -1
  _sb_last_snapshot_trainer_iter = -1
  ...
  def __init__(
    ...
    self._loop_thread = threading.Thread(target=run_loop_forever, daemon=True)
  ...
  def _init_sub_batch_checkpointing(self) -> None:
    ...
    if actor_trainer.checkpoint_manager.latest_step() is None:
      ...
      try:
        ...
        try:
          ...
          logging.exception(
              "Cleanup failed after sub-batch initialization failed"
          )
        raise
    ...
    try:
      ...
      try:
        ...
        logging.exception("Cleanup failed after sub-batch restore failed")
      raise
    ...
    if state.step_complete:
      # The manager validated that this final apply carries no accumulator
      # before purging any dead lineage.
    ...
    if state.training_state is not None:
      ...
      # Drop the staging reference; the live accumulator now owns the leaves.
      state.training_state = None
  ...
  def _sb_accumulator_shardings(self, trainer) -> dict[tuple[Any, ...], Any]:
    ...
    if mesh is None:
      ...
      return {
          path: leaf.sharding for path, leaf in zip(paths, leaves, strict=True)
      }
    out = {}
      ...
      for path, var in nnx.state(accumulator).flat_state():
        ...
        if isinstance(value, jax.Array):
          value = jax.device_put(
              value,
              value.sharding.with_memory_kind("pinned_host"),
              may_alias=False,
          )
        out[path] = value
    return out
  ...
  def _sb_chunk_will_apply(self, chunk: Any) -> bool:
    ...
    k = self._training_config.get_with_default("gradient_accumulation_steps", 1)
  ...
  def _sb_reinject(self, train_data_queue) -> None:
    ...
    if (
      ...
      return
    with self._sb_lock:
      active_items = list(self._sb_active)
  ...
  def _sb_resync_rollout_weights(self) -> None:
    ...
    if (
        self._sb_resumed_mid_step or self._sb_pending_state is not None
    ) and anchor != resume:
  ...
  async def _orchestrator_producer(
    ...
    async_generator = orchestrator.yield_batches(batch_size=num_generations)
    try:
  ...
  def close(self) -> None:
    ...
    """
    # Detach before closing: a failing close must not leave a half-closed
    # manager attached for a later close() (or __del__) to reuse.
    manager = self._sb_mgr
    self._sb_mgr = None
