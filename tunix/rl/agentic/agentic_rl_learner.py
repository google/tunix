...
class AgenticRLConfig(algo_config_lib.AlgorithmConfig):
  ...
  Parameters:
    ...
    sub_batch_checkpointing: Whether to checkpoint the sample rollout components
      ...
      sampler_is='token' are rejected there. Multi-host runs are rejected.
...
class AgenticRLLearner(abc.ABC, Generic[TConfig]):
  ...
  def _init_sub_batch_checkpointing(self) -> None:
    ...
    if jax.process_count() > 1:
      raise ValueError(
          "sub_batch_checkpointing does not support multi-host runs yet:"
          " the grad-accum buffer is snapshotted through host numpy copies"
          " of every leaf, which a globally sharded array cannot provide."
      )
  ...
  def _sb_inject_accumulator(
    ...
    for path, var in ga_state.flat_state():
      ...
      if isinstance(restored, jax.Array):
        restored.delete()
        # Later readers of the payload must see the host copy, not a
    ...
    if self._sb_pending_state is not None and anchor != resume:
      if not trainer.checkpoint_manager.has_step(anchor):
        raise RuntimeError(
            ...
            " pin. Move or delete <root>/sub_batch to abandon the step."
        )
  ...
  def _sb_set_anchor(self, train_steps: int) -> None:
    ...
    pinned checkpoint there. The step is still recorded in every snapshot:
    a run relaunched disaggregated can then resume it.
    """
  ...
  def _sb_validate_batch_geometry(
    ...
    if saved is not None and saved != full_batch_size:
      raise sub_batch_checkpoint.SubBatchGeometryError(
          ...
          " original configuration, or move or delete <root>/sub_batch to"
    ...
    if (
      ...
      raise sub_batch_checkpoint.SubBatchGeometryError(
          ...
          " configuration, or move or delete <root>/sub_batch to change"
  ...
  def close(self) -> None:
    ...
    if getattr(self, "_sb_mgr", None) is not None:
      self._sb_mgr.close()
      self._sb_mgr = None
