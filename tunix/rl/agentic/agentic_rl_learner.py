...
class AgenticRLConfig(algo_config_lib.AlgorithmConfig):
  ...
  Parameters:
    ...
    sub_batch_checkpointing: Whether to checkpoint the sample rollout components
      ...
      to reconcile against otherwise). Under sequence packing
      (`training_config.max_seq_token_per_tpu`) identity, skip and epoch
      accounting are per segment.
  """
...
class AgenticRLLearner(abc.ABC, Generic[TConfig]):
  ...
  _sb_preempt_at: set[int] | frozenset[int] = frozenset()
  ...
  _bench_last_apply_logged = -1
  _bench_last_save_timings = None
  ...
  def __init__(
    ...
    self._sb_pack_tags: collections.deque = collections.deque()
  ...
  def _init_sub_batch_checkpointing(self) -> None:
    ...
    if not self.algo_config.sub_batch_checkpointing:
      return
    # Sequence packing: identity, skip and epoch accounting move to the
    # segment level (tags through the packer + per-segment masking); the
    # keying and the buffer payload are mode-agnostic already.
    self._sb_packed = self._training_config.max_seq_token_per_tpu is not None
    self._sb_item_fifo = collections.deque()
    self._sb_pack_tags = collections.deque()
    ...
    if state is None:
      self._sb_report_abandoned_step(train_steps)
      return
    ...
    # (`_iter_steps % k == 0`) desyncs from the injected buffer.
  ...
  def _sb_report_abandoned_step(self, train_steps: int) -> None:
    """ERROR when a durable apply checkpoint has no matching snapshot while
    the previous window says its step was still in progress.

    `_sb_train_chunk` writes the boundary snapshot before the apply so this
    cannot happen; if it is ever seen, the remainder of that step was lost
    (the trainer's stamp advanced the run past it) and the run must not be
    trusted silently.
    """
    if self._sb_mgr is None or train_steps < 1:
      return
    meta = self._sb_mgr.peek_latest_meta(train_steps - 1)
    if meta is None or meta.get("step_complete"):
      return
    if meta.get("global_step") == self.rl_engine.global_steps - 1:
      logging.error(
          "Sub-batch restore: trainer checkpoint %d (global_step %d) has no"
          " matching snapshot, but window %d's latest snapshot says step %d"
          " was still in progress. The remainder of that step is NOT"
          " trained: the apply-boundary commit order was violated.",
          train_steps,
          self.rl_engine.global_steps,
          train_steps - 1,
  ...
  def _bench_log_progress(self) -> None:
    ...
    if not self._bench_metrics:
      return
    try:
      import wandb  # pyrefly: ignore[missing-import] # pytype: disable=import-error # pylint: disable=g-import-not-at-top
      ...
      timings = self._bench_last_save_timings
      if timings:
        for field, key in (
            ("perf/sub_batch_save_total_s", "save_total_s"),
            ("perf/sub_batch_save_wait_prev_s", "save_wait_prev_s"),
            ("perf/sub_batch_save_call_s", "save_call_s"),
            ("perf/sub_batch_boundary_barrier_s", "boundary_barrier_s"),
        ):
          if timings.get(key) is not None:
            payload[field] = timings[key]
  ...
  def _sb_snapshot(
    ...
    buffer. Called immediately after `update_actor` so the saved epoch counts
    match the saved gradient buffer -- except for a chunk whose training
    triggers the optimizer apply: `anticipated_apply=True` is called BEFORE
    `update_actor`, keys the snapshot under the post-apply `train_steps + 1`
    (local 0, no buffer: the apply resets the accumulator), and makes it
    durable, so the trainer's own apply checkpoint can never be durable ahead
    of it (see `_sb_train_chunk`). Such a snapshot is only ever selected once
    that apply is durable, and is purged as dead lineage otherwise.
    ...
    if at_apply_boundary:
      ...
      self._sb_mgr.wait()
      _barrier_s = round(time.perf_counter() - _tb, 4)
    # TEMPORARY (bench): overhead decomposition for this snapshot -- the
    # training-thread cost of each phase, plus the manager's own split of
    # the save call (wait-on-previous / payload build / sync save) and the
    # retention delta. These ride the snapshot event (durable, per-key)
    # and the perf/ payload (chartable); production behavior unchanged.
    self._bench_last_save_timings = {
        "extract_s": round(_t_extract - _t0, 4),
        "save_total_s": round(_t_save - _t_extract, 4),
        "boundary_barrier_s": _barrier_s,
        **(getattr(self._sb_mgr, "last_save_timings", None) or {}),
    }
    self._bench_event(
        "snapshot",
        key=sub_batch_key,
  ...
  def _sb_reinject(self, train_data_queue) -> None:
    ...
    Groups are re-fed in ledger (consumption) order. Under packing the packer
    re-arranges segments freely, but the update boundary falls every
    mini_batch_size * num_generations sequences of the STREAM, so this order
    is what puts each still-untrained segment back in its original window.
    """
    if (
  ...
  def _sb_claim_item_tag(self) -> tuple[Hashable, int]:
    """Packer `item_tagger` hook: the next staged identity, in row order."""
    if not self._sb_item_fifo:
      raise RuntimeError(
          "Sub-batch packed-identity FIFO underflow: the packer consumed more"
          " sequences than identities were staged. This is a wiring bug in"
          " the identity side-channel."
      )
    return self._sb_item_fifo.popleft()

  def _sb_packed_stream(self, train_data_gen):
    """Wraps the packer's input: stages each pulled micro-batch's identities
    on the item FIFO, in row order, before the packer consumes it."""
    for batch in train_data_gen:
      for ids in self._sb_dequeue_identities(
          1 if self._process_in_consumer else len(batch)
      ):
        self._sb_item_fifo.extend(ids)
      yield batch

  def _sb_pack_has_untrained(self, identity_bins, epoch: int) -> bool:
    """Whether any segment of a pack still needs epoch `epoch`."""
    with self._sb_lock:
      return any(
          self._sb_counts.get(ident, 0) < epoch
          for row in identity_bins
          for ident in row
      )

  def _sb_mask_trained_segments(self, chunk, identity_bins, epoch: int):
    """Masks the segments of a pack already trained `epoch` times.

    Zeroing a segment's completion_mask and advantages over its token span
    drops it from both numerator and denominator of the packed loss, so a
    resumed pack that mixes trained and untrained segments (FFD re-packs
    freely) trains exactly the untrained ones. Segment i of row b is
    identity_bins[b][i-1]. Returns (chunk, trained_ids, has_untrained); the
    chunk comes back untouched when nothing needs masking.
    """
    with self._sb_lock:
      done = [
          [self._sb_counts.get(ident, 0) >= epoch for ident in row]
          for row in identity_bins
      ]
    if not any(any(row) for row in done):
      trained_ids = [ident for row in identity_bins for ident in row]
      return chunk, trained_ids, bool(trained_ids)
    seg = np.asarray(chunk.segment_ids)
    keep = np.ones_like(seg, dtype=bool)
    trained_ids = []
    for b, (row, flags) in enumerate(zip(identity_bins, done)):
      for i, (ident, already) in enumerate(zip(row, flags), start=1):
        if already:
          keep[b] &= seg[b] != i
        else:
          trained_ids.append(ident)
    keep = jnp.asarray(keep)
    chunk = chunk.replace(
        completion_mask=chunk.completion_mask
        * keep.astype(chunk.completion_mask.dtype),
        advantages=chunk.advantages * keep.astype(chunk.advantages.dtype),
    )
    return chunk, trained_ids, bool(trained_ids)
  ...
  async def _producer(
    ...
    try:
      ...
      producer_stream = self._orchestrator_producer(**producer_kwargs)
      if inspect.iscoroutine(producer_stream):
        producer_stream = await producer_stream
      async for batch in producer_stream:
        try:
          ...
          if (
              self._sb_enabled
              and self._sb_identity_queue is not None
  ...
  def train(
    ...
    max_inflight = self._training_config.max_inflight_computations or 0
    train_data_queue = queue_lib.SimpleDataQueue(maxsize=max_inflight)
    if self._sb_enabled:
      # A parallel side-channel queue that passes trajectory identities
      # (group_id, pair_index) perfectly in step with the main
      # train_data_queue.
      self._sb_identity_queue = queue_lib.SimpleDataQueue(maxsize=max_inflight)
    ...
    if is_packed:
      ...
      if self._sb_enabled:
        # Identity rides each sequence through the packer as a tag: FFD
        # sorting and leftover carry-over make consumption order useless
        # for it. _sb_packed_stream stages identities in row order, the
        # tagger stamps them as the packer consumes, and every emitted
        # pack's per-row identity bins land on _sb_pack_tags.
        pack_kwargs.update(
            item_tagger=self._sb_claim_item_tag,
            on_chunk_tags=self._sb_pack_tags.append,
        )
        train_data_gen = self._sb_packed_stream(train_data_gen)
    ...
    sb_units_seen = 0
    # Packed windows are ragged, so the unit is the packer's update flag
    # (read off every pack, trained or skipped): fbs // mini per step.
    sb_units_per_step = (
        update_steps_per_full_batch
        if is_packed
        else max(1, full_batch_size // train_micro_batch_size)
    )
    did_eval_this_global_step = False
    full_batch_chunks = []
    full_batch_chunk_identities = []
    # Packed sub-batch mode, parallel to full_batch_chunks (one entry per
    # pack): the pack's per-row identity bins, and whether its old/ref logps
    # have been computed yet -- they are computed lazily, the first time the
    # pack executes, so a count-skipped pack never pays that forward.
    full_batch_chunk_bins = []
    full_batch_chunk_logps_done = []

    def _packed_logps(c):
      if not full_batch_chunk_logps_done[c]:
        full_batch_chunks[c] = self._compute_packed_logps(full_batch_chunks[c])
        full_batch_chunk_logps_done[c] = True
      return full_batch_chunks[c]

    # Whether a pack of the CURRENT accumulation window trained since the
    # window began. Its flagged pack must then execute (masked if need be)
    # so the apply drains the accumulator. Reset at every flag.
    sb_window_trained_any = False
    ...
    for train_micro_batch in train_data_gen:
      ...
      row_identity_bins: list[list[Any]] | None = None
      if is_packed:
        if self._sb_enabled:
          # This pack's per-row identity bins, reported by the packer at
          # emission: exact composition, whatever FFD did. Its old/ref logps
          # are computed lazily (_packed_logps), only if it executes.
          row_identity_bins = self._sb_pack_tags.popleft()
        else:
          # pack-first: old/ref logp were deferred (left None) so they can be
          # computed here on the packed buffer (segment-aware forward),
          # sharing the same packed representation training uses.
      ...
      # Identity recovery. Packed: flattened from the pack's bins. Otherwise
      # one identity list per queue item, dequeued in lockstep with the
      # batch generator's grouping.
      if self._sb_enabled and is_packed and row_identity_bins is not None:
        row_identities = [t for b in row_identity_bins for t in b]
      ...
      # Packed rows carry many segments each; their identity agreement is
      # the packer's tag callback's, not one-per-row.
      if self._sb_enabled and not is_packed and len(row_identities) != n_total:
        raise RuntimeError(
      ...
      if self._sb_enabled and not is_packed:
        # Packed chunks need no analogue: a resumed pack may legitimately
        # mix trained and untrained segments, and per-segment masking
        # (_sb_mask_trained_segments) makes that harmless.
      ...
      if self._sb_enabled and is_packed:
        full_batch_chunk_bins.append(row_identity_bins)
        full_batch_chunk_logps_done.append(False)
      ...
      if self._sb_enabled:
        if is_packed:
          ...
          mb_closes_step = mb_flag and sb_units_seen + 1 == sb_units_per_step
        else:
          mb_flag = False
      ...
      if self._sb_enabled and is_packed:
        # Mask-or-skip, per pack. Already-trained segments are MASKED (zero
        # loss contribution), never re-trained. A pack with nothing left to
        # train is skipped (no denom, no ghost optimizer step) -- unless it
        # carries the window's update flag while earlier packs of the window
        # trained: then it executes fully masked so the apply drains the
        # accumulator. (A flagged pack whose apply never landed always has
        # untrained segments itself: its boundary snapshot is pre-committed
        # before training, and a crash between the two purges that snapshot
        # as dead lineage.) The flagged pack is the window's last, so
        # step_complete anchors on it.
        c_cur = len(full_batch_chunks) - 1
        if self._sb_pack_has_untrained(row_identity_bins, 1) or (
            mb_flag and sb_window_trained_any
        ):
          chunk, train_ids, has_untrained = self._sb_mask_trained_segments(
              _packed_logps(c_cur), row_identity_bins, epoch=1
          )
          ...
          self._sb_train_chunk(
              ...
              eval_ds,
              skip_jit,
              step_complete=self._num_iterations() == 1 and mb_closes_step,
          )
          sb_window_trained_any = sb_window_trained_any or has_untrained
        if mb_flag:
          sb_window_trained_any = False
      elif self._sb_enabled:
      ...
      # --- Weight Sync Logic ---
      if self._sb_enabled:
        sb_units_seen += (1 if mb_flag else 0) if is_packed else 1
      else:
        if is_packed:
      ...
      if _step_boundary_reached:
        ...
        for i in range(1, iterations):
          ...
          if self._sb_enabled and is_packed:
            # Mask-or-skip over the stored packs (the epoch-1 rule). The plan
            # is precomputed so step_complete anchors on the last pack that
            # actually executes; packs hold disjoint segments, so training
            # one never changes another's decision.
            epoch = i + 1
            plan = []
            window_trained = False
            for c, bins in enumerate(full_batch_chunk_bins):
              flag = bool(
                  np.asarray(full_batch_chunks[c].is_update_step).item()
              )
              untrained = self._sb_pack_has_untrained(bins, epoch)
              plan.append(untrained or (flag and window_trained))
              window_trained = (
                  False if flag else (window_trained or untrained)
              )
            last_exec = max((c for c, e in enumerate(plan) if e), default=-1)
            for c in range(len(full_batch_chunks)):
              if not plan[c]:
                continue
              chunk, train_ids, _ = self._sb_mask_trained_segments(
                  _packed_logps(c), full_batch_chunk_bins[c], epoch=epoch
              )
              eval_ds = pending_eval_dataset
              pending_eval_dataset = None
              self._sb_train_chunk(
                  ...
                  eval_ds,
                  skip_jit,
                  step_complete=(i == iterations - 1 and c == last_exec),
              )
          elif self._sb_enabled:
            # To accurately set `step_complete=True` during snapshots, we must
        ...
        full_batch_chunk_bins.clear()
        full_batch_chunk_logps_done.clear()
  ...
  def close(self) -> None:
    """Cleans up background threads and event loop resources."""
