...
class AgenticRLLearner(abc.ABC, Generic[TConfig]):
  ...
  def _sb_split_by_epoch(
    ...
    If a crash occurs mid-sweep during a multi-iteration step (num_iterations > 1),
    re-injected chunks might mix rows that have already been trained N times
    pre-crash with rows that have only been trained N-1 times pre-crash.
    Because you cannot partially evaluate or mask a tensor inside the TPU
    `update_actor` gradient step, we must physically slice the mixed tensor array
    here to prevent double-training or under-training individual rows.

    Returns the chunk unchanged in the common case where all rows
  ...
  async def _orchestrator_producer(
    ...
    Args:
      ...
      apply_sub_batch_skip: Whether to consult the sub-batch ledger to skip
        pre-crash groups. Must be True ONLY for the training stream. Because this 
        generator is shared with evaluation, if eval passed True, its independently
        numbered group IDs could accidentally collide with the training ledger 
        and cause eval prompts to be silently dropped.
    ...
    try:
      ...
      async for batch in self._orchestrator_producer(**producer_kwargs):
        try:
          ...
          # TrainExample tensors. The parallel identity queue (run 1:1) is populated
          # now so these string IDs can safely bypass the tensor pipeline.
