...
class Qwen3EngineForwardAdapter:
  ...
  def segmented_grpo_value_and_grad(
    ...
    # G6 (the committed update path) streams every microbatch gradient to
    # exactly one sink; with G6 off the call keeps one aggregate gradient,
    # which is the oracle the adapter tests compare the stream against.
    g6_update = os.environ.get("CANON_P28_G6_UPDATE", "") == "1"
    num_sinks = int(gradient_microbatch_sink is not None)
    if g6_update and num_sinks != 1:
      raise FunctionalMappingError(
          "P28 G6 must provide exactly one streaming gradient sink"
      )
    if not g6_update and num_sinks != 0:
      raise FunctionalMappingError(
          "P28 aggregate loss (CANON_P28_G6_UPDATE unset) admits no sink"
      )
    if getattr(train_example, "segment_ids", None) is not None:
      raise FunctionalMappingError(
