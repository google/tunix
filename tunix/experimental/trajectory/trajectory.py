...
import datetime
import enum
from typing import Annotated, Any, Dict, Final, Literal, get_args
...
class TunixStep(Step):
  """A single turn/interaction step with Tunix-specific extensions."""
  ...
  @pydantic.model_validator(mode="after")
  def validate_tunix_fields(self) -> TunixStep:
...
class TunixTrajectory(TunixTrajectoryMetadata):
  ...
  steps: list[TunixStep] = pydantic.Field(
      default_factory=list,
      description="Sequential step history.",
  )
  ...
  @pydantic.field_validator("steps")
  @classmethod
  def validate_step_ids(cls, steps: list[TunixStep]) -> list[TunixStep]:
