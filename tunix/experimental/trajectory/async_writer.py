...
class WriteTask:
  ...
  def __post_init__(self) -> None:
    """Deep copies and projects the payload to base ATIF."""
    object.__setattr__(
        self,
        "metadata",
        self.metadata.to_atif_metadata().model_copy(deep=True),
    )
    if self.step is not None:
      object.__setattr__(
          self, "step", self.step.to_atif_step().model_copy(deep=True)
      )
