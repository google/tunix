...
class VllmSamplerAdapter(Sampler, weight_sync.WeightSyncDestination):
  ...
  async def sample(
    ...
    # Tunix spells "leave this at the sampler's default" as None -- see
    # run_gsm8k_dist_grpo.py, which maps --top_k<0 to None before putting it in
    # generation_kwargs, which collector.py then forwards here as **kwargs.
    # RLVllmSampler instead treats these kwargs as literal fallbacks:
    # _build_vllm_params does kwargs.get("top_k", -1), so an explicit None wins
    # over the -1 and reaches vLLM's SamplingParams, where `top_k < -1` raises
    # "'<' not supported between instances of 'NoneType' and 'int'". Its
    # _get_val already coalesces None out of sampling_params; only the kwargs
    # leg is unguarded. Drop the Nones so the sampler's own defaults apply.
