...
Address model. Four addresses are involved and must not be conflated:
  ...
  2. The controller's advertised address: what remote workers dial, typically
     a name-service address rather than a literal host:port.
     Deployment-specific; passed in, never derived.
...
resolver when worker addresses require resolution; giving the resolver only to
...
# The runtime alias resolves against whichever import branch succeeded; the
# type checker cannot follow that, so it gets a concrete (type-only) import.
if TYPE_CHECKING:
...
class RaidenWorkUnitMetadata:
  ...
  Attributes:
    ...
    shards: Data-plane addresses, one "ip:port" per participating shard.
      ...
      transfer port, so the list carries the shard count while the addresses
      coincide.
...
class RaidenHandler(WeightSyncHandler):
  ...
  def __init__(
    ...
    Args:
      ...
      advertised_address: Address remote workers dial to reach this controller,
        e.g. a name-service address. Defaults to the self-dial address, which
        is only correct when every worker is on this host.
      dial_address: Address this handler's own facade dials. Defaults to
        `[::1]:{port}`. IPv6 loopback is load-bearing: the controller's
        listener can come up IPv6-only (see module docstring), and dialing
        127.0.0.1 then times out rather than failing fast.
      name_resolver: Optional name-service resolver. Wired into both the
        facade (this handler's outbound calls) and the controller's own
        ...
        under names rather than literal addresses are unreachable without it.
      transfer_parallelism: Default number of transport streams per unit.
      transfer_uuid: Generation id carried on every transfer this handler
        starts when the caller does not supply one. Status lookups are keyed
        by (req_id, uuid).
    """
    ...
    # Registrations run concurrently on the coordinator's executor (the
    # facade opens a connection per call, so the transport supports it);
    # this set is the one piece of state they share.
  ...
  def transfer(
    ...
    with self._registered_lock:
      missing = [
          u for u in (*src_units, *dst_units) if u not in self._registered
      ]
    if missing:
