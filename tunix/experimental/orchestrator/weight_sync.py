  ...
  def __init__(
    ...
    Args:
      ...
      name_resolver: Optional resolver, e.g. a BNS resolver on Borg. Wired into
        both the facade (this handler's outbound calls) and the controller's
        own `WeightSyncWorkerRpcClient` (the controller's outbound calls to
        worker control-plane addresses). The second wiring matters: workers
        registered under BNS control addresses are unreachable without it.
