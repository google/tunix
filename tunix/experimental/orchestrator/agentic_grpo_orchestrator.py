...
Remaining seams (`_model_call` -> rollout workers, `_sync_and_advance_step` ->
worker-driven weight sync, `_maybe_run_eval`/`_process_micro_batch`) are wired in
the same way in follow-up steps. Everything else -- reward/advantage math,
TrainExample assembly, metrics, checkpointing -- is inherited unchanged.

A trainer handle must expose:
    train(chunks: list, eval_ds, skip_jit: bool) -> None
"""
...
class AgenticGRPOOrchestrator(agentic_grpo_learner.GRPOLearner):
  ...
  def __init__(self, *, trainer_worker: Any, **kwargs):
