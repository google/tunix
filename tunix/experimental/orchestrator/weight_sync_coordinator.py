...
class RemoteParticipantProxy(abstract_worker.Worker):
  ...
  async def prepare_weight_sync(self, sync_request: Any = None, **kwargs: Any):
    return await self._handle.asubmit(
        "prepare_weight_sync", sync_request, **kwargs
    )

  async def release_weight_sync(self, sync_request: Any = None, **kwargs: Any):
    return await self._handle.asubmit(
        "release_weight_sync", sync_request, **kwargs
    )
  ...
  async def post_weight_sync(self, sync_request: Any = None, **kwargs: Any):
    return await self._handle.asubmit(
        "post_weight_sync", sync_request, **kwargs
    )

  async def abort_weight_sync(self, sync_request: Any = None, **kwargs: Any):
    return await self._handle.asubmit(
        "abort_weight_sync", sync_request, **kwargs
    )
  ...
  async def sync(
    ...
    Args:
      ...
      expected_block_count: The number of physical pushes each destination
        listener expects under one transfer uuid (per destination unit, not a
        global total and not per device shard). None/0 — the default — is
        AUTO: Raiden's sender-side direct-schedule planning derives the count
          from its own schedule, the only authoritative source (one schedule
          entry can carry several pushes, so caller-side guesses from shapes are
          unreliable). The delegation is scoped to that path; other controller
          paths treat 0 differently, which is why the handler, not this
          coordinator, owns the wire encoding. A positive value overrides the
          transport, for experiments only.
    ...
    try:
      ...
      if (
        ...
        if state is RoundState.PENDING_COMMIT:
          ...
          await asyncio.shield(self._abort_all(destinations, prepared_request))
      raise
    finally:
      if release_source:
