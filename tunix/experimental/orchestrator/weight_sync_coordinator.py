...
def _manifest_entries(
    metadata_list: Sequence[weight_sync.RaidenWorkUnitMetadata],
    side: str,
    problems: list[str],
  ...
  units contribute under their RaidenId.data_name. Units of one side share
...
def _manifest_mismatches(
    src_metadata: Sequence[weight_sync.RaidenWorkUnitMetadata],
    dst_metadata: Sequence[weight_sync.RaidenWorkUnitMetadata],
) -> list[str]:
...
class WeightSyncCoordinator:
  ...
  async def _run_round(
    ...
    try:
      ...
      # Registrations are independent (the facade opens a connection per
      # call), so they run concurrently. `return_exceptions=True` is
      ...
      # future settles before any error is acted on; the facade's own
      # socket deadline is what bounds them. The shield exists for the same
      # reason cancellation-side: a cancel cannot stop the executor
