...
class WeightSyncCoordinator:
  ...
  async def _run_round(
    ...
    def fail(message: str) -> WeightSyncError:
      # `failures` (per-worker phase errors, e.g. from `_phase_on_all`) is
      # otherwise only reachable via the raised error's `.result.failures`,
      # which callers logging the exception itself (e.g. `str(e)`) never see
      # -- leaving phase failures undiagnosable from logs alone. Log them
      # here, matching the manifest-preflight logging a few lines up.
      if failures:
        logging.error(
