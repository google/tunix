...
def _submit_worker(
  ...
  """Invokes worker.submit safely even when an event loop is active on the current thread."""
  try:
  ...
  if loop is not None and loop.is_running():
    with futures.ThreadPoolExecutor(max_workers=1) as pool:
      return pool.submit(worker.submit, method_name, *args, **kwargs).result()
  return worker.submit(method_name, *args, **kwargs)
  ...
  def configure_worker(
    ...
    match role:
      case datatypes.Role.ACTOR | datatypes.Role.CRITIC:
        ...
        _submit_worker(worker, "with_loss_fn", algo.loss_fn(), has_aux=True)
        ...
        _submit_worker(worker, "with_gen_model_input_fn", gen_fn)
