...
class ClusterOrchestrator:
  ...
  def run_program(
    ...
    if isinstance(program, async_rl_program.AsyncRLProgram):
      try:
      ...
      if loop and loop.is_running():
        asyncio.create_task(program.run_async(engine, num_steps=num_steps or 1000, **kwargs))
      else:
        asyncio.run(program.run_async(engine, num_steps=num_steps or 1000, **kwargs))
    elif hasattr(program, "run"):
      if train_dataset is not None:
        program.run(train_dataset=train_dataset, num_steps=num_steps, **kwargs)
      else:
        program.run(engine=engine, num_steps=num_steps, **kwargs)
