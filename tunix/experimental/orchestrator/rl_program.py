...
def _generation_metrics(
  ...
  `agentic_utils.response_budget_facts`, because only the producer knows the
  budget a rollout was held to -- `DistributedRLEngine` lets a dataset item
  override `max_response_length`. Raw length counts env and tool tokens, per
  the rLLM/VERL `response_length` convention.
  ...
  for group in groups:
    ...
    for item in group:
      ...
      if not isinstance(traj, dict):
        continue
      if "clipped" not in traj or "raw_length" not in traj:
        continue
      ...
      lengths.append(int(traj["raw_length"]))
      clipped += int(traj["clipped"])
    if not lengths:
      continue
