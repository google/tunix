...
class PeftTrainer(abstract_trainer.AbstractTrainer):
  ...
  def _update_step(
    ...
    print("JIT TRACE v2 [update_step] acc_grads output dtypes: {x}", x=acc_dtypes)
  ...
  def fwd_bwd(self, payload: datatypes.TrainerPayload | Any, **kwargs) -> None:
    ...
    train_loss, aux = fwd_bwd_step(self.model, self.grad_accumulator, payload)
    print("Debug: train_loss: ", jax.device_get(train_loss))
  ...
  def update(self, **kwargs) -> int:
    ...
    grad_norm = update_step(self.model, self.optimizer, self.grad_accumulator)
