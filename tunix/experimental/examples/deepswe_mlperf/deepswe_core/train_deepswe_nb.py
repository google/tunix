...
k8s_config: typing.Any = None
try:
  from kubernetes import client as _client, config as _k8s_config  # pytype: disable=import-error
  client = _client
...
try:
  from tunix.experimental.examples.deepswe_mlperf.deepswe_core import swe_agent
  from tunix.experimental.examples.deepswe_mlperf.deepswe_core import swe_env
except ImportError:
...
# Kubernetes Setup
if k8s_config is not None and client is not None:
  try:
    getattr(k8s_config, "load_kube_config")()
    k8s_client = getattr(client, "CoreV1Api")()
