...
# Detect physical TPU chip count on the host.
if [[ -z "${HOST_CHIP_COUNT:-}" ]]; then
  HOST_CHIP_COUNT=$("$PYTHON_BIN" -c "import jax; print(len(set(tuple(d.coords) for d in jax.devices())))" 2>/dev/null || echo 8)
fi

if (( HOST_CHIP_COUNT <= 4 )); then
  DEFAULT_TRAINER_CHIPS="0,1"
  DEFAULT_ROLLOUT_CHIPS="2,3"
  ...
  DEFAULT_TRAINER_CHIPS="0,1,2,3"
  DEFAULT_ROLLOUT_CHIPS="4,5,6,7"
...
TRAINER_TPU_CHIPS=${TRAINER_TPU_CHIPS:-$DEFAULT_TRAINER_CHIPS}
TRAINER_FSDP=${TRAINER_FSDP:-1}
TRAINER_TP=${TRAINER_TP:-4}
ROLLOUT_TPU_CHIPS=${ROLLOUT_TPU_CHIPS:-$DEFAULT_ROLLOUT_CHIPS}
ROLLOUT_FSDP=${ROLLOUT_FSDP:-1}
ROLLOUT_TP=${ROLLOUT_TP:-4}

if [[ -z "${TPU_CHIPS_PER_HOST_BOUNDS:-}" ]]; then
  TRAINER_CHIPS_COUNT=$(count_chips "$TRAINER_TPU_CHIPS")
  if (( TRAINER_CHIPS_COUNT == 2 )); then
    TPU_CHIPS_PER_HOST_BOUNDS="1,2,1"
  elif (( TRAINER_CHIPS_COUNT == 4 )); then
    TPU_CHIPS_PER_HOST_BOUNDS="1,4,1"
  else
    TPU_CHIPS_PER_HOST_BOUNDS="1,${TRAINER_CHIPS_COUNT},1"
  fi
fi
