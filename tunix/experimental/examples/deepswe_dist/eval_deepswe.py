...
def parse_args(argv=None):
  ...
  p.add_argument("--max_context_limit", type=int, default=14336)
  p.add_argument(
      "--max_response_length",
      type=int,
      default=12288,
  ...
  p.add_argument("--prewarm_batch_size", type=int, default=8)
  p.add_argument(
      "--docker_image_prefix",
      default=(
          os.environ.get("IMAGE_REWRITE_PREFIX")
          or os.environ.get("DOCKER_IMAGE_PREFIX")
          or ""
      ),
  )
  p.add_argument("--clip_obs_len", type=int, default=12000)
  p.add_argument("--enforce_xml_function_check", type=boolean, default=True)
  ...
  if not 0 < a.max_context_limit < a.max_model_len:
    p.error("--max_context_limit must be smaller than --max_model_len")
  if (
...
def request_fields(a, entry, index, attempt):
  ...
  return {
      ...
      # Evaluation limits each turn and the complete rendered context
      # independently; a training-style cumulative response budget is not used.
      ...
      "generation_kwargs": {
          ...
          "top_k": a.top_k,
          "seed": (
              (a.seed + index * a.num_rollouts_per_instance + attempt) % (2**31)
          ),
      ...
      "metadata": {
          ...
          "collection_timeout": a.timeout + a.reward_timeout + 180,
          "max_context_length": a.max_context_limit,
          "max_model_length": a.max_model_len,
          ...
          "env_config": {
              ...
              "scaffold": a.scaffold,
              "clip_obs_len": getattr(a, "clip_obs_len", 12000),
              "enforce_xml_function_check": (
                  getattr(a, "enforce_xml_function_check", True)
                  and a.scaffold != "openhands"
              ),
              "verbose": False,
          },
...
async def evaluate_worker(
    handle, jobs, limit, timeout, write_record, jobs_lock=None
):
      ...
      try:
        if jobs_lock is not None:
          async with jobs_lock:
            sentinel = object()
            fields = await asyncio.to_thread(next, jobs, sentinel)
...
def load_entries(a):
  ...
  prefix = (
      getattr(a, "docker_image_prefix", "")
      or os.environ.get("IMAGE_REWRITE_PREFIX")
      or os.environ.get("DOCKER_IMAGE_PREFIX")
      or None
  )
  entries = []
  ...
  for i in range(count):
    entry = swe_env._normalize_entry(  # pylint: disable=protected-access
        dict(dataset[i]), docker_image_prefix=prefix
    )
    if not entry.get("problem_statement"):
      stmt = swe_env._problem_statement(entry)  # pylint: disable=protected-access
      if stmt:
        entry["problem_statement"] = stmt
    if not all(
        entry.get(key)
...
async def run_controller(a):
  ...
  os.environ.setdefault("JOB_PREFIX", "qwen35-eval")
  if getattr(a, "docker_image_prefix", ""):
    os.environ.setdefault("IMAGE_REWRITE_PREFIX", a.docker_image_prefix)
  ...
  try:
    ...
    if a.use_agent_sandbox:
      try:
        ...
        logging.warning("Skipping controller warmpool pre-warming: %s", exc)
    jobs_lock = asyncio.Lock() if len(handles) > 1 or prewarm_iter else None
    ...
    for index, handle in enumerate(handles):
      ...
      tasks.append(
          asyncio.create_task(
              evaluate_worker(handle, jobs, limit, deadline, record, jobs_lock)
          )
      )
    ...
    if prewarm_iter is not None:
      try:
        await asyncio.to_thread(prewarm_iter.close)
