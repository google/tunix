...
def _parse_args(argv: list[str]) -> argparse.Namespace:
  ...
  parser.add_argument(
      "--trainer_backend",
      choices=("peft", "maxtext", "tunix"),
      default="peft",
      help=(
          "peft/tunix runs tunix's PeftTrainer on a tunix Qwen3; maxtext runs "
  ...
  parser.add_argument(
      ...
      default=0,
      help=(
          "Explicit padded_base_moe_mlp_dim override for MoE models."
      ),
  )
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      default=0.0,
      help=(
          "MaxText's LR schedule warms up linearly from zero."
      ),
  )
  parser.add_argument(
      ...
      type=int,
      default=250,
      help="Number of steps between saving actor checkpoints. 0 means disable.",
  )
  parser.add_argument(
      "--checkpoint_max_to_keep",
      type=int,
      default=1,
      help="Number of actor checkpoints to keep.",
  )
  parser.add_argument(
...
def _create_maxtext_trainer_factory(args) -> Any:
  ...
  def _factory():
    ...
    return _MeshBoundTrainer(engine, mesh, log_train_steps=args.log_train_steps)
