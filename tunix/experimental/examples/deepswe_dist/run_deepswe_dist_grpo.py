...
TODO: We will add the trainer and sampler once the logic in gsm8k is merged and
stable.
...
# TODO(wuhao): We will add the trainer and sampler once the logic in
# gsm8k is merged and stable.
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
  ...
  parser.add_argument(
      ...
      action="store_true",
      default=False,
      help="Run mock rollout/trainer",
  )
  parser.add_argument(
      "--max_turns", type=int, default=10, help="Max interaction turns"
  )
...
def _configure_trainer_loss(
  ...
  """Configures GRPO loss on the trainer worker."""
...
def _make_weight_sync_coordinator(trainer_handle, rollout_handles, mode: str):
  ...
  # TODO(tunix-dev): Standardize a handler registry separate from the
  # worker registry.
  if mode == "raiden":
...
def main(argv: list[str], context: Any = None) -> None:
  is_test_mode = (
      "--test_mode" in argv
      or "--test_mode=True" in argv
      or "--test_mode=true" in argv
      or "-test_mode" in argv
  )

  if (
      context
      and getattr(context, "ipc", None)
      and getattr(context.ipc, "discovery", None)
  ) or is_test_mode:
    pass
  else:
    raise RuntimeError(
  ...
  if args.test_mode and "JAX_PLATFORMS" in os.environ:
  ...
  os.environ["NAMESPACE"] = "rl-tunix-swebench"
  os.environ["NODE_SELECTOR_KEY"] = "cloud.google.com/gke-nodepool"
  os.environ["NODE_SELECTOR_VAL"] = "deepswe-cpu-pool"

  from datasets import load_dataset  # pylint: disable=g-import-not-at-top

  dataset = load_dataset(
      "R2E-Gym/R2E-Gym-V1", split="train", trust_remote_code=True
  )
  # pylint: disable=protected-access
  swe_env._patch_r2egym_for_agent_sandbox()
  swe_env._init_global_fleet(
      tasks=dataset, max_concurrency=args.num_rollout_workers
  )
  ...
  if args.test_mode:
    ...
    class MockRolloutWorker(abstract_worker.Worker):
      """Mock Rollout Worker for offline testing."""
      ...
      def generate(self, *unused_args, **unused_kwargs):
        del unused_args, unused_kwargs
        return [
            datatypes.TrajectoryItem(
                traj="<function=execute_bash><parameter=cmd>pwd</parameter></function>"
            )
        ]
      ...
      def __getattr__(self, name):
        ...
        def dummy_method(*unused_args, **unused_kwargs):
          del unused_args, unused_kwargs
          return datatypes.Response()

        return dummy_method

    class MockTrainerWorker(abstract_worker.Worker):
      """Mock Trainer Worker for offline testing."""
      ...
      def info(self) -> datatypes.WorkerInfo:
        return datatypes.WorkerInfo(
            worker_id=self.worker_id, roles=frozenset({"trainer"})
        )
    ...
    rollout_port = portpicker.pick_unused_port()

    trainer_server = remote_execution.GrpcRemoteExecutionServer(
        MockTrainerWorker("mock-train-0")
    )
    rollout_server = remote_execution.GrpcRemoteExecutionServer(
        MockRolloutWorker("mock-rollout-0")
    )

    def serve():
      async def run_servers():
        await trainer_server.start_serving_async(port=trainer_port)
        await rollout_server.start_serving_async(port=rollout_port)
    ...
    # Inject directly into Futures
    trainer_addr_future.set_result(f"localhost:{trainer_port}")
    rollout_addrs_by_worker["mock-rollout-0"] = f"localhost:{rollout_port}"
  ...
  logging.info(
      ...
      trainer_addr,
      [addr for _, addr in rollout_entries],
  ...
  program = DeepsweRLProgram(
      dataset=swe_env.PrewarmDatasetIterator(
          dataset,
