  ...
  "$PYTHON_BIN" "$YAML_GENERATOR" \
    ...
    --worker_startup_command=" \
      ...
      python -m tunix.experimental.distributed.runtime.main \
        ...
        $([[ "${USE_ROLLOUT_LOGPS}" == "false" || "${USE_ROLLOUT_LOGPS}" == "False" || "${USE_ROLLOUT_LOGPS}" == "0" || "${FORCE_ON_POLICY_RATIO}" == "true" || "${FORCE_ON_POLICY_RATIO}" == "True" ]] && echo --no-use_rollout_logps || echo --use_rollout_logps) \
        ${dataset_args} \
        ${shuffle_arg} \
        ${sandbox_arg} \
...
stop_rollout() {
  ...
  if [[ ${ROLLOUT_REPLICAS} -gt 1 ]]; then
    kubectl delete jobset $(seq -f "${ROLLOUT_ID}-%g" 0 $((ROLLOUT_REPLICAS - 1))) --ignore-not-found=true 2>/dev/null || true
  fi
}
...
start_rollout() {
  ...
  if [[ "$SAMPLER" == "vllm" || "$SAMPLER" == "inprocess_vllm" ]]; then
    ...
    ${VLLM_DATA_PARALLEL_SIZE:+--data-parallel-size=${VLLM_DATA_PARALLEL_SIZE}} \
    $([[ "${VLLM_ENABLE_EXPERT_PARALLEL}" == "true" || "${VLLM_ENABLE_EXPERT_PARALLEL}" == "1" ]] && echo --enable-expert-parallel) \
    ${VLLM_PREFIX_CACHE_RETENTION_INTERVAL:+--prefix-cache-retention-interval=${VLLM_PREFIX_CACHE_RETENTION_INTERVAL}} \
    ${VLLM_KV_CACHE_DTYPE:+--kv-cache-dtype=${VLLM_KV_CACHE_DTYPE}} \
    ${VLLM_BLOCK_SIZE:+--block-size=${VLLM_BLOCK_SIZE}} \
    $([[ "${VLLM_ASYNC_SCHEDULING}" == "true" || "${VLLM_ASYNC_SCHEDULING}" == "1" ]] && echo --async-scheduling) \
    $([[ "${VLLM_ENABLE_CHUNKED_PREFILL}" == "true" || "${VLLM_ENABLE_CHUNKED_PREFILL}" == "1" ]] && echo --enable-chunked-prefill) \
    $([[ "${VLLM_LANGUAGE_MODEL_ONLY}" == "true" || "${VLLM_LANGUAGE_MODEL_ONLY}" == "1" ]] && echo --language-model-only) \
    $([[ "${VLLM_ENABLE_AUTO_TOOL_CHOICE}" == "true" || "${VLLM_ENABLE_AUTO_TOOL_CHOICE}" == "1" ]] && echo --enable-auto-tool-choice) \
    ${VLLM_TOOL_CALL_PARSER:+--tool-call-parser=${VLLM_TOOL_CALL_PARSER}} \
    ${VLLM_REASONING_PARSER:+--reasoning-parser=${VLLM_REASONING_PARSER}} \
    ${VLLM_DEFAULT_CHAT_TEMPLATE_KWARGS:+--default-chat-template-kwargs='${VLLM_DEFAULT_CHAT_TEMPLATE_KWARGS}'} \
    ${VLLM_LIMIT_MM_PER_PROMPT:+--limit-mm-per-prompt='${VLLM_LIMIT_MM_PER_PROMPT}'} \
    ${VLLM_ADDITIONAL_CONFIG:+--additional_config='${VLLM_ADDITIONAL_CONFIG}'} \
    ${ROLLOUT_EXTRA_ARGS} \
    "
  fi
  local lora_args=""
...
if [[ -z "${KUBECONFIG:-}" ]]; then
  ...
  elif [[ -f "$HOME/.kube/config.cloud-tpu-multipod-dev.us-central1.trellis-demo-0810" ]]; then
