...
DEFAULT_DEEPSWE_BOOTSTRAP="sed -i 's/_runtime_version.ValidateProtobufRuntimeVersion/if False: _runtime_version.ValidateProtobufRuntimeVersion/g' /app/tunix/experimental/distributed/runtime/discovery/*_pb2.py /opt/venv/lib/python3.12/site-packages/tunix/experimental/distributed/runtime/discovery/*_pb2.py 2>/dev/null || true; python3 -c 'import r2egym' 2>/dev/null || (uv pip install kubernetes gym 'k8s-agent-sandbox>=0.5.1' 'datasets>=2.21.0' httpx && uv pip install --no-deps swebench==3.0.2 'openhands-sdk>=1.44.1' git+https://github.com/kubernetes-sigs/agent-sandbox.git#subdirectory=clients/python/agentic-sandbox-client git+https://github.com/kubernetes-sigs/agent-sandbox.git#subdirectory=examples/agent-sandbox-rl git+https://github.com/kubernetes-sigs/agent-sandbox.git#subdirectory=clients/integrations/openhands git+https://github.com/r2e-gym/r2e-gym.git@0d94c4eb9431cd195c55a7ea3abd54006c9a1735 && sed -i 's/create_repo, upload_folder, HfFolder/create_repo, upload_folder/' /opt/venv/lib/python3.12/site-packages/r2egym/agenthub/utils/utils.py && sed -i 's/self.commit = ParsedCommit(\*\*json.loads(self.commit_json))/self.commit = ParsedCommit(\*\*(json.loads(self.commit_json) if isinstance(self.commit_json, str) else self.commit_json))/' /opt/venv/lib/python3.12/site-packages/r2egym/agenthub/runtime/docker.py); sed -i 's/pa\.PyExtensionType/getattr(pa, \"PyExtensionType\", pa.ExtensionType)/g' /opt/venv/lib/python3.12/site-packages/datasets/features.py /opt/venv/lib/python3.12/site-packages/datasets/features/features.py 2>/dev/null || true; sed -i 's/\"scan_layers=True\",/\"scan_layers=True\", \"allow_split_physical_axes=True\",/; s/cosine_learning_rate_final_fraction=/learning_rate_final_fraction=/g; /_weight_dtype.*float32/d; /enable_router_replay=True/d' /app/tunix/utils/maxtext_utils.py 2>/dev/null || true; sed -i 's|\"init_weights_seed=42\",|\"init_weights_seed=42\", *([\"trainable_parameters_mask=\" + os.environ[\"TRAINABLE_PARAMETERS_MASK\"]] if os.environ.get(\"TRAINABLE_PARAMETERS_MASK\") else []),|' /app/tunix/utils/maxtext_utils.py 2>/dev/null || true; sed -i 's|if not is_fp8_dtype(config.weight_dtype):|if getattr(config, \"float32_gate_logits\", False) and module_name in (\"norm\", \"gate\", \"shared_expert_gate\", \"A_log\", \"dt_bias\", \"conv1d\", \"logits_dense\"): return \"float32\"\n  if not is_fp8_dtype(config.weight_dtype):|' /app/maxtext/src/maxtext/common/common_types.py 2>/dev/null || true;"
    ...
    "$PYTHON_BIN" "$YAML_GENERATOR" \
      ...
      --worker_startup_command=" \
        ...
        SKIP_JAX_PRECOMPILE=1 VERIFY_WEIGHTS=${VERIFY_WEIGHTS} ${sandbox_env} ${ROLLOUT_USE_BATCHED_RPA:+USE_BATCHED_RPA_KERNEL=1} python -m tunix.experimental.distributed.runtime.main \
          ...
          ${DEBUG:+--debug} \
      " \
      | (if [[ "${ROLLOUT_ENV_FLAGS:-}" == *"TPU_HOST_BOUNDS=1,1,1"* ]]; then sed 's/completions: [0-9]\+/completions: 1/g; s/parallelism: [0-9]\+/parallelism: 1/g'; else cat; fi) \
      | apply_manifest
  done
}
