...
def main(argv: list[str], context: Any = None) -> None:
  ...
  os.environ.setdefault("VLLM_ENABLE_V1_MULTIPROCESSING", "0")
  # vLLM selects its TPU implementation while importing the package, so this
  # must be set before `_import_vllm_sampler()` imports `vllm`. Setting it in
  # VllmSampler.__init__ is too late for platform/plugin discovery.
  if args.sampler in ("vllm", "inprocess_vllm"):
