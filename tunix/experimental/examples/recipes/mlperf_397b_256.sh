...
export MAXTEXT_OUTPUT_DIR="${MAXTEXT_OUTPUT_DIR:-gs://${USER}-trellis-europe-west4-dev/maxtext/${JOB_PREFIX}}"
export TRAJECTORY_LOG_DIR="${TRAJECTORY_LOG_DIR:-gs://${USER}-trellis-europe-west4-dev/trajectories/${JOB_PREFIX}}"
...
export MAXTEXT_CKPT="${MAXTEXT_CKPT:-gs://mlperf-6-1-submission/ckpt/qwen35_397b/scanned_reshard_ocdbt_fsdp64_tp2/0/items}"
export TRAINABLE_PARAMETERS_MASK='^(?!.*routed_experts/gate/kernel).*'
export EOS_TOKENS="${EOS_TOKENS:-151645,151643}"
export TRAJECTORY_LOG_DIR="${TRAJECTORY_LOG_DIR:-gs://${USER}-trellis-europe-west4-dev/trajectories}"
...
# Topologies (256 chips Trainer 4x8x8, 8x 16-chip Rollout slices)
export TRAINER_JOBSET_YAML="jobset.pathways.yaml"
export TRAINER_TPU_SLICE="tpuv5:4x8x8"
...
export ROLLOUT_TPU_SLICE="tpuv5:2x2x4"
...
export ONEHOT_MOE_PERMUTE_THRESHOLD=${ONEHOT_MOE_PERMUTE_THRESHOLD:-131072}
...
# Extra environment variables for trainer, orchestrator, and rollout workers
...
export MAX_SEQ_TOKEN_PER_TPU=65536
