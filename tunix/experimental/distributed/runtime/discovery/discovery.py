...
class ClientRegistration:
  """Data class holding registered client node information."""
...
class _OneShotServicer(pb2_grpc.DiscoveryServiceServicer):
  ...
  def Register(
    ...
    if self._callback is not None:
      try:
        ...
        logging.exception("Error in one-shot discovery server callback: %s", e)
  ...
  def Connect(
    ...
    context.abort(
        ...
        "Discovery server is running in one-shot register mode. Use Register"
  ...
  def Heartbeat(
    ...
    context.abort(
        grpc.StatusCode.UNIMPLEMENTED,
        "Discovery server is running in one-shot register mode. Heartbeats not"
        " supported.",
    )


class _PersistentServicer(pb2_grpc.DiscoveryServiceServicer):
  """gRPC servicer for managed persistent client sessions with heartbeats and lease eviction."""
  ...
  def __init__(
    ...
    self._registered_clients: dict[str, ClientRegistration] = {}
  ...
  def Register(
    ...
    context.abort(
        grpc.StatusCode.UNIMPLEMENTED,
        "Discovery server is running in persistent connect mode. Use Connect"
  ...
  def Connect(
    ...
    with self._lock:
      is_reconnect = client_id in self._registered_clients
      self._registered_clients[client_id] = ClientRegistration(
          client_id=client_id,
  ...
  def Heartbeat(
    ...
    with self._lock:
      if (
          ...
          or request.client_id not in self._registered_clients
      ):
      ...
      self._registered_clients[request.client_id].last_seen = time.time()
      return pb2.HeartbeatResponse(
  ...
  def _run_evictor_loop(self) -> None:
    while not self._stop_event.is_set():
      ...
      now = time.time()
      evicted: list[ClientRegistration] = []

      with self._lock:
        timeout_threshold = 3 * self._heartbeat_sec
        stale_ids = [
            cid
            for cid, reg in self._registered_clients.items()
            if now - reg.last_seen > timeout_threshold
        ]
        for cid in stale_ids:
          evicted.append(self._registered_clients.pop(cid))
...
class DiscoveryServer:
  ...
  def __init__(self, heartbeat_sec: int = 5) -> None:
    ...
    self._servicer: _OneShotServicer | _PersistentServicer | None = None
  ...
  def on_register(self, callback: Callable[[str, int, bytes], None]) -> None:
    """Configures one-shot registration mode for the server."""
  ...
  def on_connect(
    ...
    """Configures persistent connection mode for the server."""
  ...
  def start(
    ...
    if self._mode == "connect":
      self._servicer = _PersistentServicer(
          on_client_connected=self._on_client_connected,
          on_client_disconnected=self._on_client_disconnected,
          heartbeat_sec=heartbeat_sec or self._heartbeat_sec,
      )
    else:
      self._servicer = _OneShotServicer(self._on_client_register)
  ...
  def stop(self, timeout: float | None = None) -> None:
    ...
    if isinstance(self._servicer, _PersistentServicer):
