...
def main() -> None:
  ...
  parser.add_argument(
      ...
      help=(
          "Kueue WorkloadPriorityClass name (optional). Workloads default to"
          " priority 0 and are preempted by anything above them in the cohort,"
          " which on a shared cluster means a multi-hour run can lose its"
          " chips at any point."
      ),
  )
