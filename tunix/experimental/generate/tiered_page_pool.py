...
class TieredPagePoolConfig:
  ...
  def _make_pool(
    ...
    """Creates a page pool."""
    if is_host and sharding is not None:
      raise ValueError("Cannot shard pages on host.")
  ...
  def create_manager(self) -> "TieredPagePoolManager":
    ...
    device_pool = self._make_pool(
        num_pages=self.num_device_pages, sharding=self.sharding
    )

    host_pool = (
        self._make_pool(num_pages=self.num_host_pages, is_host=True)
        if self.num_host_pages > 0
        else None
    )
...
class TieredPagePoolManager:
  ...
  def __init__(
      self,
      device_pool: _PartitionedPagePool,
      host_pool: _PartitionedPagePool | None,  # Host pool is None if host offloading is disabled.
  ):
  ...
  @property
  def physical_device_pages(self) -> dict[str, jax.Array | np.ndarray]:
  ...
  @property
  def _device_sharding(self) -> jax.sharding.Sharding | None:
  ...
  def offload(self, page_ids: Sequence[int]) -> None:
    ...
    # the engine step.
    device_slices = _get_device_slices(self._device_pool.partition_pages, device_indices_arr)
