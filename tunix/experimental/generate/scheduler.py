...
class Scheduler:
  ...
  def __init__(
    ...
    self._last_num_decode_tokens: int = 0
  ...
  def last_num_generated_tokens(self) -> int:
    """The number of tokens actually generated in the last update_from_output."""
    return self._last_num_generated_tokens

  @property
  def last_num_decode_tokens(self) -> int:
    """The number of decode tokens actually generated in the last update_from_output."""
    return self._last_num_decode_tokens
  ...
  def update_from_output(
    ...
    A request terminates on EOS only if it carries its own `eos_token_ids`;
    there is no scheduler-wide default. Callers that want EOS termination set
    it when they build the request.
    """
    completed_reqs = []
    surviving_reqs = []
    self._last_num_generated_tokens = 0
    self._last_num_decode_tokens = 0
    ...
    for i, req in enumerate(self._scheduled_requests):
      ...
      req_eos_set = req.eos_token_ids

      valid_new_tokens = []
      for new_token in req_tokens:
        token_id = int(new_token)
        if req_eos_set and token_id in req_eos_set:
          terminated = True
          break
      ...
      max_tokens = (
          ...
          else self._config.max_tokens_to_generate
      )

      n_overflow = n_generated - max_tokens
      if n_overflow >= 0:
      ...
      # --- PERF LOGGING ---
      self._last_num_generated_tokens += num_added
      if req.is_decode:
        self._last_num_decode_tokens += num_added
      # --- END PERF LOGGING ---
    ...
    surviving_ids = {req.request_id for req in surviving_reqs}
    self._running_requests = collections.deque(
        req for req in self._running_requests if req.request_id in surviving_ids
    )
