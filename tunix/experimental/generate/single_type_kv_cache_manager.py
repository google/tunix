...
"""Manager for a group of KV caches with identical geometry and attention type."""
...
class SingleTypeKVCacheManager:
  """Coordinates KV cache page management for a group of caches with identical geometry and attention type.
  """
  ...
  def __init__(
    ...
    self._page_manager = dataclasses.replace(
        page_pool_config,
        num_tpu_pages=(page_pool_config.num_tpu_pages),
    ).create_manager()
  ...
  @property
  def num_pages_in_window(self) -> int:
    if self._window_size is None:
      return 0
    # Add 1 to account for the first page being partially filled.
  ...
  def _release_out_of_window(self, request_id: str, num_completed_tokens: int):
  ...
  def _cache_full_pages(
    ...
    for i in range(n_hashed - 1, -1, -1):
      ...
      if cached_page is None:
        ...
        # referenced. Free `page` and reuse the cached page.
        pages[i] = cached_page
        ...
        # immediately.
        self._prefix_hash_to_page[prefix_hash] = page
        self._release_page(cached_page)
      else:
        raise ValueError(
  ...
  def _longest_cache_hit_full_attention(
    ...
    """Finds the longest kv cache hit for a group full attention layers."""
  ...
  def _longest_cache_hit_local_attention(
    ...
    w_start = 0
    while w_end >= 0:
      w_start = max(0, w_end - self.num_pages_in_window + 1)
  ...
  def has_sufficient_space(
    ...
    """Checks whether there is sufficient space to allocate `num_tokens` token slots for the request.
  ...
  def _allocate_tpu_pages(
    ...
    """Allocates n_tpu_pages TPU pages for a request, and binds them to the request.
  ...
  def allocate_slots(
    ...
    """Allocate new pages for the request to give it at least num_tokens free token slots, and fetches any CPU computed pages onto the TPU.

    Args:
      request_id: The ID of the request to allocate pages for.
      num_tokens: The number of tokens to be computed in the next step.
      num_completed_tokens: The number of completed tokens for the request.
      computed_pages: The list of computed pages for the request. Entries should
    ...
    # in a single step.
    if num_tokens > self._window_size:
      raise ValueError(
          "Cannot allocate more than window size tokens in a single step."
      )

    # Must be computed before `_bind_pages`, which folds `computed_pages` into
  ...
  def reset_kv_caches(self) -> None:
    ...
    self._prefix_hash_to_page.clear()
    self._unreferenced_tpu_pages.clear()
    self._unreferenced_cpu_pages.clear()
    self._request_to_pages.clear()
