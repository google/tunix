...
def copy_physical_pages(
  ...
  safe_src_idxs = jnp.where(valid_mask, src_idxs, 0)
  src_slice = src_pages[safe_src_idxs]
  sentinel_idx = dst_pages.shape[0]
  safe_dst_idxs = jnp.where(valid_mask, dst_idxs, sentinel_idx)

  dst_pages = dst_pages.at[safe_dst_idxs].set(src_slice, mode='drop')
