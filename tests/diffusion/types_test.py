...
class DiffusionTokenBatchTest(absltest.TestCase):
  ...
  def test_requires_nonempty_batch_major_array_inputs(self):
    ...
    with self.assertRaisesRegex(
        TypeError, "must be a JAX/NumPy array or ShapeDtypeStruct"
    ):
  ...
  def test_accepts_nested_pytree_model_inputs(self):
    batch = _batch(
        model_inputs=(
            jnp.ones((2, 3), dtype=jnp.int32),
            {"noise_levels": jnp.ones((2,), dtype=jnp.float32)},
        )
    )

    self.assertIsInstance(batch.model_inputs, tuple)

  def test_accepts_eval_shape_abstract_arrays(self):
    abstract_batch = jax.eval_shape(lambda batch: batch.validate(), _batch())
    validated_batch = abstract_batch.validate()
    abstract_logits = jax.ShapeDtypeStruct((2, 3, 8), jnp.float32)

    self.assertIsInstance(validated_batch.target_ids, jax.ShapeDtypeStruct)
    self.assertIs(
        diffusion.validate_diffusion_logits(validated_batch, abstract_logits),
        abstract_logits,
    )
