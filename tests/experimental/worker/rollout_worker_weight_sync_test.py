...
Silent failures this file locks out:
  ...
  - get_weight_sync_status answering from the sampler's internal sub-state
...
class FakeSampler:
  ...
  Test knobs:
    ...
    round_override: if set, get_weight_sync_status returns it verbatim,
      simulating a sampler sub-state that disagrees with the worker.
    round_error: if set, get_weight_sync_status raises it (status RPC
      failure).
  """
  ...
  async def get_weight_sync_status(self, **kw):
...
class RolloutWorkerWeightSyncTest(absltest.TestCase):
  ...
  async def _pre_leaves_worker_syncing_not_ready(self):
    ...
    report = await worker.get_weight_sync_status()
  ...
  def test_stop_before_pre_does_not_consume_round_key(self):
    asyncio.run(
        asyncio.wait_for(
            self._stop_before_pre_does_not_consume_round_key(), 30
        )
    )

  async def _stop_before_pre_does_not_consume_round_key(self):
    worker = self._make_worker(FakeSampler())
    worker.stop()
    with self.assertRaisesRegex(RuntimeError, "STOPPED"):
      await worker.pre_weight_sync(ws_req(uuid=1))
    # Stop won before the round claim. Advancing the high-water mark here
    # would make a later legitimate round look stale even though pre never ran.
    report = await worker.get_weight_sync_status()
    self.assertIsNone(report["req_id"])
    self.assertEqual(report["uuid"], -1)
    self.assertEqual(report["phase"], "idle")
    self.assertEqual(worker.state, WorkerState.STOPPED)

  def test_pre_refused_before_serving_does_not_consume_round_key(self):
    asyncio.run(
        asyncio.wait_for(
            self._pre_refused_before_serving_does_not_consume_round_key(), 30
        )
    )

  async def _pre_refused_before_serving_does_not_consume_round_key(self):
    # A worker that never finished initialization must refuse to quiesce:
    # letting pre through would let the post/abort terminal later mark it
    # READY without initialize() ever completing. And the refusal must not
    # consume the round key, or the retry after initialization would be
    # rejected as stale for a pre that never ran.
    worker = self._make_worker(FakeSampler())
    # Bypasses the validating setter: the point is to pin a worker CAUGHT in
    # INITIALIZING, and READY -> INITIALIZING is (rightly) not a legal
    # transition. Without pre's own guard, admit would consume the round key
    # and only THEN would the SYNCING transition raise -- key gone, pre
    # never ran.
    worker._state = WorkerState.INITIALIZING  # pylint: disable=protected-access
    ...
    # The SAME key succeeds once the worker is serving: proof it was not
  ...
  def test_pre_supersede_while_syncing_still_allowed(self):
    asyncio.run(
        asyncio.wait_for(self._pre_supersede_while_syncing_still_allowed(), 30)
    )

  async def _pre_supersede_while_syncing_still_allowed(self):
    # SYNCING is deliberately in pre's allowlist: the tracker documents that
    # a NEWER round's pre supersedes an abandoned one, and a crash-between-
    # do-and-record retry of the same round also arrives while SYNCING.
    # Tightening the guard to READY-only would break both.
    worker = self._make_worker(FakeSampler())
    await worker.pre_weight_sync(ws_req(uuid=1))
    self.assertEqual(worker.state, WorkerState.SYNCING)
    await worker.pre_weight_sync(ws_req(uuid=2))
    report = await worker.get_weight_sync_status()
  ...
  async def _stop_during_weight_then_post_rejected(self):
    ...
    with self.assertRaisesRegex(RuntimeError, "not serving"):
      await worker.post_weight_sync(req)
    self.assertNotIn("post", sampler.calls)
    report = await worker.get_weight_sync_status()
    self.assertEqual(report["phase"], "h2d_done")
  ...
  def test_stop_during_abort_reconciles_needs_restart(self):
    asyncio.run(
        asyncio.wait_for(
            self._stop_during_abort_reconciles_needs_restart(), 30
        )
    )
  ...
  def test_post_resume_failure_stays_syncing_and_uncommitted(self):
    asyncio.run(
        asyncio.wait_for(
            self._post_resume_failure_stays_syncing_and_uncommitted(), 30
        )
    )

  async def _post_resume_failure_stays_syncing_and_uncommitted(self):
    worker = self._make_worker(FakeSampler())
    req = ws_req(uuid=1)
    await worker.pre_weight_sync(req)
    await worker.weight_sync(req)
    with mock.patch.object(
        worker.manager, "resume_all", side_effect=RuntimeError("resume failed")
    ):
      with self.assertRaisesRegex(RuntimeError, "resume failed"):
        await worker.post_weight_sync(req)
    self.assertEqual(worker.state, WorkerState.SYNCING)
    self.assertFalse(worker.manager._admission_open.is_set())
    report = await worker.get_weight_sync_status()
    self.assertEqual(report["phase"], "h2d_done")

  def test_abort_resume_failure_stays_syncing_and_unaborted(self):
    asyncio.run(
        asyncio.wait_for(
            self._abort_resume_failure_stays_syncing_and_unaborted(), 30
        )
    )

  async def _abort_resume_failure_stays_syncing_and_unaborted(self):
    worker = self._make_worker(FakeSampler())
    req = ws_req(uuid=1)
    await worker.pre_weight_sync(req)
    with mock.patch.object(
  ...
  def test_stop_reentering_post_terminal_prevents_false_commit(self):
    asyncio.run(
        asyncio.wait_for(
            self._stop_reentering_post_terminal_prevents_false_commit(), 30
        )
    )

  async def _stop_reentering_post_terminal_prevents_false_commit(self):
    worker = self._make_worker(FakeSampler())
    req = ws_req(uuid=1)
    await worker.pre_weight_sync(req)
    await worker.weight_sync(req)
    with mock.patch.object(
        worker.manager, "resume_all", side_effect=worker.stop
    ):
      with self.assertRaisesRegex(RuntimeError, "not serving"):
        await worker.post_weight_sync(req)
    self.assertEqual(worker.state, WorkerState.STOPPED)
  ...
  def test_stop_reentering_abort_terminal_prevents_false_abort(self):
    asyncio.run(
        asyncio.wait_for(
            self._stop_reentering_abort_terminal_prevents_false_abort(), 30
        )
    )

  async def _stop_reentering_abort_terminal_prevents_false_abort(self):
    worker = self._make_worker(FakeSampler())
    req = ws_req(uuid=1)
    await worker.pre_weight_sync(req)
    with mock.patch.object(
  ...
  def test_same_uuid_different_req_id_raises(self):
    asyncio.run(
        asyncio.wait_for(self._same_uuid_different_req_id_raises(), 30)
    )
  ...
  def test_abort_confirmed_records_aborted(self):
    asyncio.run(
        asyncio.wait_for(self._abort_confirmed_records_aborted(), 30)
    )
  ...
  def test_abort_may_open_fresh_round_without_quiescing(self):
    asyncio.run(
        asyncio.wait_for(
            self._abort_may_open_fresh_round_without_quiescing(), 30
        )
    )

  async def _abort_may_open_fresh_round_without_quiescing(self):
    # Cancellation recovery may reach a worker before pre_weight_sync. CL4's
    # tracker therefore permits "aborted" to open an unseen round. The worker
    # is already READY in this path; a confirmed no-op rollback must close the
    # round as aborted rather than falsely requiring a restart.
    sampler = FakeSampler()
    worker = self._make_worker(sampler)
    req = ws_req(uuid=1)
    self.assertEqual(worker.state, WorkerState.READY)
    ...
    self.assertEqual(report["uuid"], 1)
    self.assertEqual(worker.state, WorkerState.READY)
    result = await self._generate(worker, "after-fresh-abort")
  ...
  def test_abort_idle_report_not_confirmed(self):
    asyncio.run(asyncio.wait_for(self._abort_idle_report_not_confirmed(), 30))

  async def _abort_idle_report_not_confirmed(self):
    sampler = FakeSampler()
    worker = self._make_worker(sampler)
    req = ws_req(uuid=1)
    await worker.pre_weight_sync(req)
    # "idle" with a MATCHING round key is the one report an unfinished abort
    # produces: the round was admitted and nothing completed. It is the
    # absence of confirmation, not a form of it, so it must reconcile as
    # needs-restart -- never as a confirmed rollback.
    sampler.round_override = {"req_id": "r1", "uuid": 1, "phase": "idle"}
    with self.assertRaisesRegex(RuntimeError, "not positively confirmed"):
      await worker.abort_weight_sync(req)
    report = await worker.get_weight_sync_status()
    self.assertEqual(report["phase"], "prepared")
  ...
  def test_abort_unknown_report_format_not_confirmed(self):
    asyncio.run(
        asyncio.wait_for(
            self._abort_unknown_report_format_not_confirmed(), 30
        )
    )
  ...
  def test_get_status_is_worker_authoritative(self):
    asyncio.run(
        asyncio.wait_for(self._get_status_is_worker_authoritative(), 30)
    )

  async def _get_status_is_worker_authoritative(self):
    sampler = FakeSampler()
    worker = self._make_worker(sampler)
