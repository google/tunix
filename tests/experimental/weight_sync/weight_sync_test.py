...
class WeightSyncE2ETest(absltest.TestCase):
  ...
  def test_raiden_mode_multiprocess_weight_sync_with_vanilla_sampler(self):
    ...
    # 3. Verify destination metadata and source work units match
    rollout_parent_conn.send(("bind_weight_sync",))
    status, _ = rollout_parent_conn.recv()
    self.assertEqual(status, "ok")

    rollout_parent_conn.send(("get_weight_sync_metadata",))
    status, dst_meta = rollout_parent_conn.recv()
    self.assertEqual(status, "ok")
    dst_names = {var.name for wu in dst_meta for var in wu.variables}

    self.assertIn("['emb']['embedding'].value", dst_names)

    trainer_parent_conn.send(("prepare_weight_sync", None))
    status, src_work_units = trainer_parent_conn.recv()
    self.assertEqual(status, "ok")
    src_names = {var.name for var in src_work_units[0].variables}

    for var_name in dst_names:
      self.assertIn(
          var_name,
          src_names,
          msg=f"Destination variable {var_name} missing from source!",
      )

    # 4. Register remote workers in orchestrator registry
    ...
    # 5. Run real RaidenHandler and WeightSyncCoordinator in main process
  ...
  def test_raiden_mode_multiprocess_weight_sync_with_vllm_sampler(self):
    ...
    # 3. Verify destination metadata and converted source work units match
    rollout_parent_conn.send(("bind_weight_sync",))
    status, _ = rollout_parent_conn.recv()
    self.assertEqual(status, "ok")
    ...
    self.assertIn("['model']['embed_tokens']['embedding'].value", dst_names)
    self.assertIn("['model']['layers'][0]['self_attn']['query']['kernel'].value", dst_names)
    self.assertIn("['model']['layers'][0]['mlp']['gate_proj']['kernel'].value", dst_names)
    ...
    for var_name in dst_names:
      self.assertIn(
          var_name,
          src_names,
          msg=f"Destination variable {var_name} missing from converted source!",
      )
