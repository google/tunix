...
class TrainerWorkerMeshExecutionContextTest(absltest.TestCase):
  ...
  def test_prepare_weight_sync_never_drains_checkpoint(self):
    ...
    Waiting on the in-flight Orbax write here would serialise the two and add
    latency to every step, so prepare_weight_sync must not block on it.
    """
...
class RunTrainerNodeMainAndShutdownTest(absltest.TestCase):
  ...
  def test_parse_args_defaults_and_custom(self):
    ...
    self.assertEqual(args.flash_attention_block_size, 1024)
