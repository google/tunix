...
if [[ "${DRY_RUN}" == "1" || "${DRY_RUN}" == "true" ]]; then
  ...
  COMPILED_BIN=$(ls "${SCRIPT_DIR}"/../../../../../../*-bin/third_party/py/tunix/experimental/examples/deepswe_mlperf/sandbox_k8s_e2e_test 2>/dev/null | head -n 1 || true)
  if [[ -n "${PYTHON_BIN:-}" ]]; then
    ${PYTHON_BIN} -m tunix.experimental.examples.deepswe_mlperf.sandbox_k8s_e2e_test \
      --dry_run \
      --scaffold="${SCAFFOLD}" \
    ...
    python3 -m tunix.experimental.examples.deepswe_mlperf.sandbox_k8s_e2e_test \
      --dry_run \
      --scaffold="${SCAFFOLD}" \
      --batch_size="${BATCH_SIZE}" \
      --num_generations="${NUM_GENERATIONS}" \
...
kubectl create configmap "${CONFIGMAP_NAME}" \
  --namespace="${NAMESPACE}" \
  --from-file=sandbox_utils.py="${SCRIPT_DIR}/deepswe_core/sandbox_utils.py" \
  --from-file=swe_env.py="${SCRIPT_DIR}/deepswe_core/swe_env.py" \
  --from-file=openhands_utils.py="${SCRIPT_DIR}/deepswe_core/openhands_utils.py" \
  --from-file=template.py="${SCRIPT_DIR}/deepswe_core/template.py" \
    ...
    spec:
      ...
      - name: e2e-tester
        ...
        - |
          ...
          mkdir -p /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core
          ...
          cp /e2e_code/sandbox_utils.py /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core/
          cp /e2e_code/swe_env.py /app/examples/deepswe/
          cp -r /app/examples/deepswe/* /app/tunix/oss/examples/deepswe/ 2>/dev/null || true
          cp -r /app/examples/deepswe/* /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core/ 2>/dev/null || true
          cp /e2e_code/swe_env.py /app/tunix/oss/examples/deepswe/
          cp /e2e_code/swe_env.py /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core/
          ...
          cp /e2e_code/openhands_utils.py /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core/ 2>/dev/null || true
          ...
          cp /e2e_code/template.py /app/tunix/experimental/examples/deepswe_mlperf/deepswe_core/ 2>/dev/null || true
          cp /e2e_code/deepswe.py /app/tunix/experimental/examples/deepswe_mlperf/
          cp /e2e_code/sandbox_k8s_e2e_test.py /app/tunix/experimental/examples/deepswe_mlperf/
          cp /e2e_code/*.py /app/

          export PYTHONPATH="/app:/e2e_code:\${PYTHONPATH:-}"

          python3 /app/tunix/experimental/examples/deepswe_mlperf/sandbox_k8s_e2e_test.py \
            --run_as_job \
