...
class _FakeOrchestrator:
  ...
  def train_step(self, chunks, eval_ds=None, skip_jit=False):
    self.calls.append(("train_step", len(chunks), eval_ds, skip_jit))
...
class GRPOLoopTest(unittest.TestCase):
  ...
  def test_train_step_runs_formal_grpo_choreography(self):
    ...
    np.testing.assert_allclose(
        ...
        np.array([[0.0, 1.0, 0.0], [1.0, 2.0, 0.0], [2.0, 3.0, 0.0]]),
    )
    np.testing.assert_allclose(
