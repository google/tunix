...
"""End-to-end test: the unchanged agentic learner runs on OrchestratorRLEngine.
...
class OrchestratorRlEngineIntegrationTest(absltest.TestCase):
  ...
  def _build_base_engine(self, max_steps):
  ...
  def test_learner_trains_through_orchestrator_cluster_with_handle(self):
    ...
    with mock.patch.object(handle, "update", wraps=handle.update) as spy_update:
      learner.train(self._train_ds())

    # Training was driven through the trainer handle via the cluster primitive.
    self.assertGreater(spy_update.call_count, 0)
