...
class ClusterOrchestratorTest(absltest.TestCase):
  ...
  def test_create_engine(self):
    ...
    # register_worker_handle now also registers the handle with the
    # WorkerRegistry, which calls handle.info() to learn its id/roles. A bare
    # spec'd mock auto-mocks info() too (even though it's concrete on the real
    # base class), so wire it to read back the `_info` register_worker_handle
    # sets -- matching ActorHandle.info()'s real behavior.
  ...
  def test_bring_up_and_shutdown_remote_worker_handles(self):
    ...
    # Remote worker lifecycle now runs through LifecycleDriver against the
    # registry (register_worker_handle registers the handle too), not through
    # ClusterOrchestrator submitting to handles directly -- use a real driver
    # so this test exercises that path instead of asserting on a mock that
    # nothing actually drives.
    registry = worker_registry.WorkerRegistry()
  ...
  def test_shutdown_survives_a_wedged_worker(self):
    # NOTE: LifecycleDriver.shutdown() (tunix/experimental/orchestrator/
    # lifecycle.py) stops every registered worker concurrently and is
    # best-effort about *failures* (test_shutdown_is_best_effort_and_
    # aggregates_failures in lifecycle_test.py covers that), but unlike the
    # old ClusterOrchestrator._shutdown_remote_workers() it removed, it has
    # no hard deadline: shutdown() still waits on every future via
    # as_completed, so a worker whose stop() call truly hangs (not just
    # raises) blocks the whole shutdown(). This test only verifies a *slow*
    # (not infinitely hung) worker doesn't prevent others from being asked to
    # stop -- it does not assert a bounded-timeout guarantee, because the
    # current implementation doesn't provide one.
    from tunix.experimental.orchestrator import lifecycle
    from tunix.experimental.worker import remote_execution
