...
def _completion_text(traj, metadata):
  item = mock.Mock()
  item.traj = traj
  return gsm8k._completion_text(item, metadata)  # pylint: disable=protected-access
...
class Gsm8kCompletionTextTest(absltest.TestCase):
  """Tests for extracting the scored completion from a trajectory."""

  def test_returns_the_last_assistant_turn(self):
    messages = [
        {"role": "user", "content": "q?"},
        {"role": "assistant", "content": r"so \boxed{13}"},
    ]
    self.assertEqual(
        _completion_text({"conversation_text": messages}, {"text": messages}),
        r"so \boxed{13}",
    )

  def test_returns_empty_when_there_is_no_assistant_turn(self):
    messages = [{"role": "user", "content": "q?"}]
    self.assertEqual(
        _completion_text({"conversation_text": messages}, {"text": messages}), ""
    )

  def test_falls_back_to_metadata_text_when_it_is_a_string(self):
    self.assertEqual(
        _completion_text(None, {"text": "plain string"}), "plain string"
    )
