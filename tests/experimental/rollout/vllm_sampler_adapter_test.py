...
class VllmSamplerAdapterTest(absltest.TestCase):
  ...
  def test_weight_sync_delegations(self):
    self.mock_sampler_instance.get_weight_sync_metadata.return_value = {
        "sharding": "2x1"
    }
    self.mock_sampler_instance.pre_weight_sync.return_value = True
    self.mock_sampler_instance.weight_sync.return_value = True
    self.mock_sampler_instance.post_weight_sync.return_value = True
    self.mock_sampler_instance.get_transfer_status.return_value = "SUCCESS"

    meta = asyncio.run(self.sampler_adapter.get_weight_sync_metadata())
    self.assertEqual(meta, {"sharding": "2x1"})

    sync_req = base_sampler_lib.WeightSyncRequest(policy_version=1)
    ...
    self.assertTrue(res_post)

    status = asyncio.run(self.sampler_adapter.get_transfer_status("req_123"))
