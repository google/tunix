...
class FakeSampler:
  ...
  async def get_weight_sync_round(self, **kwargs):
    return {}
...
class LegacySampler:
  ...
  nothing about Raiden weight sync.
  """
...
class ManagerWeightSyncTest(absltest.TestCase):
  ...
  def test_cancel_all_is_permanent(self):
    asyncio.run(asyncio.wait_for(self._cancel_all_is_permanent(), timeout=30))
  ...
  async def _legacy_sampler_without_weight_sync_capability_constructs(self):
    ...
    # carrying them in Sampler would stop every sampler that predates Raiden
    # weight sync from constructing a manager at all.
  ...
  async def _weight_sync_on_incapable_sampler_raises_typeerror(self):
    ...
    # with bind but no abort/get_round cannot be driven through a round.
    with self.assertRaisesRegex(TypeError, "bind/abort/get_round"):
    ...
    with self.assertRaisesRegex(TypeError, "bind/abort/get_round"):
      await manager.abort_weight_sync(_ws_req(1))
    with self.assertRaisesRegex(TypeError, "bind/abort/get_round"):
      await manager.get_weight_sync_round()
