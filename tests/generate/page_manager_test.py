...
class PageManagerTest(parameterized.TestCase):
  ...
  def test_allocate_twice(self):
    ...
    pm = pm.assign(jnp.arange(5), first_allocated_idxs, jnp.array([0, 1, 2, 0, 5]))
    pm, second_allocated_idxs = pm.allocate(jnp.array([1, 1, 0, 0, 2]))
    pm = pm.assign(jnp.arange(5), second_allocated_idxs, jnp.array([1, 1, 0, 0, 2]))
  ...
  def test_release(self):
    ...
    pm = config.init()
    q_lens = jnp.array([0, 1, 2, 0, 5])
    pm, alloc = pm.allocate(q_lens)
    pm = pm.assign(jnp.arange(5), alloc, q_lens)
  ...
  def test_insert_values_no_subshape(self):
    ...
    pm, alloc = pm.allocate(jnp.array([1, 2, 0]))
    pm = pm.assign(jnp.arange(3), alloc, jnp.array([1, 2, 0]))
    
    init_values = jnp.array([42, 43, 44])
    pm = pm.load_values(init_values, jnp.array([1, 2, 0]))

    pm, alloc2 = pm.allocate(jnp.array([1, 1, 1]))
    pm = pm.assign(jnp.arange(3), alloc2, jnp.array([1, 1, 1]))
  ...
  def test_offload_and_load(self):
    ...
    # Load back
    pm, new_tpu_page_idxs = pm.load(pages_to_offload, jnp.array([0, 1]))
