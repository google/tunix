...
class VLProcessorTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.processor = vl_processor.VLProcessor(config)

  def _create_dummy_image_file(self, filename='test_image.png'):
    img_array = np.zeros((100, 100, 3), dtype=np.uint8)
    img = Image.fromarray(img_array)
    temp_file = self.create_tempfile(filename)
    img.save(temp_file.full_path)
