...
import numpy as np
from tunix.distillation.feature_extraction import feature_shape


class ReduceArrayToTargetShapeTest(parameterized.TestCase):
  ...
  def test_valid_reductions_shape(
    ...
    result = feature_shape.reduce_array_to_target_shape(
  ...
  def test_same_padding_count_include_pad_effect_on_values(self):
    ...
    result_true = feature_shape.reduce_array_to_target_shape(
    ...
    result_false = feature_shape.reduce_array_to_target_shape(
        input_array=input_arr,
        target_shape=target_shape,
        padding_mode=feature_shape.PaddingMode.SAME,
        count_include_pad_for_same_padding=False,
  ...
  def test_invalid_input_configurations_raise_value_error(
    ...
    with self.assertRaisesRegex(ValueError, error_msg_regex):
      feature_shape.reduce_array_to_target_shape(
