...
class FunctionRegistryTest(parameterized.TestCase):
  ...
  def test_register_duplicate_name_logs_warning(self):
    ...
    # We use assertLogs to catch the logging.warning call
    with self.assertLogs(level="WARNING") as cm:
      self.registry.register("policy_loss_fn", "my_func")(dummy_func_b)

    # Verify the specific warning message is in the output
    self.assertTrue(
        any(
    ...
    # Optional: Verify that the second function actually overwrote the first
    self.assertEqual(
