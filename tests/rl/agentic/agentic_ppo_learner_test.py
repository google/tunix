...
class AgenticPpoLearnerTest(parameterized.TestCase):
  ...
  def test_critic_model_required(self):
    ...
    with self.assertRaisesRegex(ValueError, "PPO requires a critic model"):
      agentic_ppo_learner.PPOLearner(
  ...
  def test_value_loss_fn(self):
    ...
    class MockValueModel(nnx.Module):
      ...
      def __call__(
        ...
        del kwargs
        return jnp.full(inputs.shape, 0.5, dtype=jnp.float32), None

    algo_config = agentic_ppo_learner.PPOConfig()
    value_loss_fn = function_registry.get_value_loss_fn(
        algo_config.value_loss_fn
    )
    loss, aux = value_loss_fn(
        model=MockValueModel(rngs=nnx.Rngs(0)),
    ...
    self.assertIn("vpred_mean", aux)
    self.assertIn("vf_clipfrac", aux)

  def test_critic_weight_update_during_training(self):
    vocab = _mock_vocab()
    tokenizer = tokenizer_adapter.TokenizerAdapter(vocab)
    model = test_common.ToyTransformer(
        config=test_common.ModelConfig(vocab_size=vocab.GetPieceSize()),
    ...
    original_critic_variables = jax.tree.map(
        jnp.copy, nnx.state(value_model, nnx.Param)
    )
    ...
    train_ds = _dummy_dataset(MySource(repeat=5), batch_size=2)
    with mock.patch.object(
        rl_cluster, "generate", side_effect=self._mock_generate
    ):
      ppo_learner.train(train_ds)

    critic_variables = nnx.state(value_model, nnx.Param)
    jax.tree.map_with_path(
        test_common.assert_not_equal,
        original_critic_variables,
        critic_variables,
    )

  def test_process_results_values_and_returns(self):
    class MockTraj:
      ...
      def __init__(self, index):
        self.traj = {
            ...
            "old_logprobs": np.array([-0.1, -0.2, -0.3]),
            "policy_version": 0,
    ...
    with mock.patch.object(
        learner.rl_cluster,
        "get_values",
        return_value=jnp.ones((2, 11)),
        autospec=True,
    ) as mock_get_values:
      [train_example] = learner._process_results(trajectories)

    mock_get_values.assert_called_once()
    self.assertIsNotNone(train_example.returns)
    self.assertIsNotNone(train_example.old_values)
    chex.assert_shape(train_example.returns, (2, 10))
    chex.assert_shape(train_example.old_values, (2, 10))
