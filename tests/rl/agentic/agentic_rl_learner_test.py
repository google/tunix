...
class AgenticRLLearnerTest(parameterized.TestCase):
  ...
  def test_model_call_routes_signed_p45_pre_tokenized_prompt_exactly(self):
    learner = object.__new__(DummyLearner)
    learner.chat_parser = mock.Mock()
    learner.rl_cluster = mock.Mock()
    learner.rl_cluster.generate.return_value = mock.sentinel.rollout
    prompt_ids = np.asarray([151644, 28, 1725], dtype=np.int32)
    contract = agentic_rl_learner.token_continuity.FrozenLakeTokenContinuity(
        workload="p45",
        mode="exact",
        selector=agentic_rl_learner.token_continuity.P57_TOKEN_CONTINUITY_ENV,
    )
  ...
  def test_model_call_routes_signed_p57_m15_pre_tokenized_prompt_exactly(self):
    learner = object.__new__(DummyLearner)
    learner.chat_parser = mock.Mock()
    learner.rl_cluster = mock.Mock()
    learner.rl_cluster.generate.return_value = mock.sentinel.rollout
