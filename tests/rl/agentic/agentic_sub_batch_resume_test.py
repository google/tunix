...
class SubBatchInitResumeTest(absltest.TestCase):
  ...
  def test_empty_root_on_existing_run_warns_and_starts_fresh(self):
    ...
    learner._sb_mgr.close()
    # The first enable marked the root: the same empty root now means the
    # ledger was lost, and init stops.
    self.assertTrue(os.path.exists(
        f"{self.tmp}/sub_batch/{sub_batch_checkpoint.ENABLED_MARKER}"
    ))
    learner = _make_learner(
        mgr=None, k=2, train_steps=5, global_steps=5,
        checkpoint_manager_latest_step=5,
  ...
  def test_multi_host_rejected(self):
    ...
    with mock.patch.object(jax, "process_count", return_value=2):
      with self.assertRaisesRegex(ValueError, "multi-host"):
        learner._init_sub_batch_checkpointing()
...
def _orchestrator_items(gid, pairs):
  ...
  async def go():
    ...
    qm = group_queue_manager.GroupQueueManager(
        group_size=pairs, key_fn=lambda it: it.prompt_id
    )
