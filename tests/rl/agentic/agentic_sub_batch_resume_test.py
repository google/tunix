...
class SubBatchInitResumeTest(absltest.TestCase):
  ...
  def test_zero_baseline_missing_trainer_does_not_purge_surviving_ledger(self):
    mgr = _make_manager(f"{self.tmp}/sub_batch", k=1)
    mgr.mark_enabled(0)
    mgr.close()
    _plant(self.tmp, 1, 0, buffer=False)
    learner = _make_learner(mgr=None, k=1, checkpoint_manager_latest_step=None)
    learner._training_config.checkpoint_root_directory = self.tmp
    with self.assertRaisesRegex(
        sub_batch_checkpoint.SubBatchLedgerMissingError,
        "surviving ledger snapshot",
    ):
      learner._init_sub_batch_checkpointing()
    evidence = _make_manager(f"{self.tmp}/sub_batch", k=1)
    try:
      self.assertEqual(evidence._select_step(1), 1_000_000)
    finally:
      evidence.close()

  def test_marker_only_initial_save_retry(self):
    mgr = _make_manager(f"{self.tmp}/sub_batch", k=1)
    mgr.mark_enabled(0)
    mgr.close()
    learner = _make_learner(mgr=None, k=1, checkpoint_manager_latest_step=None)
  ...
  def test_initialization_error_survives_checkpoint_close_error(self):
    for latest, failing_method in ((None, "mark_enabled"), (0, "try_restore")):
      with self.subTest(failing_method):
        learner = _make_learner(
            mgr=None, k=1, checkpoint_manager_latest_step=latest
        )
        learner._training_config.checkpoint_root_directory = self.tmp
        manager = mock.Mock()
        error = sub_batch_checkpoint.SubBatchUnreadableError("bad ledger")
        getattr(manager, failing_method).side_effect = error
        manager.close.side_effect = OSError("close failed")
        with mock.patch.object(
            sub_batch_checkpoint, "SubBatchCheckpointManager",
...
class SubBatchTrainChunkTest(absltest.TestCase):
  ...
  def test_anticipated_snapshot_requires_trainer_durability(self):
    ...
    with self.assertRaisesRegex(RuntimeError, "durable trainer checkpoint"):
      learner._sb_snapshot([], step_complete=False, anticipated_apply=True)
...
class SubBatchResyncTest(absltest.TestCase):
  ...
  def test_failed_sync_can_retry_without_global_step_drift(self):
    learner, calls = self._learner(restored=True, disaggregated=True)
    original_sync = learner.rl_engine.sync_weights

    def failed_sync():
      original_sync()
      raise OSError("transfer failed")

    learner.rl_engine.sync_weights = failed_sync
    with self.assertRaisesRegex(OSError, "transfer failed"):
      learner._sb_resync_rollout_weights()
    self.assertTrue(learner._sb_restored_trainer_state)
    self.assertEqual(learner.rl_engine.global_steps, 5)
    learner.rl_engine.sync_weights = original_sync
    learner._sb_resync_rollout_weights()
    self.assertFalse(learner._sb_restored_trainer_state)
    self.assertEqual(learner.rl_engine.global_steps, 5)
    self.assertEqual(calls, ["sync", "sync"])

  def test_failed_anchor_pin_can_retry_after_reinjection(self):
    learner, _ = self._learner(
        restored=True, disaggregated=True, train_steps=3, anchor=1,
        mid_step=True,
    )
    learner._sb_pending_state = None
    learner._sb_resumed_mid_step = True
    pin = mock.Mock(side_effect=[OSError("pin failed"), None])
    learner.rl_engine.pin_behavior_policy = pin
...
class SubBatchQueueTest(absltest.TestCase):
  ...
  def test_undrained_identity_fifo_raises_before_extending_it(self):
    learner = _make_learner()
    first = agentic_rl_learner._SbItem("first", [(1, 0)])
    second = agentic_rl_learner._SbItem("second", [(2, 0)])
    batches = learner._sb_unwrap(iter([[first], [second]]))
    self.assertEqual(next(batches), ["first"])
    with self.assertRaisesRegex(RuntimeError, "FIFO not drained"):
      next(batches)
    self.assertEqual(list(learner._sb_item_fifo), [(1, 0)])
