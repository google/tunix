...
class SubBatchInitResumeTest(absltest.TestCase):
  ...
  def test_abandoned_step_is_reported_loudly(self):
    ...
    of step 0 is lost. The restore must say so at ERROR level and leave the
    window-0 evidence intact."""
    ...
    with self.assertLogs(level="ERROR") as logs:
      learner._init_sub_batch_checkpointing()
    self.assertTrue(
        any("no matching snapshot" in m for m in logs.output), logs.output
    )
    self.assertIsNone(learner._sb_pending_state)
    self.assertEqual(learner._sb_mgr._select_step(0), 1)
    learner._sb_mgr.close()
...
class SubBatchGuardsTest(absltest.TestCase):
  ...
  def test_packing_initializes_packed_mode_state(self):
    """Sequence packing + sub-batch is supported: init sets up the tag-based
    identity state instead of raising. Retention needs no packed-specific
    sizing: the default preservation policy counts windows, not snapshots."""
    tmp = tempfile.mkdtemp()
    try:
      learner = _make_learner(
      ...
      learner._training_config.checkpoint_root_directory = tmp
      learner._init_sub_batch_checkpointing()
      self.assertTrue(learner._sb_packed)
      self.assertEmpty(learner._sb_item_fifo)
      self.assertEmpty(learner._sb_pack_tags)
      self.assertIsInstance(
          learner._sb_mgr._options.preservation_policy,
