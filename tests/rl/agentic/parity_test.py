...
class ParityTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      # Note: VanillaRollout does not support `return_logprobs=True`.
      # Since AgenticGRPO requires logprobs from the rollout engine,
      # we only test num_iterations=1.
      dict(
          testcase_name='single_iter_mini1_micro1',
          name='single_iter_mini1_micro1',
          full_batch_size=4,
          num_generations=2,
          mini_batch_size=1,
      ...
      dict(
          testcase_name='single_iter_mini4_micro1',
          name='single_iter_mini4_micro1',
          full_batch_size=4,
          num_generations=2,
          mini_batch_size=4,
          train_micro_batch_size=1,
  ...
  def test_model_weights_parity(
    ...
    grpo_config2 = agentic_grpo_learner.GRPOConfig(
        ...
        system_prompt='',
        max_concurrency=1,
        max_response_length=max_generation_steps,
    )
      ...
      with self.subTest('Data Parity'):
        ...
        for i, (ex1, ex2) in enumerate(zip(examples1, examples2)):
          ...
          np.testing.assert_array_equal(
              ex1.prompt_mask,
              ex2.prompt_mask,
              err_msg=f'prompt_mask mismatch at index {i}',
          )
          np.testing.assert_array_equal(
              ex1.completion_ids,
              ex2.completion_ids,
          ...
          np.testing.assert_array_equal(
              ex1.completion_mask,
              ex2.completion_mask,
              err_msg=f'completion_mask mismatch at index {i}',
          )
          ...
          if ex1.ref_per_token_logps is not None:
            self.assertIsNotNone(
                ex2.ref_per_token_logps,
                msg=(
                    'ref_per_token_logps is None for Agentic but not Standard'
                    f' at index {i}'
                ),
            )
            np.testing.assert_allclose(
                ex1.ref_per_token_logps,
                ex2.ref_per_token_logps,
                atol=1e-5,
                err_msg=f'ref_per_token_logps mismatch at index {i}',
            )
          else:
            self.assertIsNone(
                ex2.ref_per_token_logps,
                msg=(
                    'ref_per_token_logps is not None for Agentic but is None'
                    f' for Standard at index {i}'
                ),
            )
          if ex1.old_per_token_logps is not None:
            self.assertIsNotNone(
                ex2.old_per_token_logps,
                msg=(
                    'old_per_token_logps is None for Agentic but not Standard'
                    f' at index {i}'
                ),
            )
            np.testing.assert_allclose(
                ex1.old_per_token_logps,
                ex2.old_per_token_logps,
                atol=1e-5,
                err_msg=f'old_per_token_logps mismatch at index {i}',
            )
          else:
            self.assertIsNone(
                ex2.old_per_token_logps,
                msg=(
                    'old_per_token_logps is not None for Agentic but is None'
                    f' for Standard at index {i}'
                ),
            )
