...
class SubBatchCheckpointTest(absltest.TestCase):
  ...
  def test_empty_root_first_enable_marks_then_lost_ledger_raises(self):
    """An entirely empty ledger root under a trainer at T>0 is a first
    enable exactly once: None with a WARNING, and the root gets the enable
    marker. The same situation with the marker present means the ledger
    was lost: SubBatchLedgerMissingError, nothing written. A fresh anchor
    (T=0) is silent either way."""
    marker = os.path.join(self.test_dir, sub_batch_checkpoint.ENABLED_MARKER)
    self.assertFalse(os.path.exists(marker))
    with self.assertLogs(level="WARNING") as logs:
      self.assertIsNone(self.mgr.try_restore(train_steps=2, grad_accum_steps=2))
    joined = "\n".join(logs.output)
    ...
    with self.assertRaisesRegex(
      ...
      self.mgr.try_restore(train_steps=2, grad_accum_steps=2)
    with self.assertRaises(AssertionError):  # no WARNING for the anchor
      with self.assertLogs(level="WARNING"):
        self.assertIsNone(
            self.mgr.try_restore(train_steps=0, grad_accum_steps=2)
        )

  def test_marker_does_not_disturb_step_enumeration(self):
    """The marker is a dotfile in the ledger root: orbax's step scan must
    ignore it, and a fresh manager on the root must see the marker."""
    ...
    try:
      ...
      self.assertTrue(fresh._marked_enabled())
    finally:
      fresh.close()
