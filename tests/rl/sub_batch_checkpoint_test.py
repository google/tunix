...
class SubBatchCheckpointTest(absltest.TestCase):
  ...
  def test_default_options_are_sub_batch_defaults_not_peft_defaults(self):
    ...
    try:
      ...
      self.assertEqual(policy.interval, 1)
  ...
  def test_step_complete_roundtrip(self):
    self._save(
        6, k=2, global_step=3, step_complete=True, omit_training_state=True
    )
    self.mgr.wait()
    st = self.mgr.try_restore(
        train_steps=3, grad_accum_steps=2
    )
    self.assertIsNotNone(st)
  ...
  def test_completed_snapshot_rejects_accumulator_at_save(self):
    self._save(3, k=2)
    self.mgr.wait()
    before = self._keys()
    with self.assertRaisesRegex(ValueError, "completed global step"):
      self._save(4, k=2, step_complete=True, training_state=_state())
    self.assertEqual(self._keys(), before)

  def test_completed_snapshot_with_buffer_raises_before_purge(self):
    self._save(3, k=2)
    self._save_boundary(self.mgr, 2)
    self.mgr.wait()
    key = KB + 1
    payload = self.mgr._checkpointer.load_checkpointables(
        key, abstract_checkpointables={
            "meta": None, "rollout": None,
            "state": {"training_state": _state()},
        }
    )
    payload["meta"]["step_complete"] = True
    self.mgr._save_checkpointables(
        key, payload, force=True, custom_metadata=None, overwrite=True
    )
    self.mgr.wait()
    before = self._keys()
    with self.assertRaisesRegex(
        sub_batch_checkpoint.SubBatchUnreadableError,
        "completed global step carries a gradient accumulator",
    ):
      self.mgr.try_restore(
    ...
    self.assertEqual(self._keys(), before)
    for step in before:
      self.assertTrue(os.path.isdir(os.path.join(self.test_dir, str(step))))
  ...
  def test_missing_required_meta_keeps_dead_lineage_on_disk(self):
    """Required metadata cannot be defaulted before purging dead lineage."""
    self._save(3, k=2, counts={(1, 0): 1}, omit_training_state=True)  # 1.1
    self._save_boundary(self.mgr, 2)  # 2.0
    self.mgr.wait()
