...
class SubBatchCheckpointTest(absltest.TestCase):
  ...
  def test_default_options_are_sub_batch_defaults_not_peft_defaults(self):
    ...
    Inheriting the base class blindly would hand this stream the PEFT
    defaults, whose ContinuousCheckpointingPolicy(180s) silently SKIPS most
    saves -- a skipped snapshot is a missing key the restore window can
    never select. This is the trap the dedicated resolver closes."""
    d = tempfile.mkdtemp()
    try:
  ...
  def test_run_options_carry_async_but_never_policies(self):
    ...
    try:
      run_options = SimpleNamespace(
          enable_async_checkpointing=False,
          save_decision_policy=SimpleNamespace(interval=1),  # NOT an orbax policy
  ...
  def test_preservation_policy_keeps_top_two_windows_whole(self):
    ...
    cks = [SimpleNamespace(step=st) for st in (
        0, 1,                       # parent 0: 2 snapshots
        1 * KB, 1 * KB + 1, 1 * KB + 17,  # parent 1: ragged, 3 snapshots
  ...
  def test_ragged_retention_end_to_end_through_real_manager(self):
    ...
    try:
      ...
      def save(t, local):
        mgr.save(
            t, local, iter_steps=t * 10 + local, global_step=t,
            grad_accum_steps=2, step_complete=False,
            completed_group_ids=[], trained_trajectory_counts={},
            active_group_trajectories=[], training_state=None,
        )
      for local in (0, 1):
        save(0, local)
  ...
  def test_ragged_window_selection_has_no_k_dependence(self):
    ...
    self.assertIsNotNone(st)
    self.assertEqual(
        st.sub_batch_key, 1 * sub_batch_checkpoint.KEY_BASE + 17
    )
  ...
  def test_non_roundtrippable_group_id_fails_at_save(self):
    ...
    self._save(
        1, k=1, counts={(np.int64(5), 0): 1}, training_state=state
    )
    self.mgr.wait()
    st = self.mgr.try_restore(
  ...
  def test_out_of_int64_metadata_int_round_trips_exactly(self):
    ...
    self._save(2, k=2, counts={(10, 0): 1}, items=[item],
               omit_training_state=True)
    self.mgr.wait()  # the save (incl. async finalize) must not raise
    st = self.mgr.try_restore(train_steps=1, grad_accum_steps=2)
    self.assertIsNotNone(st)
    self.assertEqual(
        st.active_group_trajectories[0].metadata["trace_id"], big
    )
  ...
  def test_zero_size_arrays_round_trip_with_dtype_and_shape(self):
    ...
    dict_item = agent_types.TrajectoryItem(
        group_id=1, pair_index=0, start_step=0,
        traj={
    ...
    self._save(2, k=2, items=[dict_item, dataclass_item],
               omit_training_state=True)
