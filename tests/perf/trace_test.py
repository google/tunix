...
def timeline_tostring(timeline: Timeline) -> str:
  ...
  timeline_id = timeline.id if not timeline.id.startswith("thread-") else "thread"
  return (
      f"{timeline_id}:"
...
def timelines_tostring(tracer: PerfTracer) -> list[str]:
  return [timeline_tostring(timeline) for timeline in tracer._get_timelines().values()]
...
class TracerTest(parameterized.TestCase):
  ...
  def test_host_ok(self, mock_perf_counter):
    ...
    self.assertListEqual(timelines_tostring(tracer), [
  ...
  def test_span_group_ok(self, mock_perf_counter):
    mock_perf_counter.side_effect = [0.0, 1.0, 2.0, 3.0, 5.0, 6.0, 7.0, 8.0, 9.0,10.0, 11.0, 12.0, 13.0]
    waitlist = mock_array()
    ...
    with tracer.span_group("outer_loop"):
      ...
      with tracer.span_group("inner_loop"):
        with tracer.span("step2", devices=["tpu1"]) as span:
          span.device_end(waitlist)
      with tracer.span("step3") as span:
        pass
