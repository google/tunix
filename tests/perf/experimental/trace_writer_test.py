...
class PerfettoTraceWriterTest(parameterized.TestCase):
  ...
  def test_write_timelines_grouping(self, mock_builder_cls):
    ...
    self.assertEqual(captured_packets[1].track_descriptor.uuid, 2000000)

    # The third packet should be host-1
    self.assertEqual(captured_packets[2].track_descriptor.name, "host-1")
    self.assertEqual(captured_packets[2].track_descriptor.parent_uuid, 1000000)

    # The fourth packet should be host-2
    self.assertEqual(captured_packets[3].track_descriptor.name, "host-2")
    self.assertEqual(captured_packets[3].track_descriptor.parent_uuid, 2000000)

    # The fifth packet should be tpu0, renamed to "actor - tpu0"
    self.assertEqual(
        captured_packets[4].track_descriptor.name, "Actor Cluster - tpu0"
    )
