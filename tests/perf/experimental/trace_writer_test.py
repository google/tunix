...
class PerfettoTraceWriterTest(parameterized.TestCase):
  ...
  def test_write_timelines_grouping(self, mock_builder_cls):
    ...
    self.assertEqual(captured_packets[0].track_descriptor.uuid, 1000000)

    # The second packet should be the "Host - Rollout threads" group
    self.assertEqual(
        captured_packets[1].track_descriptor.name, "Host - Rollout threads"
    )
    self.assertEqual(captured_packets[1].track_descriptor.uuid, 1000001)

    # The third packet should be "Actor Cluster - tpu0"
    self.assertEqual(
        captured_packets[2].track_descriptor.name, "Actor Cluster - tpu0"
    )

    # The fourth packet should be host-1
    self.assertEqual(captured_packets[3].track_descriptor.name, "host-1")
    self.assertEqual(captured_packets[3].track_descriptor.parent_uuid, 1000000)

    # The fifth packet should be host-2
    self.assertEqual(captured_packets[4].track_descriptor.name, "host-2")
    self.assertEqual(captured_packets[4].track_descriptor.parent_uuid, 1000001)

    # The sixth packet should be tpu0
    self.assertEqual(captured_packets[5].track_descriptor.name, "tpu0")
    self.assertEqual(
        captured_packets[5].track_descriptor.parent_uuid,
        captured_packets[2].track_descriptor.uuid,
    )
