...
class PerfTracerTest(unittest.TestCase):
  ...
  def setUp(self):
    self.mock_async_wait_patcher = mock.patch(
        "tunix.perf.experimental.timeline._async_wait"
    )
  ...
  def test_tracer_with_multiple_devices(self):
    ...
    d2 = MockDevice("tpu", 1)
    t = tracer.PerfTracer(
        devices=[d1, d2], collect_on_first_device_per_mesh=False
    )
  ...
  def test_tracer_collect_on_one_device(self):
    d1 = MockDevice("tpu", 0)
    d2 = MockDevice("tpu", 1)
    # Default collect_on_one_device_per_mesh=True
    ...
    # Only first device should have span
    self.assertEqual(len(tl1.spans), 1)
    self.assertEqual(tl1.spans[0].name, "multi_dev_op")
    self.assertEqual(len(tl2.spans), 0)
  ...
  def test_synchronize_waits_for_pending_spans(self):
    ...
    with mock.patch("tunix.perf.experimental.tracer._synchronize_devices"):
      ...
      with mock.patch(
          "tunix.perf.experimental.timeline._async_wait", side_effect=controller
      ):
