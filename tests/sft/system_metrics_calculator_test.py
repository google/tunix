...
# A simple model and train_step for testing `measure_tflops_per_step`.
class _FakeModel(nnx.Module):
  """A simple model for testing purposes."""

  def __init__(self, *, rngs: nnx.Rngs):
    self.linear = nnx.Linear(10, 2, rngs=rngs)
...
class SystemMetricsCalculatorTest(absltest.TestCase):
  ...
  def setUp(self):
    super().setUp()
    self.model = _FakeModel(rngs=nnx.Rngs(0))
  ...
  def test_measure_tflops_per_step_success(self):
    ...
    self.assertGreater(tflops_per_step, 0)
