...
def _score_fn(
  ...
  assert isinstance(model, _LogitModel)
...
class DiffusionLossTest(absltest.TestCase):
  ...
  def test_configure_diffusion_sft_wires_trainer(self):
    ...
    if isinstance(aux, sft_utils.LossOutput):
      self.assertEmpty(aux.aux_metrics)
    else:
      self.assertIsInstance(aux, dict)
      self.assertEmpty(aux)
    self.assertGreater(float(np.asarray(grad_norm)), 0.0)
  ...
  def test_uses_target_aligned_logits_without_shifting(self):
    ...
    expected_token_losses = optax.softmax_cross_entropy_with_integer_labels(
        logits, jnp.asarray(batch.target_ids)
    )
    expected_sum = jnp.sum(
        expected_token_losses * jnp.asarray(batch.loss_weights),
        dtype=jnp.float32,
    )
