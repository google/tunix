...
"""PEFT Checkpoint manager unittests."""
...
class TestModel(nnx.Module):
  """A simple mock NNX module used to validate model parameters."""
...
def create_sharded_model(model_ctor, rngs, mesh):
  """Creates a sharded model with the given model constructor and rngs.

  Args:
    model_ctor: The model constructor.
    rngs: The random number generators.
    mesh: The mesh to use for sharding.

  Returns:
    A tuple of the sharded model and its state sharding.
  """
...
class BaseCheckpointManagerTest(parameterized.TestCase):
  """Base class providing shared checkpoint manager evaluation suites."""
  ...
  def test_empty_root_directory(self):
    """Verifies disabled execution lifecycle when directory path is None."""
  ...
  def test_checkpoint_manager_options_none_sets_default(self):
    """Verifies default manager configurations populate when options omitted."""
  ...
  def test_save(self):
    """Verifies parameters properly serialize to checkpoint file trees."""
    cp_path = f'{self.temp_path}/{self.id()}'
  ...
  def test_restore(self):
    """Verifies checkpoint restoration accurately updates model variables."""
    cp_path = f'{self.temp_path}/{self.id()}'
    cp_manager = checkpoint_manager.CheckpointManager(cp_path)
    model, _ = create_sharded_model(TestModel, nnx.Rngs(0), self.mesh)
  ...
  def test_restore_different_sharding(self):
    """Verifies robust restoration across disparate target sharding layouts."""
    cp_path = f'{self.temp_path}/{self.id()}'
    cp_manager = checkpoint_manager.CheckpointManager(cp_path)
  ...
  def test_restore_with_lora(self):
    """Verifies selective restoration bounds targeting adapted LoRA state."""
    cp_path = f'{self.temp_path}/{self.id()}'
    cp_manager = checkpoint_manager.CheckpointManager(cp_path)
    model, _ = create_sharded_model(TestModel, nnx.Rngs(0), self.mesh)
  ...
  def test_save_and_restore_with_custom_metadata(self):
    """Verifies top-level metadata dictionaries save and restore."""
    cp_path = f'{self.temp_path}/{self.id()}'
  ...
  def test_save_and_restore_with_optimizer_state(self):
    """Verifies optimizer tracking properties and active state reload cleanly."""
    cp_path = f'{self.temp_path}/{self.id()}'
    ckpt_manager = checkpoint_manager.CheckpointManager(cp_path)
    model, _ = create_sharded_model(TestModel, nnx.Rngs(0), self.mesh)
  ...
  def test_restore_without_optimizer(self):
    """Verifies save and restore function when optimizer is skipped."""
    cp_path = f'{self.temp_path}/{self.id()}'
    ckpt_manager = checkpoint_manager.CheckpointManager(cp_path)
    model, _ = create_sharded_model(TestModel, nnx.Rngs(0), self.mesh)
    optimizer = nnx.Optimizer(
  ...
  def test_restore_with_backward_compatibility(self, ckpt_path):
    """Verifies checkpoints saved with StandardSave can be restored.
    ...
    Args:
      ckpt_path: Path to the checkpoint to restore.
    """
