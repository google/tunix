...
class PeftTrainerTest(absltest.TestCase):
  ...
  def test_basic_training_with_profiler(self, mock_profiler_init):
    ...
    expected_calls = (
        # steps 0 through 8.
        [mock.call.should_activate(step) for step in range(8)]
        # steps 1 through 9 as the step number is incremented during each step.
        + [mock.call.should_deactivate(step) for step in range(1, 9)]
        + [mock.call.activate(), mock.call.deactivate()]
    )
