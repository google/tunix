...
class AttentionTest(parameterized.TestCase):
  ...
  def test_bucket_prefix_length_and_overflow(self):
    """Verify _bucket_prefix_length and _maybe_bucket_prefix_length behavior."""
    boundaries = (0, 128, 256)
    cache_len = 1024

    # Exact boundary
    self.assertEqual(
        config_lib._bucket_prefix_length(128, cache_len, boundaries), 128
    )
    # In-between boundary (rounds up to 128)
    self.assertEqual(
        config_lib._bucket_prefix_length(100, cache_len, boundaries), 128
    )
    # Overflow past boundaries (falls back to cache_len)
    self.assertEqual(
        config_lib._bucket_prefix_length(500, cache_len, boundaries), 1024
    )
    # Beyond cache_len (clamped to cache_len)
    self.assertEqual(
        config_lib._bucket_prefix_length(2000, cache_len, boundaries), 1024
    )
    # Passthrough when not chunked prefill
    self.assertEqual(
        config_lib._maybe_bucket_prefix_length(
            100, None, is_chunked_prefill=False, boundaries=(0, 128)
        ),
        100,
    )
