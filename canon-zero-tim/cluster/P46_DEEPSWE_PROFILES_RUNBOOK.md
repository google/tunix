...
This package has local CPU evidence only. A rendered YAML is not a target PASS.
Do not apply it until the exact implementation commit has been published to
...
prefix cache off, and a stable seed for every `(docker_image, sample_index)`.
...
1. Run one 64-chip `q4-clean-eval` physical shard at logical index 0 and
   ...
   not a PASS.
2. Manually inspect at least one successful and one failed trajectory from the
...
5. Launch `q32-train` against the original clean 1851-row whitelist. Require
