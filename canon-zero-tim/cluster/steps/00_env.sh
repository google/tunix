...
if [ -n "${CANON_P38_SERVING_CAPTURE_DIR:-}" ]; then
  ...
  if [ "${CANON_P38_FIXED_LM_HEAD:-0}" = "1" ]; then
    ...
    [ -z "${CANON_P38_KV_OBSERVER_DIR:-}${CANON_P38_KV_OBSERVER_CLASSIFICATION:-}${CANON_P38_SEAM_OBSERVER:-}${CANON_P38_TAIL_OBSERVER:-}${CANON_P38_TERMINAL_DISCRIMINATOR:-}" ] || {
      echo "[env] P38 fixed lm-head must not attach diagnostic observers" >&2
      fail=1
    }
    for k in CANON_FIXED_AR CANON_FIXED_AR_EMBED \
             CANON_PALLAS_ALL_PROJ CANON_PALLAS_ALL_RMSNORM \
