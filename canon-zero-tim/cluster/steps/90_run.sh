...
n_p57_stock_train_runtime=$(grep -ac '^\[P57.STOCK\] TRAIN_RUNTIME_PASS regime=stock-fast arm=mismatch canonical_bundle=off observer=warning-only processed_b=observer-only$' "$LOG" || true)
...
n_p57_tim_purity=$(grep -ac '^\[P57.TIM_PURITY\] PASS sampler_is=none old_logps=rollout tis_weights=absent trainer_rescore=observer-only$' "$LOG" || true)
