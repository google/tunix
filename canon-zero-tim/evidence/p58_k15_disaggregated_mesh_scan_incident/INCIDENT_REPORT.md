# DeepSWE Qwen3-4B Zero-HP Full (K15) Disaggregated Mesh Scan Mismatch Incident Report

**Incident ID**: `p58_k15_disaggregated_mesh_scan_incident`  
**Workload**: `canon-p58-ds4b-zero-hp-full-k15` (128 TPU v5p, 32 worker pods + 1 head pod)  
**Execution Date**: 2026-08-31  
**Target Hardware**: 128 TPU v5p (DP32xTP4, 32 worker nodes, 4 TPU/node)  
**Failure Point**: `tunix/rl/canonical_qwen3_adapter.py:8100` -> `run_layers_fwd_tape_scan:3687` -> `self._p71_fwd_scan_fn` -> `qwen3_p22xh.py:144` (`P22XHRmsNorm.__call__`)  
**Error**: `ValueError: Received incompatible devices for jitted computation`

---

## 1. Executive Summary

JobSet `canon-p58-ds4b-zero-hp-full-k15` was launched on a full 128 TPU v5p slice (32 worker nodes) for continuous production DeepSWE RL training.

### Major Milestone Achievements in K15:
1. **Full Step 0 Multi-Turn Rollout Succeeded**:
   - 128 multi-turn trajectories completed rollout across 32 TPU hosts (116 finished naturally, 12 max-turn truncated, 0 timeouts/environment failures).
   - Solved **3 SWE tasks** in Step 0 (`Reward = 1.0`), producing **31 non-zero advantage samples** (24.2%).
   - Total action tokens generated: **407,262**.
2. **Rescore-B Prefill Succeeded**:
   - Rescored all 407,262 action tokens in parallel.
3. **100% Strict Pre-Alignment PASS**:
   ```text
   ...
   ```
   $S_{decode} - S_{prefill} = 0$ B and $S_{prefill} - T_{old} = 0$ B across all 407,262 action tokens.

---

## 2. Crash Root Cause Analysis

Following successful pre-alignment validation, execution entered Step 0 segmented backward / forward tape reconstruction in `run_layers_fwd_tape_scan`:
...
### Mechanism:
- In the 128 TPU disaggregated setup, 128 chips are split into two 64-chip meshes:
...
- In `SegmentedEngine.__init__`, per-layer functions are wrapped with `bind_execution_mesh` (which enters `_canonical_fixed_ar_execution_mesh(self._source_engine_mesh, self._engine_mesh, ...)`).
...
2. Commit with tags `TAG=agy` and `CONV=94af9720-af95-4534-8f73-f43308cb7196`.
3. Submit next attempt K16 on the 128 TPU slice.
