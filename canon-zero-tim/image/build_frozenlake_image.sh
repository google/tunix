...
# (canon-zero-tim/image/Dockerfile.frozenlake), then optionally push it.
#
...
#   L3  a real run    -- train_frozenlake_v5p_1host_docker.sh (archive branch) with
...
#   bash canon-zero-tim/image/build_frozenlake_image.sh              # build + L1 + L2
#   PROBE=1 bash canon-zero-tim/image/build_frozenlake_image.sh      # resolve only, no build
#   NO_LOCK=1 bash canon-zero-tim/image/build_frozenlake_image.sh    # ignore the lock and re-resolve
#   VLLM_TPU_VERSION=0.23.0 NO_LOCK=1 bash canon-zero-tim/image/build_frozenlake_image.sh
#
# By default the build installs canon-zero-tim/image/requirements.frozenlake.lock.txt, a
...
#     > canon-zero-tim/image/requirements.frozenlake.lock.txt
#
...
TUNIX_DIR="${TUNIX_DIR:-$(dirname "$(dirname "$SCRIPT_DIR")")}"
...
$DOCKER build \
  --network=host \
  -f canon-zero-tim/image/Dockerfile.frozenlake \
...
$DOCKER run --rm --privileged --net=host "$IMAGE" \
  python3 canon-zero-tim/image/verify_tpu_stack.py \
...
echo "    bash train_frozenlake_v5p_1host_docker.sh   # archive branch only"
echo
if [ -n "${PUSH:-}" ]; then
  echo "===== PUSH (requested) ====="
