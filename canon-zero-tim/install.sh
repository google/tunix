...
patch -s --no-backup-if-mismatch "$OUT/qwen3.py" \
  "$PKG/patches/tpu_inference/17-qwen3-p38-seam-observer.patch" || {
  echo "      PATCH FAILED: 17-qwen3-p38-seam-observer.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/18-tpu-runner-p38-seam-observer.patch" || {
  echo "      PATCH FAILED: 18-tpu-runner-p38-seam-observer.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/19-tpu-runner-p38-terminal-tail.patch" || {
  echo "      PATCH FAILED: 19-tpu-runner-p38-terminal-tail.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/20-tpu-runner-p38-terminal-discriminator.patch" || {
...
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/26-tpu-runner-m15-replay-envelope.patch" || {
  echo "      PATCH FAILED: 26-tpu-runner-m15-replay-envelope.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/27-tpu-runner-m15-mixed-program-tail.patch" || {
  echo "      PATCH FAILED: 27-tpu-runner-m15-mixed-program-tail.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/28-tpu-runner-m15-mixed-program-path.patch" || {
  echo "      PATCH FAILED: 28-tpu-runner-m15-mixed-program-path.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/30-tpu-runner-m15-wide-incident-bypass.patch" || {
  echo "      PATCH FAILED: 30-tpu-runner-m15-wide-incident-bypass.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/31-tpu-runner-m15-multiround-budget.patch" || {
  echo "      PATCH FAILED: 31-tpu-runner-m15-multiround-budget.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/32-tpu-runner-p58-mixed-program-path.patch" || {
  echo "      PATCH FAILED: 32-tpu-runner-p58-mixed-program-path.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/33-tpu-runner-m15-replay-round-provenance.patch" || {
  echo "      PATCH FAILED: 33-tpu-runner-m15-replay-round-provenance.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/34-tpu-runner-p58-multiround-budget.patch" || {
  echo "      PATCH FAILED: 34-tpu-runner-p58-multiround-budget.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/35-tpu-runner-m15-layer0-kv-prefix.patch" || {
...
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/38-tpu-runner-p57-tito-witness.patch" || {
  echo "      PATCH FAILED: 38-tpu-runner-p57-tito-witness.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/39-tpu-runner-perf3-engine-step-log.patch" || {
  echo "      PATCH FAILED: 39-tpu-runner-perf3-engine-step-log.patch" >&2
  exit 1
}
patch -s --no-backup-if-mismatch "$OUT/tpu_runner_p21_l30.py" \
  "$PKG/patches/tpu_inference/40-tpu-runner-perf-receipt-v2.patch" || {
  echo "      PATCH FAILED: 40-tpu-runner-perf-receipt-v2.patch" >&2
  exit 1
}
