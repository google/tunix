#!/usr/bin/env python3
"""Qwen3-8B TP2 projection contract for the FrozenLake one-host carrier."""
...
BK = 256
MATMUL_K_PADDING = {}
# 151936 / 2 = 75968 TP-local vocabulary rows.  The fixed-size projection
# pads only this endpoint to the next BN=256 multiple.
...
SITES = (
    Site(".q_proj", "q_proj", ("TD,DNH->TNH", "TD,NDH->TNH"), 4096, 2048, False),
    Site(".k_proj", "k_proj", ("TD,DKH->TKH",), 4096, 512, False),
    Site(".v_proj", "v_proj", ("TD,DKH->TKH",), 4096, 512, False),
    Site(".o_proj", "o_proj", ("TNH,NHD->TD",), 2048, 4096, True),
    Site(".gate_proj", "gate_proj", ("mn,np->mp",), 4096, 6144, False),
    Site(".up_proj", "up_proj", ("mn,np->mp",), 4096, 6144, False),
    Site(".down_proj", "down_proj", ("mn,np->mp",), 6144, 4096, True),
)
...
def validate_model_env() -> None:
  ...
  if wrong:
    raise RuntimeError(
        "Qwen3-8B TP2 model contract mismatch: " + ", ".join(wrong)
    )
...
def validate_manifest(sites) -> None:
  ...
  if len(sites) != 7 or len({site.family for site in sites}) != 7:
    raise ValueError("expected seven unique Qwen3-8B TP2 projection sites")
  ...
  if MATMUL_K_PADDING or MATMUL_N_PADDING != {75968: 76032}:
    raise ValueError(
        "Qwen3-8B TP2 must admit only lm_head N75968->76032 padding"
    )
  local_feature = INTERMEDIATE_SIZE // TP_SIZE
  if local_feature != 6144 or local_feature % 256 or SWIGLU_FEATURE_PADDING:
    raise ValueError(
        "Qwen3-8B TP2 SwiGLU must remain on the unpadded BF256 path"
    )
...
def match_site(prefix: str, equation: str) -> Site | None:
  ...
  if not matches:
    return None
  if len(matches) != 1:
    raise RuntimeError(f"ambiguous Qwen3-8B TP2 site for {prefix!r}")
  site = matches[0]
  ...
  try:
    ...
    for name, previous in (
      ...
      if previous is None:
        ...
        os.environ[name] = previous
  print("V2_QWEN8B_TP2_CONTRACT_SELFTEST_PASS cases=5/5")
