#!/usr/bin/env python3
"""Qwen3-8B TP1 projection contract for one-host FrozenLake carriers."""
...
BK = 256
MATMUL_K_PADDING = {}
# The TP1 endpoint consumes all 151936 vocabulary rows and pads only that
# endpoint to the next BN=256 multiple.
MATMUL_N_PADDING = {151936: 152064}
...
SITES = (
    Site(".q_proj", "q_proj", ("TD,DNH->TNH", "TD,NDH->TNH"), 4096, 4096, False),
    Site(".k_proj", "k_proj", ("TD,DKH->TKH",), 4096, 1024, False),
    Site(".v_proj", "v_proj", ("TD,DKH->TKH",), 4096, 1024, False),
    Site(".o_proj", "o_proj", ("TNH,NHD->TD",), 4096, 4096, True),
    Site(".gate_proj", "gate_proj", ("mn,np->mp",), 4096, 12288, False),
    Site(".up_proj", "up_proj", ("mn,np->mp",), 4096, 12288, False),
    Site(".down_proj", "down_proj", ("mn,np->mp",), 12288, 4096, True),
)
...
def validate_model_env() -> None:
  ...
  if wrong:
    raise RuntimeError(
        "Qwen3-8B TP1 model contract mismatch: " + ", ".join(wrong)
    )


# The shared Qwen3-8B RMSNorm wrapper imports this historical symbol.
...
def validate_manifest(sites) -> None:
  ...
  if len(sites) != 7 or len({site.family for site in sites}) != 7:
    raise ValueError("expected seven unique Qwen3-8B TP1 projection sites")
  ...
  if MATMUL_K_PADDING or MATMUL_N_PADDING != {151936: 152064}:
    raise ValueError(
        "Qwen3-8B TP1 must admit only lm_head N151936->152064 padding"
    )
  local_feature = INTERMEDIATE_SIZE // TP_SIZE
  if local_feature != 12288 or local_feature % 256 or SWIGLU_FEATURE_PADDING:
    raise ValueError(
        "Qwen3-8B TP1 SwiGLU must remain on the unpadded BF256 path"
    )
...
def match_site(prefix: str, equation: str) -> Site | None:
  ...
  if not matches:
    return None
  if len(matches) != 1:
    raise RuntimeError(f"ambiguous Qwen3-8B TP1 site for {prefix!r}")
  site = matches[0]
...
def self_test() -> None:
  ...
  try:
    ...
    for name, previous in (
        (ENV, old_enabled),
        ("CANON_FIXED_AR", old_fixed_ar),
    ):
      if previous is None:
        os.environ.pop(name, None)
      else:
        os.environ[name] = previous
  print("V2_QWEN8B_TP1_CONTRACT_SELFTEST_PASS cases=6/6")
