#!/usr/bin/env python3
"""Qwen3-1.7B TP1 projection contract for the P59 DP4 proxy."""
...
SITES = (
    Site(".q_proj", "q_proj", ("TD,DNH->TNH", "TD,NDH->TNH"), 2048, 2048, False),
    Site(".k_proj", "k_proj", ("TD,DKH->TKH",), 2048, 1024, False),
    Site(".v_proj", "v_proj", ("TD,DKH->TKH",), 2048, 1024, False),
    Site(".o_proj", "o_proj", ("TNH,NHD->TD",), 2048, 2048, True),
    Site(".gate_proj", "gate_proj", ("mn,np->mp",), 2048, 6144, False),
    Site(".up_proj", "up_proj", ("mn,np->mp",), 2048, 6144, False),
    Site(".down_proj", "down_proj", ("mn,np->mp",), 6144, 2048, True),
)
...
def validate_model_env() -> None:
  ...
  if wrong:
    raise RuntimeError("Qwen3-1.7B P59 TP1 model contract mismatch: " + ", ".join(wrong))
...
def validate_manifest(sites) -> None:
  ...
  if len(sites) != 7 or len({site.family for site in sites}) != 7:
    raise ValueError("expected seven unique Qwen3-1.7B TP1 projection sites")
  ...
  if MATMUL_K_PADDING or MATMUL_N_PADDING != {151936: 152064}:
    raise ValueError(
        "Qwen3-1.7B TP1 must admit only lm_head N151936->152064 padding"
    )
  if SWIGLU_FEATURE_PADDING:
    raise ValueError("Qwen3-1.7B TP1 SwiGLU must remain unpadded")
...
def match_site(prefix: str, equation: str) -> Site | None:
  ...
  if not matches:
    return None
  if len(matches) != 1:
    raise RuntimeError(f"ambiguous Qwen3-1.7B TP1 site for {prefix!r}")
  site = matches[0]
...
def self_test() -> None:
  ...
  try:
    ...
    assert match_site("model.layers.0.self_attn.q_proj", "TD,DNH->TNH").n_local == 2048
    assert match_site("model.layers.0.mlp.down_proj", "mn,np->mp").k_local == 6144
    ...
    for name, previous in ((ENV, old_enabled), ("CANON_FIXED_AR", old_fixed_ar)):
      if previous is None:
        ...
        os.environ[name] = previous
  print("P59_QWEN1P7B_TP1_CONTRACT_SELFTEST_PASS cases=4/4")
