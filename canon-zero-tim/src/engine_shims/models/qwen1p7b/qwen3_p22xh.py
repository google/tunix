"""Qwen3-1.7B all-RMSNorm wrapper for the additive P22.XK stack.

This overlay changes only the admitted model hidden width.  The Pallas
RMSNorm primal, shard_map boundary, and canonical VJP are the already admitted
P22.XH/P22.XK implementations.
"""
...
P21_Q17_RMSNORM_WRAPPER_ID = "qwen3-1.7b-p22xh"
...
spec = importlib.util.spec_from_file_location(
    "_canon_qwen3_p22xh_qwen1p7b_base", BASE_PATH
)
...
def _site(prefix: str) -> str:
    ...
    if prefix == "model.norm" or prefix.endswith(".model.norm"):
        return "final"
    raise RuntimeError(
        f"Qwen3-1.7B P22.XH unregistered RMSNorm prefix={prefix!r}"
    )
...
class P22XHRmsNorm(stock_rmsnorm):
    ...
    def __call__(self, x, mask=None):
        ...
        if x.dtype != jnp.bfloat16 or int(x.shape[-1]) not in P21_Q17_RMSNORM_WIDTHS:
            raise RuntimeError(
                f"Qwen3-1.7B P22.XH requires bf16 F=128/{HIDDEN_SIZE} at "
        ...
        def local(x_local, weight_local):
            ...
            print(
                ...
                f"F={flat.shape[1]} model=qwen3-1.7b",
                flush=True,
            )
