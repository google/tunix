"""Qwen3-8B additive all-RMSNorm wrapper for the P22.XK stack.

The 32B wrapper is left untouched.  This model-specific copy changes only the
admitted hidden width from 5120 to the Qwen3-8B contract width 4096 while
retaining the already promoted Pallas primal and sharding semantics.
"""
...
from p22xf_contract import HIDDEN_SIZE, validate_qwen8b_env
...
spec = importlib.util.spec_from_file_location("_canon_qwen3_p22xh_qwen8b_base", BASE_PATH)
...
def _site(prefix: str) -> str:
    ...
    if prefix == "model.norm" or prefix.endswith(".model.norm"):
        return "final"
    raise RuntimeError(f"Qwen3-8B P22.XH unregistered RMSNorm prefix={prefix!r}")
...
class P22XHRmsNorm(stock_rmsnorm):
    ...
    def __call__(self, x, mask=None):
        ...
        if not self.use_scale:
            raise RuntimeError(
                f"P22.XH requires scale at {self._p22xh_prefix}"
            )
        if x.dtype != jnp.bfloat16 or int(x.shape[-1]) not in (128, HIDDEN_SIZE):
            raise RuntimeError(
                f"Qwen3-8B P22.XH requires bf16 F=128/{HIDDEN_SIZE} at "
        ...
        def local(x_local, weight_local):
            ...
            print(
                ...
                f"F={flat.shape[1]} model=qwen3-8b",
                flush=True,
            )
