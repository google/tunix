"""Qwen3-8B TP8 additive all-RMSNorm wrapper for the P22.XK stack."""
...
spec = importlib.util.spec_from_file_location(
    "_canon_qwen3_p22xh_qwen8b_tp8_base", BASE_PATH
)
...
def _site(prefix: str) -> str:
    ...
    if prefix == "model.norm" or prefix.endswith(".model.norm"):
        return "final"
    raise RuntimeError(f"Qwen3-8B TP8 P22.XH unregistered RMSNorm prefix={prefix!r}")
...
class P22XHRmsNorm(stock_rmsnorm):
    ...
    def __call__(self, x, mask=None):
        ...
        if x.dtype != jnp.bfloat16 or int(x.shape[-1]) not in (128, HIDDEN_SIZE):
            raise RuntimeError(
                f"Qwen3-8B TP8 P22.XH requires bf16 F=128/{HIDDEN_SIZE} at "
        ...
        def local(x_local, weight_local):
            ...
            print(
                ...
                f"F={flat.shape[1]} model=qwen3-8b-tp8",
                flush=True,
            )
