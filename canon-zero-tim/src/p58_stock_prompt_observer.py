"""Observer-only processed prompt logprobs for the P58 native arm.
...
scores raw logits even when decode reports processed logprobs. P58 native uses
this module only for the post-rollout B observer: rollout sampling, model
forward, training, loss, backward, and optimizer state never call it.
"""
...
def _expand_sampling_params(
  ...
  if req_ids_dp is None:
    raise ValueError("P58 native prompt observer requires req_ids_dp")
  if dp_size <= 0 or padded_num_tokens <= 0:
    raise ValueError(
        "invalid P58 native prompt dimensions: "
        f"dp={dp_size}, tokens={padded_num_tokens}"
    )
  if padded_num_tokens % dp_size:
    raise ValueError(
        f"P58 native prompt rows {padded_num_tokens} not divisible by "
        f"dp_size {dp_size}"
    )
  ...
  if populated != expected:
    raise ValueError(
        f"P58 native prompt sampling rows incomplete: expanded={populated}, "
...
def _expand_absolute_target_ids(
  ...
  if dp_size <= 0 or padded_num_tokens <= 0:
    raise ValueError(
        "invalid P58 native target dimensions: "
        f"dp={dp_size}, tokens={padded_num_tokens}"
    )
  if padded_num_tokens % dp_size:
    raise ValueError(
        f"P58 native target rows {padded_num_tokens} not divisible by "
        f"dp_size {dp_size}"
    )
  ...
  if populated != expected:
    raise ValueError(
        f"P58 native prompt target rows incomplete: expanded={populated}, "
        f"scheduled={expected}"
    )
  return target_ids
...
def compute_processed_prompt_logprobs(
  ...
  if not _ANNOUNCED:
    print(
        "[P58.STOCK_OBSERVER] PROCESSED_PROMPT_LOGPROBS_PASS "
