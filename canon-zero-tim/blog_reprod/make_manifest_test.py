...
Every case stages a throwaway copy of `runs/` and `data/` in a temp directory
and runs a copy of the script there, so the tree's own `data/manifest.json` is
never rewritten.  `BLOG_REPROD_DIR` overrides where the inputs are read from;
by default it is this file's directory when that directory holds `data/`,
otherwise the checked-out worktree.
"""
import json
import os
import pathlib
...
class MakeManifestTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    shutil.copytree(ROOT / "runs", self.tmp / "runs")
    shutil.copytree(ROOT / "data", self.tmp / "data")
    shutil.copy2(SCRIPT, self.tmp / SCRIPT.name)
    self.shipped = (ROOT / "data" / "manifest.json").read_bytes()

  def run_script(self, *args):
    return subprocess.run([sys.executable, str(self.tmp / SCRIPT.name), *args],
                          capture_output=True, text=True)

  def regenerate(self, *args):
    done = self.run_script("--source-commit", ARCHIVE_COMMIT, *args)
    self.assertEqual(done.returncode, 0, done.stderr)
    return (self.tmp / "data" / "manifest.json").read_bytes()
  ...
  def test_default_prefix_differs_only_in_the_six_provenance_strings(self):
    ...
    for field in differing:
      self.assertEqual(rebuilt[field], shipped[field].replace("wandb_exports/", "runs/", 1))
  ...
  def test_refuses_a_history_that_is_missing_steps(self):
    history = self.tmp / "runs" / STANDARD_RUN / "history.csv"
  ...
  def test_refuses_a_csv_that_is_not_the_cut_of_its_history(self):
    shutil.copy2(self.tmp / "data" / "zero_tim.csv", self.tmp / "data" / "standard.csv")
    done = self.run_script("--source-commit", ARCHIVE_COMMIT)
    self.assertEqual(done.returncode, 1)
