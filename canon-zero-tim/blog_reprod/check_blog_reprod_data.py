...
Checks, for each of the three arms (the exports live under ../results/, see
runs_layout.py):
  data/<arm>.csv                    sha256 == runs.<arm>.plotted_csv_sha256
  <run dir>/history.csv             sha256 == runs.<arm>.source_history_sha256
  <run dir>/config.yaml             sha256 == runs.<arm>.source_config_sha256
...
def main() -> int:
    ...
    for arm in ARMS:
        entry = runs[arm]
        run_dir = runs_layout.run_dir(arm, entry["run_id"])
        checks = (
