...
| V1 DP4 launcher accepts six-update but its inner profile refuses | an inherited P59 profile rejects the stage before the child profile's six-update contract runs | align the inherited stage guard only for the exact V1 kind; retain legacy P59/P66 and no-commit refusals | run the actual base and child profile with six-update plus wrong-kind, tail-8, P66 and no-commit negatives |
