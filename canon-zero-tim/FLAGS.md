...
| CANON_XPROF_LABELS | 为 rollout model/logits/sample 与 trainer fwd/bwd/report JIT 写语义名称；P60-2B 另为 G6 update 写与 Native 相同的 `StepTraceAnnotation("train", step_num=...)` API 及 bounded update/group/transaction host hierarchy，但 Zero-HP 保留 one-whole-update cadence/cardinality，不伪装 Native monolithic microsteps；P60-2E 为 accumulator 写 `micro_step/is_last_accumulate` 并为 optimizer 写 `update_step`，不改数值或同步 | 仪器；V1 full 开；P56 r21/r22 已认证；P60-2C whole-update hierarchy one-host analysis-grade 通过；P60-2E metadata 本地与 exact-image 通过、target 未跑 | XProf 原生提供等价稳定命名后退役 |
