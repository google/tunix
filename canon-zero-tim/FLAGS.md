...
| CANON_FROZENLAKE_ALIGNMENT_WARN_ONLY | FrozenLake有限差异的可观测训练策略。历史 Native/IS full 继续保留 broad warning；优化 Zero 仅精确 M15/main v1-hp、DP8×TP8、300-update、no-eval/no-checkpoint concept run 可开，且只将 `S_decode_vs_S_prefill` 与由其直接派生的 `w`/`wr`/clip/TIS 差异降为 warning。`S_prefill_vs_T_old`、`T_old_vs_T_current`、`r`、任意 nonfinite、梯度、副本与 optimizer transaction 始终 fatal；P45 Zero 始终为 0 | off；renderer 对精确 M15 Zero concept arm 写 1 | 临时收敛曲线逃生阀；host 正负控覆盖 A-B warning、B-C/nonfinite fatal 与 P45 邻臂拒绝；target 未跑，开启后只能声明 `convergence-only / alignment-degraded`，不得声明 Zero-TIM | M15 carrier 修复并完成 strict 300-update target 后恢复 0；失败与 warning 剂量证据永久保留 |
| CANON_M15_TOKEN_CONTINUITY | M15 later-turn token continuity实验 selector。`verify` 保持 rendered-text model call 原样并观察实际 prompt IDs；`exact` 令首轮仍以原始 prompt 建立 serving token tail，后续轮直接提交 initial tail + exact sampled assistant IDs + exact nonterminal environment IDs，关闭 chat-template 重应用，再将 serving 实际消费的 unpadded IDs 逐 token 回验。两种模式仅输出长度、SHA、首 mismatch 位置/两 token 的 bounded receipt；exact 任一不等立即 fatal。实现只保留给两个独立 debug carrier：目标身份仅准 M15 APC debug DP8×TP8 off/on、strict backward-no-commit、`observer=layer`、`m15-wide-v1`、3 rounds；前置 one-host 身份仅准既有 `CANON_P38_ONEHOST_REHEARSAL=1`、DP1×TP4、`CANON_P32_WORKLOAD` absent、无 target selector/profile、APC 精确 0/1、3 rounds。两者逐轮要求 exact-equal receipt；one-host 还逐轮要求 B full-reset/all-cached-token-zero receipt。两者均禁止继承旧 `kv/kv3` 1,226-prefix 载具；malformed/missing/negative token arrays hard-fail | 全局 absent；当前 P45/M15 full profile 与 YAML 均要求键完全不存在；只有 APC debug layer renderer 与签名 one-host runner 可写 `exact`。空值、`0`、未知值、P45/GSM8K/Native/IS/eval、debug `kv/kv3/full/none`、其他拓扑、target/one-host 身份混用均 fatal | 历史 exact-default implementation `3fc7ef8b` 已撤出 production identity；rollback focused 43/43、P57 181/181、V1 92/92、flags 409/409 绿。E0v/E0w host 与 pre-rebase complete pinned image 已绿；real DP1×TP4 one-host pair 在 APC-off/on 各三轮 A-B/B-C 全零、APC-on 91.5% hits 下 `ONEHOST_PAIR_EXACT`。该 one-host 结果只绑定当时 source，不可继承到当前 rebased source、TP8/multihost 或 production Zero-TIM claim；TiTO APC debug DP8×TP8 target 未跑 | 保持 production rendered-text 默认；在当前发布树重跑 immutable-image admission，再用 fresh TiTO APC debug DP8×TP8 matched pair重新建立 FIRST_RED/非复现结论；任一 target 红则保留失败 evidence并按身份回滚；one-host 仅作为前置载具，不能满足 sunset |
