...
| CANON_DEEPSWE_SYSTEM_OPTIMIZATION_ARM | DeepSWE 严格 system-optimization 数值准入 selector。`control|treatment` 只准精确 P44 Qwen3-4B three-update DP4×TP8/DP8×TP8 strict profile；此外 `control` 单独准入精确 P58 Zero `64split` three-update profile。共同 tuple 为 fixed head、rank-parallel checked-VMA、P59-only VMA scope、first-update gate、fingerprint/finite host receipts 与 P71-fwd。P44 `treatment` 才增加 `KEEP_TAPE=stream` 与 `DP_REDUCE_ONCE=1`；P58 control 强制二者真缺省 | absent/off；空值不构成 arm。P58 renderer 只在显式 `--system-optimization-arm control` 时写入，且要求 `64split:zero:three-update`；历史 P58-128 与无 selector 的 64split 语义不变 | 试验；P44 host gates 已绿，P58 64split renderer/profile/authoritative reload/Python contract/manifest/classifier 正负控待本 phase 收口；所有 DP4×TP8/DP8×TP8 target arms 仍未跑，不能从 one-host 或其他 workload/几何继承锚/性能结论 | P44 control 与 P58 64split control 完成 target A=B=C、G1、两发逐字节、runtime receipt/HBM/步时后再判断共同 tuple；P44 treatment 另行判决 keep-tape/reduce-once，失败证据永久保留 |
...
Count: 440 settable names (appendix inventory above; exclusions: none).
