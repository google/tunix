...
| M15 wide durability | CANON_P38_DURABILITY_PROFILE=`m15-wide-v1`:只允许精确 M15 target `off\|on` + `layer\|full` observer；每 30 秒最多复制 32 对完整 JSON/NPZ、逻辑 payload 最多 256 MiB，生成确定性 archive/SHA，远端下载复核后才写 `SHARD_COMPLETE.json`；round classifier 只读这些 sealed shard 的隔离副本，要求 complete live records 与 sealed union 精确相等；终态顺序固定为 shard complete → round complete → COLLECTED → postflight COMPLETE；每次持久化还核对执行 checkout HEAD 与 rendered full SHA | 试验、仅 M15 wide target；首红定位和最小修复完成后整体退役，失败 shard 永久保留 |
...
Count: 387 settable names (appendix inventory above; exclusions: none).
