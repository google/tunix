...
| CANON_M15_TOKEN_CONTINUITY | M15 later-turn token continuity实验 selector。`verify` 保持 rendered-text model call 原样并观察实际 prompt IDs；`exact` 令首轮仍以原始 prompt 建立 serving token tail，后续轮直接提交 initial tail + exact sampled assistant IDs + exact nonterminal environment IDs，关闭 chat-template 重应用，再将 serving 实际消费的 unpadded IDs 逐 token 回验。两种模式仅输出长度、SHA、首 mismatch 位置/两 token 的 bounded receipt；exact 任一不等立即 fatal。实现只保留给后续独立 M15 debug carrier；malformed/missing/negative token arrays hard-fail | 全局 absent；当前 P45/M15 full profile 与 YAML 均要求键完全不存在；空值、`0`、未知值、GSM8K/Native/IS/eval/诊断/其他生产拓扑均 fatal | 历史 exact-default implementation `3fc7ef8b` 已发布，T0 observer、host 与旧 pinned-image construction 绿，但 real M15/one-host/DP8×TP8 target 未跑；用户在 target 前撤回 production default，当前 rollback focused 43/43、P57 181/181、V1 92/92、flags 409/409 绿，immutable-image 未重跑。它改变后续轮模型输入，不能继承 DeepSWE TP4 认证，也不恢复 warning lane 的 Zero-TIM claim | 先用 rendered-text 默认取得 M15 training curve；随后建立独立 real-tokenizer、role-boundary、nonterminal-EOS debug carrier，跑 one-host/DP8×TP8 token-equality 与完整 backward/optimizer 门后再决定是否准入生产 |
