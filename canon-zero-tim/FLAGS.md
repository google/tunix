...
| CANON_M15_TOKEN_CONTINUITY | M15 later-turn token continuity selector。`verify` 保持 rendered-text model call 原样并观察实际 prompt IDs；`exact` 令首轮仍以原始 prompt 建立 serving token tail，后续轮直接提交 initial tail + exact sampled assistant IDs + exact nonterminal environment IDs，关闭 chat-template 重应用，再将 serving 实际消费的 unpadded IDs 逐 token 回验。两种模式仅输出长度、SHA、首 mismatch 位置/两 token 的 bounded receipt；exact 任一不等立即 fatal。只准精确 M15/main Zero v1-hp DP8×TP8、300-update、no-eval/no-checkpoint、alignment-warning concept 身份；malformed/missing/negative token arrays hard-fail | 全局 absent；精确 M15 full profile 与 YAML 默认写 `exact`；任何空值、未知值、P45/GSM8K/Native/IS/eval/诊断/其他拓扑均 fatal | 用户批准 exact-default implementation；T0 observer host 5/5、P57 181/181 与旧 pinned-image 已绿；exact runtime/renderer 本地 host P57 181/181、V1 92/92、flag 409/409，完整 pinned-image 以 `m15_tito_exact=1` 绿。real M15/one-host/DP8×TP8 target 未跑。它改变后续轮模型输入，不能继承 DeepSWE TP4 认证，也不恢复 warning lane 的 Zero-TIM claim | exact M15 300-update target 完成逐轮 token-equal、完整 backward/optimizer/convergence 后决定焊入该 recipe；若 target 红则回滚 profile/YAML selector并保留全部失败 evidence |
