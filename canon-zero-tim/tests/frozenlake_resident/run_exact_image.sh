...
$DOCKER run --rm \
  ...
  bash -euo pipefail -c '
    ...
    bash canon-zero-tim/install.sh "$overlay" \
      ...
      --model qwen8b_tp8
    bash canon-zero-tim/tests/frozenlake_resident/run_cpu.sh
    PYTHONPATH=/workspace python3 \
      canon-zero-tim/tests/frozenlake_three_arm/test_workloads.py \
      P57WorkloadsTest.test_primary_entrypoint_registers_constant_learning_rate_receipt
    ...
    PYTHONPATH=/workspace python3 \
      canon-zero-tim/tests/frozenlake_three_arm/test_stock_fast_contract.py
    PYTHONPATH=/workspace python3 \
...
EOF
    ...
    env PATHWAYS_HEAD="" JAX_BACKEND_TARGET="" JAX_PLATFORMS=cpu \
      python3 canon-zero-tim/tests/frozenlake_three_arm/probe_stock_prompt_observer.py
    rm -r "$observer_state"
