...
_RENDERER = (
    Path(__file__).resolve().parents[2] / "cluster/render_p33_jobsets.py"
)
_IMPORT_PROBE = """
...
shadow = types.ModuleType("render_p34_jobset")
...
assert Path(module.p34.__file__).resolve() == source.with_name("render_p34_jobset.py").resolve()
...
class RendererImportTest(unittest.TestCase):
  ...
  def test_missing_source_sibling_does_not_fall_back_to_ambient_module(self):
    ...
    self.assertIn("render_p34_jobset.py", result.stderr)
