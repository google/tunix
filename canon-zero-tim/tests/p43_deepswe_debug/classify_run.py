#!/usr/bin/env python3
"""Fail-closed classifier for the P43 DeepSWE debug stage ladder."""
...
_TRAJECTORY_SCHEMA = "canon.p43.deepswe.trajectory.v1"
_METRICS_SCHEMA = "canon.p43.deepswe.batch-metrics.v1"
_MANIFEST_SCHEMA = "canon.p43.deepswe.run-manifest.v1"
...
def _artifact_checks(
    debug_dir: Path, *, stage: str, expected_batches: int
  ...
  trajectory_paths = sorted(
      debug_dir.glob("batch-*.trajectories.jsonl.gz")
  ...
  checks = {
      "manifest_exact": (
          ...
          and manifest.get("model_id") == "Qwen/Qwen3-8B"
          and manifest.get("contract_name") == "p43-64chip-debug"
...
def classify(
  ...
  """Returns a systems bring-up verdict without a zero-TIM claim."""
  ...
  artifact_checks, metrics = _artifact_checks(
      debug_dir, stage=stage, expected_batches=expected_batches
  )
  ...
  checks = {
      ...
      == [{"num_tokens": 1024, "num_reqs": 16}],
      "artifact_log_markers": log_text.count(
          "[P43.TRAJECTORY_BATCH]"
      ) == expected_batches
      and log_text.count("[P43.BATCH_METRICS_JSON]") == expected_batches,
      "rollout_only_boundary": (
          log_text.count("[P43.ROLLOUT_ONLY] PASS") == 1
          and not updates
          and "update_step_committed" not in log_text
          if stage == "rollout-only"
          else "[P43.ROLLOUT_ONLY] PASS" not in log_text
      ),
      ...
      "update_contract": all(
          record.get("contract_name") == "p43-64chip-debug"
  ...
  return {
      "schema": "canon.p43.deepswe-debug.run.v1",
      "stage": stage,
      "verdict": "PASS" if not failed else "FAIL",
      "claim_level": "systems-debug-convergence-only",
