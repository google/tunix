...
PERSIST="$ROOT/workloads/alignment-carrier/scripts/persist_p38_gcs.sh"
ARCHIVE_TOOL="$ROOT/workloads/alignment-carrier/scripts/p38_evidence_archive.py"
tmp="$(mktemp -d)"
trap 'rm -r "$tmp"' EXIT
...
install_fake_gcloud() {
  ...
  cp "$ROOT/tests/alignment_carrier_serving/fake_gcloud.sh" "$root/bin/gcloud"
...
bash "$ROOT/workloads/alignment-carrier/scripts/p38_live_snapshot_worker.sh" \
  > "$worker_log" 2>&1 &
worker_pid=$!
printf 'run-more\n' >> "$CANON_RUN_LOG"
...
python3 - "$ROOT/workloads/frozenlake-long-horizon-debug/scripts/test_m15_e0_kv_three_round.py" \
...
python3 "$ROOT/workloads/frozenlake-long-horizon-debug/scripts/aggregate_m15_e0_kv_rounds.py" \
...
mkdir -p "$fake_pkg/workloads/alignment-carrier/scripts"
fake_persist="$fake_pkg/workloads/alignment-carrier/scripts/persist_p38_gcs.sh"
