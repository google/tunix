#!/usr/bin/env python3
"""Import the installed Qwen3-8B TP2 projection chain in the pinned image."""
...
if model.TP_SIZE != 2 or len(model.SITES) != 7:
  raise AssertionError("installed Qwen3-8B TP2 model contract is not active")
model.preflight(require_enabled=True)

print(
    "V2_QWEN8B_TP2_IMPORT_PASS "
    "chain=linear_p22xk model=qwen8b_tp2 tp=2 sites=7",
    flush=True,
)
