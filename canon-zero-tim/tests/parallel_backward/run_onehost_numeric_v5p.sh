...
sha256sum \
  "$pkg/MANIFEST.sha256" \
  "$pkg/tests/parallel_backward/probe_onehost_rpa_v5p.py" \
  "$pkg/tests/parallel_backward/probe_onehost_numeric_v5p.py" \
  "$pkg/tests/parallel_backward/run_onehost_numeric_v5p.sh" \
...
sudo docker run --rm --privileged --network=host --ipc=host \
  ...
  "$image_id" \
  bash -euo pipefail -c '
    python3 canon-zero-tim/tests/parallel_backward/probe_onehost_rpa_v5p.py
    python3 canon-zero-tim/tests/parallel_backward/probe_onehost_numeric_v5p.py
  ' >>"$raw" 2>&1 &
wait_pid=$!
wait "$wait_pid"
docker_rc=$?
