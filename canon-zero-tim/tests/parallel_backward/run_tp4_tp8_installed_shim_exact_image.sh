  ...
  bash -euo pipefail -c '
    ...
    XLA_FLAGS=--xla_force_host_platform_device_count=4 \
      PYTHONPATH="$qwen1p7b_overlay:/workspace" python3 \
        canon-zero-tim/tests/parallel_backward/probe_p59_local_projection_context.py
    XLA_FLAGS=--xla_force_host_platform_device_count=4 \
      PYTHONPATH="$qwen1p7b_overlay:/workspace" python3 \
        canon-zero-tim/tests/parallel_backward/probe_p59_local_attention_context.py
    XLA_FLAGS=--xla_force_host_platform_device_count=4 \
      PYTHONPATH="$qwen1p7b_overlay:/workspace" python3 \
        canon-zero-tim/tests/parallel_backward/probe_p59_rpa_vma_output_context.py
    XLA_FLAGS=--xla_force_host_platform_device_count=8 \
      PYTHONPATH="$qwen1p7b_overlay:/workspace" python3 \
        canon-zero-tim/tests/parallel_backward/probe_tp4_installed_shim_composition.py
    XLA_FLAGS=--xla_force_host_platform_device_count=8 \
      PYTHONPATH="$qwen1p7b_overlay:/workspace" python3 \
        canon-zero-tim/tests/parallel_backward/probe_tp4_installed_attention_composition.py
