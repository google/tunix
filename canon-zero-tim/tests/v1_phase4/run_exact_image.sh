...
$docker run --rm \
  ...
  bash -euo pipefail -c '
    ...
    echo "V1_HP_EXACT_IMAGE_PASS dp16_gathered=1 dp2tp2_parallel=2 p59_tp4_tp8=2 p59_checked_vma_real_shim=4 p59_rpa=2 p59_fused_linear=2 p62_numeric=6 p64_numeric=4 p64_capsule=3 p63_clip=1 first_update_gate=4 gsm_scale_replay=1 p57_wandb=1 frozenlake_system_optimization=1 frozenlake_ab_warning=2 m15_token=1 m15_tito_impl=1 m15_tito_option=exact m15_tito_default=off apc_m15_carrier=74 m15_d3e=1 m15_e0=30 m15_e0_kv3=3 m15_e0_kv3_return=1 m15_e0u_round0_recovery=8 m15_e0v_tito=7 m15_e0v_onehost_arm=5 m15_e0v_onehost_pair=5 m15_e0v_onehost_runner=3 m15_durability=1 m15_round_provenance=1 perfetto_window=1 manifests=3"
  '
