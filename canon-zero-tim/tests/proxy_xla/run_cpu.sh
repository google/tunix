...
python3 -m py_compile \
  canon-zero-tim/cluster/render_proxy_xla_jobset.py \
  canon-zero-tim/tests/proxy_xla/test_render_proxy_xla_jobset.py
python3 -m unittest \
  canon-zero-tim/tests/proxy_xla/test_render_proxy_xla_jobset.py

tmpdir="$(mktemp -d)"
trap 'rm -r "$tmpdir"' EXIT
python3 canon-zero-tim/cluster/render_proxy_xla_jobset.py \
