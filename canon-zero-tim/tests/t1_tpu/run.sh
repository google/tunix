...
if [ "${CANON_IN_CONTAINER:-0}" != "1" ]; then
  ...
  exec ${DOCKER:-sudo docker} run --rm --privileged --net=host \
    ...
    -e CANON_CANONICAL_DEPTHS="${CANON_CANONICAL_DEPTHS:-}" \
    -e CANON_RUN_T2_DP="${CANON_RUN_T2_DP:-0}" \
    -e CANON_RUN_P38_AVAL="${CANON_RUN_P38_AVAL:-0}" \
    -e CANON_P38_AVAL_REPORT="${CANON_P38_AVAL_REPORT:-}" \
    ...
    -e CANON_TP_SIZE="${CANON_TP_SIZE:-}" \
    -e CANON_DP_PROBE_LOCAL_SAMPLES="${CANON_DP_PROBE_LOCAL_SAMPLES:-}" \
