...
printf 'checked_vma=%s p59_only=%s first_update=%s p63_clip=%s\\n' \
  "${{CANON_P59_CHECKED_VMA-absent}}" \
  "${{CANON_P67_P66_VMA_P59_ONLY-absent}}" \
...
class P58ProfileTest(unittest.TestCase):
  ...
  def test_native_removes_complete_numerical_bundle(self):
    ...
    self.assertIn(
        "checked_vma=absent p59_only=absent first_update=absent "
        "p63_clip=absent", output
    )
...
source {CANON}
source {HP_PROFILE}
printf '%s\n' "$CANON_P58_CHECKED_VMA_DIAGNOSTIC|$CANON_P59_CHECKED_VMA|$CANON_P66_P59_CHECK_VMA|$CANON_P67_P66_VMA_P59_ONLY|$CANON_V1_HP_FIRST_UPDATE_GATE|$CANON_P63_OVERFLOW_SAFE_CLIP|$CANON_FIXED_AR_GATHER|$CANON_CONTINUE_DECODE|$CANON_VLLM_ENABLE_PREFIX_CACHING|$CANON_DEEPSWE_ALIGNMENT_WARN_ONLY"
"""
    output = subprocess.run(
        ["bash", "-c", script], check=True, text=True, capture_output=True
    ).stdout
    self.assertIn("off|0|0|0|0|0|1|8|0|0", output)

  def test_zero_hp_checked_vma_on_diagnostic_resolves_exact_tuple(self):
    script = f"""
set -euo pipefail
export CANON_STATE=/tmp/p58-vmaon-test
export CANON_V1_HP_FULL=1
export CANON_P58_TIM_ARM=zero
export CANON_P58_DEEPSWE_TIM=1
export CANON_P58_TIM_ADMITTED=1
...
export CANON_P38_CONTROLLED_EXIT=1
export CANON_P38_DIAGNOSTIC_ROUNDS=1
export CANON_P38_DIAGNOSTIC_ROUND_FILE=/tmp/p58-vmaon-test/round
source {CANON}
source {HP_PROFILE}
printf '%s\n' "$CANON_P58_CHECKED_VMA_DIAGNOSTIC|$CANON_P59_CHECKED_VMA|$CANON_P66_P59_CHECK_VMA|$CANON_P67_P66_VMA_P59_ONLY|$CANON_V1_HP_FIRST_UPDATE_GATE|$CANON_P63_OVERFLOW_SAFE_CLIP|$CANON_FIXED_AR_GATHER|$CANON_CONTINUE_DECODE|$CANON_VLLM_ENABLE_PREFIX_CACHING|$CANON_DEEPSWE_ALIGNMENT_WARN_ONLY"
"""
    ...
    self.assertIn("on|1|1|1|0|0|1|8|0|0", output)
