...
| V1 full Attempt 3 passed 194,633-element strict pre-alignment and all 16 forward groups, then stopped pre-optimizer when the attention prelude repeated already TP-local K/V from 2 to 4 heads while the correct local cache remained 2 | g64m | `tasks/v1-phase4-three-full-recipes/evidence/v1_hp_three_full_attempt3_20260824/`; both `SHA256SUMS` entries verify; `CANON_ALIGN_PRE PASS`, 0 FAIL, 0 optimizer commits; `INCONCLUSIVE_PRE_OPTIMIZER_SHAPE_CONTRACT`, no performance claim |
