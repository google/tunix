...
- Exact publication SHA: pending final P43 commit/readback
...
The final commit SHA, full commands, and gate counts are recorded in
`state.md` and `log.md` when publication completes. Remote logs and outcomes
belong in a follow-up P43.5 ledger entry; a local pass must not be relabeled as
remote evidence.
