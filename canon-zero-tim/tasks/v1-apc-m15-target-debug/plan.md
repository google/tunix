...
| E0s | rerun the E0 discriminator as an independently durable three-round stability pair | `kv3`/`m15-e0-kv-v1`; every round seals 8A+8B, checkpoints classifier input, classifies, uploads/readbacks, writes `ROUND_COMPLETE`, then ACKs; all three outcomes agree; root loss cannot erase completed rounds | Attempt 19 target executed but the carrier failed: treatment round 0 red could not join at the equality boundary; control round 1 advanced away from the static target and emitted zero KV records; no 3/3 verdict |
| E0t | repair the Attempt-19 classifier boundary and freeze one D3e-bound prompt inventory across all three rounds | next-token equality joins while future red positions remain rejected; exact KV3 profile emits one frozen and two requeue markers with one prompt SHA; neighboring profiles still advance | aggregate host PASS (task 193, V1 91, P3 31, fake-GCS, flags 408); post-repair exact-image and target NOT RUN |
| E0u | recover Attempt-20 APC-on round-0 classifier input without another target launch | original render/source and committed partial return bind; archive/manifest/receipt self-verify; archived source-bound classifier returns one compact result or an explicit `classification=NONE`; no result can claim 3/3 or authorize repair | HOST PASS: task 199/199, recovery 6/6, V1 91/91, P3 31/31, persistence, flags 409/409; exact-image and real GCS recovery NOT RUN |
| E0v | preserve the E0u failed join and establish a fresh exact-TiTO APC debug program identity | failure return retains verified counters/hashes and a bounded join matrix; exact TiTO is admitted only for matched three-round layer re-baseline and each round has an exact-equal receipt | canonical HOST PASS: task 210, failure audit 8, TiTO postflight 5, token continuity 6, V1 CPU 92, P3 31, persistence, flags 409; exact-image, scratch execution, and target not run |
| E0w | certify the exact-TiTO program on one four-chip v5p host before any multihost launch | matched DP1xTP4 APC off/on each complete three strict rounds; TiTO and B full-reset receipts cover every round; source/diff/image match; failures remain immutable | PASS: pre-rebase one-host `e0w4` is source-bound `ONEHOST_PAIR_EXACT`, both arms A-B/B-C `[0,0,0]/[0,0,0]`, APC-on hits 91.5%, root manifest `b52fe75a...`; rebased publication host SHA `fcf24a07...` and pinned image SHA `7f4b2dc4...` PASS with production TiTO default off; rebased-source TPU and DP8xTP8 NOT RUN |
| E0z | recover and audit the already executed e0w5 exact-TiTO DP8xTP8 pair without another launch | original e0w5 render/source/label bind; all available small per-round classifiers, checkpoints, stage receipts, completions, and root markers are hash-verified; failures preserve scratch/raw log; response distinguishes reported from recovered facts | complete: GCS read-only recovery executed; status=PARTIAL_ROUNDS_RECOVERED (off 2 sealed exact, on Round 0 failed classify at reported A-B=615); evidence archived at v1_apc_m15_e0w5_recovery_20260831; numerical repair unauthorized |
