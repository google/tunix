...
| D | observer-neutral coarse-to-fine first-red walk on known-red DP8xTP8 | `FIRST_RED_LOCALIZED` names last exact and first red tensor, shape ledger, request/token/cache coordinate, and `file:line` | active: expected Attempt-9 classifier objects are absent and receipt source is invalid; enumerate all root object names, then salvage exact survivors or repair durability before one fresh target pair |
