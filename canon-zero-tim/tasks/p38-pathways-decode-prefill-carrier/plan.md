...
| P38.2w | LM-head program discriminator | Screen real Qwen3-8B weight at M=16/M=256 on one v5p, then run one slim stock-only `BF16_BF16_F32` target arm without terminal recapture | host complete: both default/preset exact across M and each other for 4/4 real-weight seeds, lowering intervention present, negative=1; target not run |
