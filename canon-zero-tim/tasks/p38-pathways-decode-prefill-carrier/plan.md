...
| P38.2w1 | P38s22 offsite evidence audit | One contract-driven, read-only GCS command verifies root and round objects, recomputes endpoint evidence, rejects unproven terminal classification, and returns a small SHA-sealed receipt | local implementation and 11 focused positive/negative archive gates PASS; remote audit not run |
| P38.2x | Dedicated fixed-tile Pallas lm-head | Remove the M16/M256 lm-head program freedom with one fixed-tile/fixed-order program, first on real-weight one-host and then in one three-round P38s23 target | pending; forbidden until P38.2w1 return is reviewed |
