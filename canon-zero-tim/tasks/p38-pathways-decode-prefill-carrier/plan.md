...
| P38.2w2 | P38s22 independent-round seal salvage | Verify three actual deterministic round archives and recompute the discriminator while preserving missing root postflight as a separate unadmitted claim | implementation/local gates complete; remote audit `TARGET NOT RUN` |
| P38.2x | Dedicated fixed-tile Pallas lm-head | Remove the M16/M256 lm-head program freedom with one fixed-tile/fixed-order program, first on real-weight one-host and then in one three-round P38s23 target | pending; forbidden until P38.2w2 return is reviewed |
