...
| P38.2s | Alias-aware Round-0 seam-plus-tail reduction | All 32 red points select byte-identical A/B seam and tail aliases or retain every conflict; the official `require_tail` classifier and standalone compact-bundle auditor reproduce the result | active; zero TPU. Implement/review reducer support, then execute once beside GCS on immutable P38s18r2 Round 0. |
