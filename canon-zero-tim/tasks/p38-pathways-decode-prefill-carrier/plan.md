...
| P38.2p | GCP-side byte-preserving seam reduction | Source snapshot SHA verifies, every red action selects exactly one A and B record, sparse original indices remain provenance-attested, and a compact official-classifier bundle is sealed | local implementation/gates complete; GCP execution pending review, commit, and push |
