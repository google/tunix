...
| P38.2s | Alias-aware Round-0 seam-plus-tail reduction | All 32 red points select byte-identical A/B seam and tail aliases or retain every conflict; the official `require_tail` classifier and standalone compact-bundle auditor reproduce the result | active; zero TPU. Implementation and fake-GCS gates complete in an uncommitted review worktree; review/publish, then execute the fixed wrapper once beside GCS. |
