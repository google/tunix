...
| P38.2t | Target-aware tail join and terminal split | Tail identity includes the capsule target token, every wrong-target candidate remains auditable, all 32 red points reclassify, and a real-v5p same-input reducer control distinguishes reducer construction from production-input drift | complete; immutable v3 evidence joins 32/32 red points, with 26 normalizer-first and 6 raw-target-first signatures; continued by P38.2u |
| P38.2u | One-pass terminal discriminator | Every capsule red action joins exact selected hidden rows, shared-executable blockwise raw/processed-logit evidence, existing tail values, and the production endpoint; one-host exactness/neutrality and one-bit negatives precede one 64-chip launch | observer locally admitted; P38s20 produced 873 seam/tail/terminal records and one red numerical round, then timed out before durability ACK/classification; continued by P38.2v |
| P38.2v | Bounded-object diagnostic durability | Preserve every logical observer file and SHA while reducing each live snapshot/round to one deterministic archive, one manifest, and one completion-last marker; then finish one three-round P38s21 target | transport repair validated; P38s21 sealed 2/3 rounds then exceeded the 4-GiB local observer bound; analysis-grade 54/54 points localize the first measured red interval to lm_head |
| P38.2w | LM-head program discriminator | Screen real Qwen3-8B weight at M=16/M=256 on one v5p, then run one slim stock-only `BF16_BF16_F32` target arm without terminal recapture | target complete at analysis grade: three rounds A-B red (66 elements / 111 bytes total), B-C exact; generic preset rejected |
| P38.2w1 | P38s22 offsite evidence audit | One contract-driven, read-only GCS command verifies root and round objects, recomputes endpoint evidence, rejects unproven terminal classification, and returns a small SHA-sealed receipt | complete as sealed rc=4: root manifest/COLLECTED/COMPLETE unavailable; v1 stopped before round tar audit |
| P38.2w2 | P38s22 independent-round seal salvage | Verify three actual deterministic round archives and recompute the discriminator while preserving missing root postflight as a separate unadmitted claim | complete: PASS for the three-round forward discriminator; root postflight remains unadmitted |
| P38.2x | Dedicated fixed-tile Pallas lm-head | Remove request/learner lm-head program freedom with one M256/K4096/N38144 body, first on real-weight one-host and then in one three-round target | complete as a forward candidate: P38s23r3 measured exact A-B/B-C across 146,042 actions; no backward claim |
| P38.2h | Fixed-lm-head actual-model backward-no-commit | Repair and certify M4096 outer VJP order locally, then execute all DP16 gradient groups/reducer gates with zero optimizer commits and byte-unchanged state | active: first v5p gate found 11,950 `dWeight` differences; fixed-order custom VJP rerun is exact; 64-TPU target prepared, not launched |
