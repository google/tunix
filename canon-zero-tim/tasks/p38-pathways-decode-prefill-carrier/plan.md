...
| P38.2k | Durable final P38 artifacts in `yuxzhang-tunix-models` before controlled exit | GCS write/read preflight, SHA-preserving `COLLECTED.json`, completion-last `COMPLETE.json`, and negative controls | locally complete; publication in current CL, target launch moved to P38.2l |
| P38.2l | Incident-durable terminal capture | Mid-run immutable GCS snapshots, all-red-row incident joins, pinned-image dress rehearsal, instrumentation freeze, then one source-pinned stock P38s13a payload | active; target NOT ADMITTED before local gate |
