...
- Decision: do not stop an active legacy-v5 producer. After terminal/no-producer proof, copy its trajectory tree into a fresh `<resume-tag>/imports/<import-id>/` staging directory and use `seal_p46_legacy_v5_snapshot.py`; never import the live root, hand-author the contract, pre-create a trajectory-only manifest, or place v5 rows directly under v6 outputs.
- Decision: exact old v5 fingerprint reconstruction is not an admission gate because trajectory-v5 did not preserve absolute model/whitelist paths or the source client image. Broadly accepting any SHA-256 is also forbidden. The only admitted v5-to-v6 adoption requires SHA-sealed `legacy_source_contract.json`: stable model/data/sampling/topology semantics match exactly and every observed opaque `(logical_shard_index, config_fingerprint, run_tag)` v5 cohort is enumerated with exact record cardinality. Cohorts do not relax global consecutive-attempt/no-attempt-after-valid identity checks. Unenumerated cohorts, ambiguous identities, missing/tampered contracts or digests, semantic drift, and v6-via-legacy fail before the destination lease/runtime. The old sampler SHA remains `sampled_by`; the new checkout remains separate `harness_commit` provenance.
