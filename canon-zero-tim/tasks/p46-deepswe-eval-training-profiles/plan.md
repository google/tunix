...
| P46.6 | Persistent full clean-data washing | One Q4 runtime completes 463 bounded waves, 29,616 exact identities, 58 logical summaries, and the global learnable list; interruption resumes from fsynced v5 artifacts | active |
...
- Decision: production washing uses one `--full-campaign` JobSet and one resident Q4 runtime, processing the same 463 waves sequentially. Each wave has one shared 3600-second wall-clock budget across its initial attempt and genuine-infrastructure retries; every trajectory is fsynced immediately, and a relaunch with the same source/topology/run-id resumes exact missing identities. It never fans out sandbox waves concurrently.
