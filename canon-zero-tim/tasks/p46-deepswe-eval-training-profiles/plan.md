...
- Decision: exact old v5 fingerprint reconstruction is not an admission gate because trajectory-v5 did not preserve absolute model/whitelist paths or the source client image. Broadly accepting any SHA-256 is also forbidden. The only admitted v5-to-v6 adoption requires SHA-sealed `legacy_source_contract.json`: stable model/data/sampling/topology semantics match exactly and every observed logical shard has one structurally valid opaque `(config_fingerprint, run_tag)` cohort with exact record cardinality. Mixed cohorts, missing/tampered contracts or digests and semantic drift fail before the destination lease/runtime. The old sampler SHA remains `sampled_by`; the new checkout remains separate `harness_commit` provenance.
