...
- Confirmed: returned 256-chip run `p46e25608` attempted 64 l0/p0 identities but produced 62 valid results and two `MODEL_TIMEOUT` attempts. The old resume logic incorrectly treated those invalid records as complete and printed a false physical-shard PASS. The corrected evaluator emits `P46_EVAL_PHYSICAL_INCOMPLETE` and exits nonzero while any valid identity is missing.
