...
| P44.10 | Repair the r05 Mosaic matmul block geometry | Qwen3-4B uses BN/BK128 plus exact `1216->1280` K/N padding; target-shaped forward and canonical VJP pass on four real v5p devices; missing runtime traces fail closed | local and one-host v5p passed; unpublished; 64/256 target pending |
| P44.11 | Real one-host Qwen3-4B DeepSWE integration | A default-off DP1 x TP4 colocated profile loads the model, executes real Docker R2E rollouts, persists trajectories/solve metrics, runs backward without an optimizer commit, records HBM/placement/state fingerprints, and preserves all production contracts | rollout integration passed; backward executed but `INCONCLUSIVE_NO_SIGNAL`; unpublished |
