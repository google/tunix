...
Token-in/token-out continuation is a shared DeepSWE correctness invariant:
Native, Native+IS, and Zero all reuse exact sampled assistant token IDs and
once-tokenized environment IDs. It is not part of the treatment contrast.
...
| P58.19 | Three-round coarse decode/prefill seam localization | One default-off selector renders one exact 128-chip carrier with three sequential frozen-weight layer/tail observation rounds; every round has finite A-B, exact B-C, bounded classification, verified durable seal, and zero VJP/backward/commit | deferred — historical target and local P58.19f work remain preserved; no 128-chip rerun until P58.20 one-host exactness closes |
| P58.20 | Qwen3-4B one-host full-stack Zero-TIM admission | A separately manifested `qwen4b_tp4` seven-target overlay passes host/exact-image gates, then one direct v5p-4 real-R2E carrier proves strict A=B=C and repeat-exact finite nonzero backward with zero optimizer commits | target RED — `p58s20dev_20260829t0330z` proved real G2 rollout and exact B-C over 3,300 action tokens, but A-B differed in 1,307 elements/2,694 bytes beginning at the first environment-to-action seam; backward and commits remained zero |
...
| P58.28 | K11 prompt-only grouped-reverse admission | Only validated DeepSWE may admit prompt-only rows; their loss/output/cotangent is exactly zero, single-turn callers remain fail-closed, exact K11 DP8 geometry and complete pinned-image gates pass | active; local source/P34/flag/complete pinned-image PASS, repaired target pending |

At most one phase may be active. P58.28 is the active construction repair and
...
TiTO source, default-full YAML admission, receipt, exact-image, and direct-v5p
alignment gates. P58.26 owns the K09 production-startup scope repair and is
...
prompt-only grouped-reverse admission. K11 certifies real DP8xTP8
rollout/rescore and strict pre-alignment, but not segmented backward,
optimizer commits, checkpointing, or completion.
...
P58.19d observer-path repair
`ed8ce99a0fa4187e0619237e071990b90d453d72` is also published/read back; the
