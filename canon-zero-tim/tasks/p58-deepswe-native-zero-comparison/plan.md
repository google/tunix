...
| P58.19 | Three-round coarse decode/prefill seam localization | One default-off selector renders one exact 128-chip carrier with three sequential frozen-weight layer/tail observation rounds; every round has finite A-B, exact B-C, bounded classification, verified durable seal, and zero VJP/backward/commit | active — `p58s19c` proved 113 records in repaired `[1686,4096)`, then the observer rejected the signed continue-decode program path; P58.19d locally scopes continue-decode to chronology-only with standard-only tensor capture and fail-closed receipts; construction gates pass, publication and target rerun remain pending |
