...
| P58.26 | K09 full-startup one-host selector scope | Full training binds every later-read one-host diagnostic selector, cannot derive replay geometry, and passes executable full-mode/one-host positive controls plus the complete pinned-image gate | active; local runtime/test repair and host/pinned-image gates PASS on `0d224e4a`; publication pending, target not rerun |

At most one phase may be active. P58.26 is the active construction repair and
...
alignment gates. P58.26 owns the K09 production-startup scope repair. Its host
and pinned-image PASS do not certify rollout, backward, TP8 numerical identity, Pathways,
optimizer commits, or the DP8xTP8 target.
