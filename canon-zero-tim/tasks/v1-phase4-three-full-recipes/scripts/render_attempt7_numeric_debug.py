...
def render(
  ...
  state = values.get("CANON_STATE", "")
  run_log = values.get("CANON_RUN_LOG", "")
  expected_run_log = f"{state}/run.log"
  if not state or run_log != expected_run_log:
    raise ValueError(
        "rendered P62 full-log persistence contract drifted: "
        f"state={state!r} run_log={run_log!r} expected={expected_run_log!r}"
    )
  ...
  receipt = {
      ...
      "state": state,
      "run_log": run_log,
      "classification": f"{state}/p62_backward_numeric.classification.json",
  }
