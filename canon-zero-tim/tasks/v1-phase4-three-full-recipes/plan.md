...
| V1.P4.18 | Keep exact M15 TiTO as an explicit full option while the default remains rendered text | selector isolation, off/exact positive and leakage negatives, host and immutable-image | construction complete; default remains off and the Step-61 incident used the non-TiTO path |
| V1.P4.19 | Generalize prompt-only row admission across registered grouped-reverse workloads without changing zero-action semantics | exact M15/K11 shapes, zero-cotangent and nonempty-identity controls, all-empty fatal, host and complete pinned-image gates | source CL `813bb7c5` committed; focused/host and complete P58/V1 pinned-image gates pass, remote readback and DP8xTP8 target not run |
...
- Decision (2026-09-01 V1.P4.19): a partially prompt-only grouped batch is a
  valid fixed DP transaction when the already-validated action mask is empty
  on those rows. Admit it in every registered grouped workload, preserve the
  low-level default-false boundary, and fail closed when the entire global
  batch has no policy-action token. This changes construction admission only;
  it does not change loss, gradient, reduction, clipping, or optimizer math.
