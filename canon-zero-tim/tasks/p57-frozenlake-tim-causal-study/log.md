...
## 2026-08-27 UTC — Wave 04 diagnosis corrected; Zero reference routed to V1 HP

- Type: incident correction / user decision / fail-closed renderer and profile contract.
- Corrected fact: the durable P45 raw log resolves `CANON_PROFILE_FILE` to the baseline `qwen3-8b-dp8-tp8-frozenlake-tim.env`. Its traceback line enters the serial grouped-reverse reducer, not the P59 parallel reducer-template path. The earlier claim that `_p59_replicated_data_mesh` silently fell back on a six-axis mesh is false: that helper raises on unsupported axes and is absent from the traceback. The immutable incident report remains preserved, but its proposed six-axis whitelist is superseded and must not be implemented for this failure.
- Mechanism: baseline Zero left `CANON_P59_RANK_PARALLEL_BACKWARD=0`; the serial report adjoint carried an engine `data/.../model` mesh into a reducer constructed with the trainer `dp` axis. The constructor correctly stopped before reduction or optimizer commit. Step-0 strict A-B/B-C `0/0` remains valid forward evidence only.
- Decision: active P45/M15 300-update Zero references use the registered `frozenlake-v1-hp` path. The generic renderer rejects a primary baseline Zero train, the base P57 profile rejects a hand-authored wrong-profile Zero train during CPU preflight, and `render_three_arm_wave.sh zero` selects `--high-performance`. The verifier requires the V1 profile, P59 rank-parallel manifest flag, resolved P59/P66/P67/first-update receipts, and the common P57 W&B project.
