...
| 3 | **frozenlake-train** | V1 Phase4 已收口为 clean、线性、ahead-5 的本地发布栈：GSM8K DP16xTP4/200 + FrozenLake P45、M15-main DP8xTP8/300 三个 strict Zero-TIM full；P57 rollout-only eval 为 `0,50,...,300`，APC 仅 FrozenLake 开且 B 始终 full reset。V1 12/12、P57 128/128、P59 30/30、APC 31/31、flags 359/359；pinned-image admission receipt 绿、one-host v5p 3/3 与 51/51 绿，runtime diff 对测试树为零 | **获批 push exact five-CL stack；从 pushed 40-char SHA render 恰好三份 manifest，先单独批准 GSM8K full，完整 postflight 后依次 P45、M15** | 等用户批准 push；随后每个 JobSet apply 仍需单独批准。DP16xTP4/DP8xTP8、APC/TP8 fixed-head target 认证待 full run | tasks/v1-phase4-three-full-recipes |
...
| 5 | **deepswe-train** | p58f12 已写合法 128-row journal，但 128/128 R2E Pods 全在 Kueue `scheduling_gated` 后 start-timeout，零真实 rollout。普通 all-compact no-commit 与全 sandbox-start outage `BLOCKED_SANDBOX_CAPACITY` 分流、production-shaped one-Pod Kueue probe 及 handoff 已实现为 `135867f0`，pinned-image 全门绿。Native/Zero flags 未改，main SandboxFleet 未启用 | **fetch/readback 最终 tip；远端 agent 用真实 task image 跑 one-Pod probe，并确认 128 Pods 至少 256 requested CPU/512Gi request 的 Kueue/节点容量；PASS 后才 fresh p58f13 native full；不得绕 Kueue/apply zero** | 等 CPU sandbox admission/capacity + 128 TPU + 单独 apply 授权 | tasks/p58-deepswe-native-zero-comparison |
