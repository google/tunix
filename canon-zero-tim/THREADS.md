...
| 5 | **deepswe-train** | P58 Qwen3-4B-Instruct、P46 1,012-task clean list、B8xG16/16K、128-chip synchronous-disagg 实现已发布（implementation commit `c5bdc9d993dfaf1a6956335609fbf259f9ed95f7`）且 pinned-image gate 绿；用户决定先跑 native，one-host 明确 waived，zero 暂缓 | **只渲染并运行 native 3-update canary；要求 finite nonzero A-B、B-C exact、3 commits 与完整 trajectory journal；不得 apply zero** | 等远端执行 agent 拉取最新 readback SHA、digest image 与 128 TPU | tasks/p58-deepswe-native-zero-comparison |
