...
| 5 | **deepswe-train** | p58c04 已通过全部 bootstrap、128-chip Pathways、Qwen3-4B/vLLM/W&B 初始化并进入 rollout；128 个 sandbox 同时请求但 1200s 内无 Running，R2E 吞掉 start timeout 后对已删 pod exec，真实 404 被 client `None.decode` 掩盖；本地已改成 start fail-closed→`ENV_TIMEOUT`、sandbox concurrency 64 两波，profile 邻接契约和完整 pinned-image gate 均绿；zero 仍暂缓 | **待批准发布后 fetch 最终 SHA，使用新 run-id p58c05 只重跑 native 3-update；不得复用 p58c01–p58c04 root，不得 apply zero** | 等用户批准 commit/push，再等远端 agent + digest image + 128 TPU/CPU sandbox capacity | tasks/p58-deepswe-native-zero-comparison |
