...
| 5 | **deepswe-train** | p58f09 已正确挂上 128 TPU 并在 1699.1s 完成 B8×G16 rollout，随后 reset-timeout row 的 `original_input=None` 让 learner merge 在 journal/train 前崩溃；本地修复保留 `cpu-np` + head/worker host network，以 required hostname anti-affinity 隔离固定端口，并从 `env.task` 恢复 pre-observation 原始输入；训练/Native/Zero flags 未改 | **完成回归并经用户另行批准发布后，fetch/readback 最终 tip，fresh p58f10 native full；apply 前核对 head anti-affinity/hostname 分布/RM DNS；不得复用 p58f08/p58f09，不得 apply zero** | 等用户 commit/push 批准 + 远端 agent + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
