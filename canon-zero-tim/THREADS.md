...
| 5 | **deepswe-train** | p58c03 已通过既有 bootstrap/Pathways/entrypoint gates，后因父进程保留 renderer 的 `CANON_LOGPROB_M=256` 而被 native contract 正确拒绝；权威 `env.sh` 快照修复与真实 parent-reload 回归、本地 P58/P34/P57 及完整 pinned-image gate 均绿；修复已发布为 `c0ca41805bd65a4fdede4825ed2835cdce6e13ed` 并首次 readback `0/0`；zero 仍暂缓 | **fetch 最终 post-checkpoint SHA，使用新 run-id p58c04 只重跑 native 3-update；不得复用 p58c01/p58c02/p58c03 root，不得 apply zero** | 等远端 agent + digest image + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
