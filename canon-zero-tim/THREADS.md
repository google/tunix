...
| 5 | **frozenlake-zero-full** | M15 exact TITO本地CL已rebase至`18f29c56`；DeepSWE/M15 admission互斥，P45及邻臂selector absent；P57 181/181、V1 92/92、flags 409/409及post-rebase完整pinned-image（`m15_tito_exact=1`）绿 | **完成已批准的fast-forward push与remote SHA readback；仅用clean remote-read SHA渲染P45+M15** | exact TITO改变模型输入；M15仍是alignment-degraded claim；one-host/DP8×TP8 target未跑 | tasks/v1-phase4-three-full-recipes |
