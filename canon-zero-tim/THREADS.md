...
| # | 线程 | 状态 | 下一个门 | 等谁 | 任务目录 |
|---|---|---|---|---|---|
| 1 | **zero-tim-carrier** | P38s23r3 forward exact、P38.2h actual-model backward-no-commit PASS；P38y6 在模型加载前因 `data,model` sharding 对 `dp,tp` mesh 的 bootstrap mismatch 而 INCONCLUSIVE，随后 retry 被旧 evidence path 拦截；actual-mesh sharding + attempt-scoped evidence 已本地 55/55 gate | **审查并逐项批准 repair commit/push；随后只发 fresh P38y7 DP16xTP4 200-step full train** | 等用户批准发布，再等 64 TPU target | tasks/p38-pathways-decode-prefill-carrier |
| 2 | **perf** | P55:查明整步 xplane 实为 engine 前 ~25s(device 缓冲 ~283 万事件/核填满即丢,backward 从未入镜),缩窗 land-and-revert 否决;`CANON_XPROF_PHASE=update` 在 G6 入口起窗→backward 完整(p55c census: block_pullback×1758/adjoint×17,decode 零);p55a/a2/b/c 全 3/3 步 51/51 全零;训练段语义 span 首版(嵌套+all_devices)画面不可读已 revert(e8d4caaf),按官方 peft_trainer 样式(扁平单 span+async_end)重写挂起 | **DP16 一发**(读 p32_vag_reverse 的 adjoint= 验 E;P52 grouped 移植定量) | 等卡 + 用户渲染;P55 包待批推 | tasks/p48-onehost-perf(state.md)+ 外层 p48-p55 |
