...
| 5 | **frozenlake-zero-full** | M15 exact TITO runtime/delivery `3fc7ef8b`已fast-forward发布并精确readback；DeepSWE/M15 admission互斥，P45及邻臂selector absent；P57 181/181、V1 92/92、flags 409/409及post-rebase完整pinned-image（`m15_tito_exact=1`）绿 | **另批render/launch；只用包含`3fc7ef8b`的clean remote-read SHA，随后判one-host/DP8×TP8 target** | exact TITO改变模型输入；M15仍是alignment-degraded claim；one-host/DP8×TP8 target未跑 | tasks/v1-phase4-three-full-recipes |
