...
| 3 | **frozenlake-train** | T9g source CL `ae2e4884` 在已发布 `c07ea8fa` 上增加显式 DP4xTP8/B16xG8（32 卡、128 trajectories）full option；旧 DP8/B256 默认、数值门和 TiTO record-full 策略保留。P57 248/248、V1 104/104、APC 12/12、flags 423/423；固定镜像 8-case runtime + DP4TP8 CPU reducer 已绿，完整 pinned CPU image PASS；41 code/gate hashes 与已提交源码一致 | **发布 full SHA/readback → 另获目标授权后按 HANDOFF 渲染双 TiTO；DP4TP8 target 另验** | 本次 commit/push 已批准；源码与证据分 CL，未启动 TPU/Kubernetes；旧 observer/Orbax 开口未关闭 | tasks/p57-frozenlake-tim-causal-study；tasks/multiturn-tito-cross-workload |
