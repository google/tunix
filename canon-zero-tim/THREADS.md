...
| 5 | **deepswe-train** | p58c05 未获 Kueue quota：worker 把 `tpu-v5p-slice` sentinel 错写成 literal nodepool，导致 `0xv5p-8` flavor 与 affinity 冲突；无 pod/训练启动、无可 resume 状态。P58 renderer 已发布为 sentinel 委托 ResourceFlavor、保留精确 `4x4x8`，显式真实 pool 仍严格；实现 `abbc7600` 首次 readback 0/0；用户已取消独立 3-update stop，zero 仍暂缓 | **fetch 最终 post-checkpoint SHA；fresh p58f01 仅跑 native full 1000 updates，updates 1–3 只监控不停止；不得复用 p58c01–p58c05，不得 apply zero** | 等远端 agent + digest image + 128 TPU/CPU sandbox capacity | tasks/p58-deepswe-native-zero-comparison |
