...
| 5 | **deepswe-train** | p58c01 admission bootstrap 修复已发布；p58c02 Pathways 初始化后因 file-entrypoint 未自举 `/app` 而在 model import 前退出。direct-entrypoint 修复与真实路径回归本地完成，`/tmp` 固定镜像导入及完整 pinned-image gate 绿；本机无 `libtpu.so`，one-host 未跑；zero 仍暂缓 | **获批发布后，以最终远端 readback SHA、新 run-id p58c03 只重跑 native 3-update；不得复用 p58c01/p58c02 root，不得 apply zero** | 等用户批准 commit/push，再等远端 agent + digest image + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
