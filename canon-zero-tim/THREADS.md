...
| 5 | **deepswe-train** | P58 Qwen3-4B-Instruct、P46 1,012-task clean list、B8xG16/16K、128-chip synchronous-disagg native/zero 实现与 pinned-image gate 已绿；用户决定先跑 native，one-host 明确 waived，zero 暂缓 | **发布后只渲染并运行 native 3-update canary；要求 finite nonzero A-B、B-C exact、3 commits 与完整 trajectory journal；不得 apply zero** | 等远端执行 agent 拉取已发布 SHA、digest image 与 128 TPU | tasks/p58-deepswe-native-zero-comparison |
