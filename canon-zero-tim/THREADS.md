...
| 5 | **deepswe-train** | p58f11 已证明 concurrency128 单波：B8×G16 的 128 条 trajectory、8/8 groups 在 1209.2s 完成；随后单条 reset-timeout fallback 因 `SWEEnv.task` 只有 policy_version、缺 prompts 在 learner 前崩；normalized prompt 唯一 task-schema 修复已发布为 `43614af5`，pinned-image gate 绿，训练/Native/Zero flags 未改 | **fetch/readback 最终 tip 后 fresh p58f12 native full；必须先拿到 128-row durable journal，再看 finite backward + TPU optimizer first commit；不得复用 p58f11，不得 apply zero** | 等远端 agent + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
