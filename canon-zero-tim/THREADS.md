...
| 5 | **deepswe-train** | native p58c01 Attempt-0 在 `00_env.sh` 因 stock DP-reduction admission 与 FrozenLake-zero flags 合同裂缝 bootstrap 退出，未进 TPU program；修复已发布（`acd3136267214b367a6755d0ba28d80e883d6753`），保留 native reduction=0，真实 00_env 回归与 pinned-image gate 绿；zero 仍暂缓 | **以最终远端 readback SHA、新 run-id p58c02 只重跑 native 3-update；不得复用 p58c01 root，不得 apply zero** | 等远端 agent + digest image + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
