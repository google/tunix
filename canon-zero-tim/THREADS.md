...
| 3 | **frozenlake-train** | 用户将 superseding campaign 冻结为 300 updates，并启用同 JobSet rollout-only held-out eval：`0,50,...,300`，每点固定 100x8=800 rewards；final step-300 在最终 weight sync 后补跑。paired command 显式 seed42、vLLM global seed0，P45/M15 train/eval 数据逐行+SHA fail-closed；P57 CPU 126/126、flag 322/322、pinned-image 绿，native/IS/zero 六 manifest resolved-env 全绿；无 milestone GCS 税 | **审查 uncommitted diff；批准发布后 fresh render native+IS 四个 300-step JobSet，单独批准 launch；deferred zero 本轮不发** | 等用户批准 commit/push 与 TPU launch；per-request stochastic seed 不受 backend 支持，单次曲线不宣称跨发射 bitwise 同轨迹 | tasks/p57-frozenlake-tim-causal-study |
