...
| 5 | **frozenlake-zero-full** | Attempt 10 的 P45/M15 Step-0 A−B/B−C 均 `0/0`；两臂各完成第 1/32 reverse group 的 36 层 VJP 后，被 `peft_trainer.py` 残留的 interval-10 checkpoint 白名单挡在首个 gradient sink。当前 CL 改为复用统一 checkpoint parser；checkpoint 15/15、Phase4 89/89、P57 146/146 与完整 pinned-image 绿，post-fix target 未跑 | **remote-read exact SHA 后，用 fresh IDs 并发重跑 P45/M15 full；先硬验 32/32 reverse、finite precommit、AdamW、weight sync 和 policy step 1，再看完整 horizon** | 本修复不改数学；launch 仍需单独授权，post-fix optimizer/convergence 为 TARGET NOT RUN | tasks/v1-phase4-three-full-recipes |
