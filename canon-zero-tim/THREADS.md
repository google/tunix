...
| 3 | **frozenlake-train** | P57 的 P45/M15 × native-no-IS/native-token-IS 四臂统一为 450；训练内 eval 仍关。新增 0/50/.../450 隔离评测合同：每 10 保存、LatestN(1) 恢复点 + 每 50 证据点，评测显式 restore 指定 step；native/IS 两套 20-manifest schedule 与 119/119 host 门绿；未改 LR/batch/loss/optimizer | **审查本地 diff并接受数 TB GCS milestone 预算；发布后先跑四个 eval-0，再跑四个不间断 450-step train，最后跑正 milestone eval；deferred zero 本轮不发** | 等用户批准 commit/push、存储预算与 TPU | tasks/p57-frozenlake-tim-causal-study |
...
| 5 | **deepswe-train** | p58f12 已写合法 128-row journal，但 128/128 R2E Pods 全在 Kueue `scheduling_gated` 后 start-timeout，零真实 rollout。local repair 将普通 all-compact 保留为 no-commit/下一批；将全 sandbox-start outage 改为 journal 后 `BLOCKED_SANDBOX_CAPACITY`，不 rescore/train/吃后续 prompt；新增 production-shaped one-Pod Kueue probe，pinned-image 全门绿。Native/Zero flags 未改，main SandboxFleet 未启用 | **获批发布/readback；远端 agent 用真实 task image 跑 one-Pod probe，并确认 128 Pods 至少 256 requested CPU/512Gi request 的 Kueue/节点容量；PASS 后才 fresh p58f13 native full；不得绕 Kueue/apply zero** | 等用户批准 commit/push + CPU sandbox admission/capacity + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
