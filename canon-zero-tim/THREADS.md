...
| 2 | **perf** | v2_integrate F.1a `9dcf4fdb` 惰性解析重构已过 fresh P45 DP2×TP2 r2 scoped control：8 个 micro/update 范数与输入哈希全等，strict alignment、VMA39/39、HBM护栏通过；标准36→1复用 FAIL 保留。阶段二约定范围收口，F.1默认化/最终源码矩阵未完成 | **F.1b 实际 profile/env 默认值与控制臂隔离 → F.2–F.7 逐 CL 验证；不得把 no-commit 当 full-train 认证** | 本任务内 one-host/单一职责本地 CL 已批准；不 push，不碰 GKE；既有 8B 容量/真实 optimizer 债仍 Deferred | 外层 tasks/v2_integrate；tasks/v2-frozenlake-onehost |
