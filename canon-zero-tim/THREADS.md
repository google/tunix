...
| 2 | **perf** | v2_integrate E.1 恢复源码 `54de5c7d` fresh P45 DP2×TP2 r2 frozen-capsule control PASS：8 个 micro 与 update 范数逐位、strict alignment、VMA/finite/replica 收据；标准 36→1 程序复用 FAIL 原样保留。代码整理与最终源码矩阵未完成 | **F.1 配置权威入口 → F.2–F.7 逐 CL 验证；保留 control/Native/VMA；不得把 no-commit 当 full-train 认证** | 本任务内 one-host/单一职责本地 CL 已批准；不 push，不碰 GKE；既有 8B 容量/真实 optimizer 债仍 Deferred | 外层 tasks/v2_integrate；tasks/v2-frozenlake-onehost |
