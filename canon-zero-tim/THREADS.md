...
| 5 | **deepswe-train** | p58f09 的 reset-timeout `original_input=None` learner merge 与 Pathways head 固定端口调度修复已发布为 `678bc5cf`；保留 `cpu-np` + head/worker host network，以 required hostname anti-affinity 隔离端口并从 `env.task` 恢复 pre-observation 输入；完整 pinned-image gate 绿，训练/Native/Zero flags 未改 | **fetch/readback 最终 tip，fresh p58f10 native full；apply 前核对 head anti-affinity/hostname 分布/RM DNS；不得复用 p58f08/p58f09，不得 apply zero** | 等远端 agent + 128 TPU | tasks/p58-deepswe-native-zero-comparison |
