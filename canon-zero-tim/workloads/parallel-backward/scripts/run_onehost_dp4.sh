...

{
  ...
  sha256sum \
    ...
    "$pkg/tests/workloads/classify_run.py" \
    "$pkg/tests/parallel_backward/classify_and_analyze.py" \
...
if [ "$docker_rc" -eq 0 ] && [ "$p66" = 1 ]; then
  ...
  PYTHONPATH="$repo" python3 \
    "$pkg/tests/gsm8k_onehost_convergence_backward/classify_arm.py" \
  ...
  set +e
  PYTHONPATH="$repo" python3 "$pkg/tests/parallel_backward/classify_and_analyze.py" \
...
if [ "$capture" = 1 ]; then
  ...
  if [ "$docker_rc" -eq 0 ]; then
    set +e
    PYTHONPATH="$repo" python3 \
      "$pkg/tests/parallel_backward/inspect_xprof_capture.py" \
