#!/usr/bin/env bash
# Render-only preparation for the exact 128-chip P58 checked-VMA discriminator.
# This script never talks to Kubernetes and never publishes an image or source.
set -euo pipefail
...
python3 "$pkg/cluster/render_deepswe_comparison.py" \
...
echo "P58_CHECKED_VMA_DIAGNOSTIC_PREPARE_PASS source=$actual_source output=$output_yaml"
