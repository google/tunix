...
sudo docker run --rm --privileged --net=host --ipc=host --uts=host \
  ...
  -e CANON_P78_SEGMENTED_ACTOR_LOGPS="${CANON_P78_SEGMENTED_ACTOR_LOGPS:-0}" \
  ...
  bash -euo pipefail -c '
    ...
    exec bash "$1/canon-zero-tim/workloads/deepswe-comparison/scripts/run_onehost_deepswe_seam_probe.sh" "$3"
