...
mkdir -p "$stage"
archive_tool="$CANON_PKG/workloads/alignment-carrier/scripts/p38_evidence_archive.py"
...
if [ "$mode" = m15-shard ]; then
  ...
  mkdir -p "$shard_root"
  shard_rc=0
  python3 "$CANON_PKG/workloads/frozenlake-long-horizon-debug/scripts/stage_m15_wide_shard.py" \
...
if [ "$mode" = m15-round ]; then
  ...
  stage_rc=0
  python3 "$CANON_PKG/workloads/frozenlake-long-horizon-debug/scripts/assemble_m15_wide_round.py" \
  ...
  stage_rc=0
  python3 "$CANON_PKG/workloads/frozenlake-long-horizon-debug/scripts/checkpoint_m15_classifier_input.py" \
...
if [ "$mode" = m15-e0-round ]; then
  ...
  mkdir -p "$round_root"
  stage_rc=0
  python3 "$CANON_PKG/workloads/frozenlake-long-horizon-debug/scripts/stage_m15_e0_kv_round.py" \
    --directory "$CANON_P38_KV_OBSERVER_DIR" \
    --alignment-report "$CANON_PRE_ALIGN_REPORT" \
    --capsule-base "$CANON_P38_MISMATCH_CAPSULE" \
    --replay-ledger "$CANON_APC_M15_REPLAY_LEDGER" \
    --classifier "$CANON_PKG/workloads/alignment-carrier/scripts/classify_p38_kv_observer.py" \
    --output "$round_stage" \
    --round "$round_index" \
    --arm "$CANON_APC_M15_TARGET_DEBUG" \
...
if [ "$mode" = round ]; then
  ...
  python3 "$CANON_PKG/workloads/alignment-carrier/scripts/stage_p38_round.py" \
    --round "$round_index" \
  ...
  if [ "$CANON_P38_DURABILITY_PROFILE" = p58-seam-v1 ]; then
    ...
    JAX_PLATFORMS=cpu PYTHONPATH="$CANON_PKG/..:${PYTHONPATH:-}" \
      python3 "$CANON_PKG/workloads/deepswe-comparison/scripts/classify_p58_coarse_seam_round.py" \
        --directory "$round_stage" \
        --alignment-report "$round_stage/pre-alignment.jsonl" \
        "${p58_round_args[@]}" \
