...
python3 "$canon/cluster/render_frozenlake_long_horizon_debug.py" \
...
cp "$canon/tests/alignment_carrier_serving/fake_gcloud.sh" "$scratch/bin/gcloud"
...
for yaml_path in sorted(render.glob("*.yaml")):
  ...
  subprocess.run([
      sys.executable,
      str(canon / "workloads/frozenlake-long-horizon-debug/scripts/aggregate_m15_e0_kv_rounds.py"),
