...
repo="$(cd "$canon/.." && pwd)"
classifier="$canon/tests/alignment_carrier_serving/test_kv_observer_classifier.py"
...
if [ "$route" = host ]; then
  ...
  "${docker_cmd[@]}" run --rm --pull=never --network=none \
    ...
    "$image_id" \
    python3 "/workspace/canon-zero-tim/tests/alignment_carrier_serving/test_kv_observer_classifier.py"
fi
