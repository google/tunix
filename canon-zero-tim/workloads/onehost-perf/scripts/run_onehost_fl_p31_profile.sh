...
# canon-zero-tim/workloads/optimizer-residency/scripts/
...
sudo docker run --rm --privileged --net=host --name "$container" \
  ...
  -w "$repo" "$image" bash -lc "
    set -e
    python3 '$pkg/workloads/optimizer-residency/scripts/admit_frozenlake_runtime.py' \
