...
#
# ocdbt and zarr3 are both off so one artifact serves both execution modes: the
# Pathways persistence handler (ENABLE_PATHWAYS_PERSISTENCE=1) cannot read
# either format, and plain Orbax reads the non-ocdbt layout fine.
CONVERT_ARGS=(
  "${BASE_YML}"
