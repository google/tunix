...
RolloutGenerateRequest = rollout_protocal.RolloutGenerateRequest
RolloutUpdateParamsRequest = rollout_protocal.RolloutUpdateParamsRequest
RolloutPadIdRequest = rollout_protocal.RolloutPadIdRequest
RolloutEosIdRequest = rollout_protocal.RolloutEosIdRequest
  ...
  def run(self):
    while not self._stop_event.is_set():
      while not self._sink.empty():
        print(self._sink.get(), file=sys.stderr, flush=True)
      time.sleep(0.1)
...
class RolloutWorkerProc(PathwaysClientProcess):
  ...
  def _main(self):
    ...
    # print(f"initializing model (dummy)...")
    ...
    while True:
      ...
      if isinstance(request, RolloutGenerateRequest):
        def handle_generate(request: RolloutGenerateRequest):
        ...
        if request.response_queue:
          executor.submit(handle_generate, request)
        else:
          print(f"generation complete (no queue).")
...
class RolloutEngine:
  def __init__(self,
               ...
               rollout_request_channels: list[Any],
               rollout_ready_channel: Any,
               **kwargs):
    self.logger = logger
    self.init_parallelism = init_parallelism
    self.backend_config = backend_config
    self.worker_config = worker_config
    self.rollout_request_channels = rollout_request_channels
    self.rollout_ready_channel = rollout_ready_channel
    self.kwargs = kwargs
  ...
  def start(self):
    ...
    logger = self.logger
    num_workers = len(self.rollout_request_channels)
      ...
      for _ in range(init_parallelism):
        worker = mp.Process(
          target=RolloutWorkerProc(
            ...
            worker_request_channel=self.rollout_request_channels[worker_id],
    ...
    # worker_ready_channel is one signal for one worker
    # self.rollout_ready_channel is one signal for all workers
    self.rollout_ready_channel.put(True)

  def stop(self):
    for channel in self.rollout_request_channels:
      channel.put(None)
    for worker in self.workers:
      worker.join()

    self.workers = []
    self.rollout_request_channels = []
...
class GrpoMainProc(PathwaysClientProcess):
  ...
  def _main(self):
    ...
    print("waiting for rollout engine ready.")
    self.kwargs["rollout_ready_channel"].get()
...
def main(argv, **kwargs):
  ...
  rollout_request_channels = []
  for _ in range(num_workers):
    rollout_request_channels.append(mp.Queue())
  rollout_ready_channel = mp.Queue(num_workers)
  ...
  try:
    ...
    grpo_proc = mp.Process(
      target=GrpoMainProc(
        ...
        rollout_request_channels=rollout_request_channels,
        rollout_ready_channel=rollout_ready_channel,
        verbose=True,
        argv=argv,
        **kwargs,
      )
    ...
    rollout_engine = RolloutEngine(
      logger=logger.sink,
      init_parallelism=min(4, len(rollout_request_channels)),
      backend_config=jax_backend_config,
