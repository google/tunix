...
launch_orchestrator() {
  if [[ "$LOCAL_MODE" == "true" ]]; then
    ...
    python yaml_generator.py \
      ...
      --worker_startup_command="PYTHONPATH=./examples/distributed python -m tunix.experimental.distributed.runtime.main --discovery_id=orchestrator --discovery_port=12345 --process_executor=tunix.experimental.distributed.runtime.executor.K8sExecutor --process_main=vllm_rollout.orchestrator.main --parallelism=256" \
      | kubectl apply -f -
  fi
}
