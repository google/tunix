...
class Qwen25MathEvaluator:
  ...
  def load_model(self):
    ...
    # Downloads all files for google/gemma-4-31B-it
    # local_dir = snapshot_download(
    #     repo_id="google/gemma-4-31B-it",
    #     repo_type="model",
    #     ignore_patterns=["*.msgpack", "*.h5"], # Optional: skip formats you don't need
    # )
    # print(f"Model repository downloaded to: {local_dir}")
    ...
    #     backend="vllm_jax",
    # )
    print("Creating vLLM sampler, model_version:", self.model_version)
    ...
    for batch_idx, batch in enumerate(tqdm(dataset, desc="Evaluating")):
      ...
      for prompt, question, answer, responses in zip(
        ...
        for response in responses:
          print(f"Evaluating response: {response[:500]}...")  # Print first 200 chars of response
...
MODEL_MAPPING = {
    "google/gemma-4-31B-it": (None,"gs://tunix/models/gemma-4/gemma-4-31B-it"),
}

mesh_config = [[2, 2], ["fsdp", "tp"]]  # 2-way tensor parallelism
# %%
# MATH-500
