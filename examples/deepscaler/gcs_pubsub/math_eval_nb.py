...
def main(argv):
  ...
  import re
  print(f"{argv=}")
  assert len(argv) > 1, "Please provide the path to the trained model checkpoint as the first argument."
      ...
      for batch_idx, batch in enumerate(tqdm(dataset, desc="Evaluating")):
        ...
        for prompt, question, answer, responses in zip(
          ...
          for response in responses:
            ...
            is_correct = rewards_math_verify.math_reward(
                prompts=[prompt],
                completions=[response],
                answer=[answer],
            )[0]
            print(f"last 10 chars of response: {response[-10:]}, answer: {answer}, is_correct: {is_correct}")
...
if __name__ == "__main__":
  # This is MANDATORY for 'spawn' to work
  print("Setting multiprocessing start method to 'spawn' for compatibility.")
