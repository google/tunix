...
def _normalize_tasks_for_fleet(
    tasks: Any, scaffold: str = "openhands"
) -> list[Any]:
  ...
  with _FLEET_LOCK:
    ...
    if scaffold == "openhands":
      ...
      template = TemplateSpec(
          keepalive_command=server_cmd,
          extra_pod_spec={"containers": containers},
    ...
    if tasks is not None:
      fleet_inst.load_tasks(_normalize_tasks_for_fleet(tasks, scaffold=scaffold))
    msg = (
        ...
        f" warm_per_task=True, scaffold={scaffold})..."
    )
    logging.info(msg)
...
class SWEEnv(BaseTaskEnv):
  ...
  def compute_reward(self, verbose: bool = False) -> float:
    """Compute reward for the current environment."""
    if self.env is not None and hasattr(self.env, "compute_reward"):
      return self.env.compute_reward(verbose=verbose)

    if self.workspace is not None and "eval_script" in self.entry:
      eval_script = self.entry["eval_script"]
      if hasattr(eval_script, "decode"):
        eval_script = eval_script.decode("utf-8")
      eval_command = (
          f"echo '{eval_script}' > /eval.sh && chmod +x /eval.sh && /eval.sh"
      )
      try:
        res = self.workspace.execute_command(
            eval_command, timeout=float(self.reward_timeout)
        )
        is_pass = res.exit_code == 0
        return 1.0 if is_pass else 0.0
      except Exception as e:
  ...
  def _initial_observation(self) -> Any:
    if not self.env and not self.workspace:
      if self.use_agent_sandbox:
        ...
        try:
          ...
          if "SandboxWarmPoolNotFoundError" in type(e).__name__:
            logging.warning(
                "[SWEEnv] Warmpool not found for %s, dynamically warming: %s",
                task.image,
                e,
            )
    ...
    if self.workspace is not None:
      ...
      if isinstance(problem_statement, (list, np.ndarray)):
        problem_statement = (
            problem_statement[0] if len(problem_statement) > 0 else ""
        )
      elif hasattr(problem_statement, "decode"):
        problem_statement = problem_statement.decode("utf-8")
    ...
    if self.scaffold == "openhands" and self.workspace is not None:
      if action_obj.function_name in ("finish", "submit"):
        ...
        return EnvStepResult(
            observation="Task submitted.", reward=reward, done=True, info={}
        )

      if action_obj.function_name == "execute_bash":
        cmd = (
            action_obj.parameters.get("command")
            or action_obj.parameters.get("cmd", "")
        )
      else:
        cmd = action_obj.to_bashcmd()
      ...
      try:
        result = self.workspace.execute_command(
            cmd, timeout=float(self.step_timeout)
        )
        obs = result.stdout
        if result.exit_code != 0 and result.stderr:
          obs = f"{obs}\n{result.stderr}" if obs else result.stderr
      ...
      self.total_steps += 1
      return EnvStepResult(
          observation=str(obs), reward=0, done=False, info=info
      )
