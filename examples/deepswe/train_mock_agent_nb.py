...
# with Multi-turn Agentic framework. (Mock version)
...
parser.add_argument("--max_generation_time", type=float, default=5.0)
...
parser.add_argument("--min_env_wait", type=float, default=0.5)
parser.add_argument("--max_env_wait", type=float, default=2.0)
...
MIN_ENV_WAIT = args.min_env_wait
MAX_ENV_WAIT = args.max_env_wait
...
try:
  ...
  print("✅ tunix and pathways-utils are successfully mapped.")
...
class MockEnv(BaseTaskEnv):
  ...
  def __init__(
      ...
      entry: dict[str, str],
      max_steps: int,
      min_env_wait: float = MIN_ENV_WAIT,
      max_env_wait: float = MAX_ENV_WAIT,
      **kwargs,
  ):
    self.entry = entry
    self.min_env_wait = min_env_wait
    self.max_env_wait = max_env_wait
  ...
  def _step_impl(self, action: Any) -> EnvStepResult:
    # Random artificial wait to mock busy processing
    time.sleep(random.uniform(self.min_env_wait, self.max_env_wait))
...
CONTEXT_RATIO = (
    args.context_ratio
)  # Context length can be up to 2x responselength in DeepSWE due to multi-turn interactions and long responses, so we set context ratio to 2 to accommodate this.
