#!/bin/bash
# Environment launcher: run the single-host v5p REAL-TRAINING convergence run
# (train_v5p_1host_pack.sh) inside the tunix_base_image container on a TPU VM.
# One run yields convergence (wandb loss/reward) + a full-run Perfetto trace +
# a short xprof kernel window.
#
# NOTE: this is NOT the grad-accum profiling wrapper. train_v5p_docker.sh /
# profile_v5p_docker.sh run the optax-vs-stream PROFILING (profile_v5p_grad_
# accum.sh, VARIANTS knob); this one runs the actual training so you can compare
# pack vs unpack CONVERGENCE.
#
# Usage on the TPU VM (docker preinstalled):
#   # packed (segment-aware CL3 + weighted stream)
#   RUN_TAG=cl3_pack bash experimental/train_v5p_1host_docker.sh
...
#   ROLLOUT_HBM=0.4 MAX_STEPS=50 bash experimental/train_v5p_1host_docker.sh
set -uo pipefail
...
PASS_ENV=()
for var in MAX_TOKEN_PER_TPU MAX_SEGMENTS_PER_ROW GRAD_ACCUM ROLLOUT_HBM ROLLOUT_ENGINE \
           MESH_FSDP MESH_TP BATCH MINI MICRO LOGPS MAX_STEPS RUN_TAG \
...
LOG_DIR_HOST="${LOG_DIR:-/tmp/train_v5p_logs}"
