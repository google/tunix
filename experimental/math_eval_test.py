...
# --- MONKEY PATCH FOR TPU-INFERENCE BUG ---
# tpu_inference has a bug where it accesses `model_config.hf_config.text_config.hidden_size`
# for Qwen2, mistakenly assuming it's a Qwen2VL config. We patch it here.
from transformers.models.qwen2.configuration_qwen2 import Qwen2Config
Qwen2Config.text_config = property(lambda self: self)
# ------------------------------------------
  ...
  if NOTEBOOK_ENV == "g3":
    ...
    try:
      if not path.endswith('/'):
        path += '/'
      print(f"\\n[DEBUG] Starting to scan GCS path: {path} with fsspec")
      ...
      for root, dirs, filenames in fs.walk(path):
        for filename in filenames:
          file_count += 1
          if filename.endswith(".safetensors"):
            print(f"[DEBUG] Found safetensors file: {filename}")
            return True
            
      print(f"[DEBUG] Scan finished. Checked {file_count} files, but no .safetensors found.")
      return False
      
    except Exception as e:
      print(f"\\n[DEBUG FATAL ERROR] fsspec crashed! Reason: {e}")
      import traceback
      traceback.print_exc()
      return False
...
class Qwen25MathEvaluator:
    ...
    with self.mesh:
      if "Qwen3" in self.model_version:
        abs_model: nnx.Module = nnx.eval_shape(
            lambda: qwen3_lib.Qwen3(self.model_config, rngs=nnx.Rngs(params=0))
        )
      else:
        abs_model: nnx.Module = nnx.eval_shape(
  ...
  def load_model(self):
    ...
    if has_safetensors(self.model_path) or "torch" in self.model_path:
...
if NOTEBOOK_ENV == "g3":
    ...
    DATA_PATH_PREFIX = "gs://yuxzhang-tunix-models/data"
    MODEL_PATH_PREFIX = "gs://yuxzhang-tunix-models/models"
...
num_batches = int(num_batches_env) if num_batches_env and int(num_batches_env) > 0 else 8  # Evaluate on roughly 1/4 of MATH-500 (8 batches * 16 = 128 samples)
...
print("=" * 60)
# %%
# AIME-2024 removed for this smoke test
